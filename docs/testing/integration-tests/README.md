...
LocalEmu has an extensive set of [integration tests](https://github.com/localemu/localemu/tree/main/tests/integration). This document describes how to run and write integration tests.
...
You can find the list of available fixtures in the [fixtures.py](https://github.com/localemu/localemu/blob/main/src/localemu/testing/pytest/fixtures.py) file.
...
### Test against a running LocalEmu instance

When you run the integration tests, LocalEmu is automatically started (via the pytest conftest mechanism in [tests/integration/conftest.py](https://github.com/localemu/localemu/blob/main/tests/integration/conftest.py)).
You can disable this behavior by setting the environment variable `TEST_SKIP_LOCALEMU_START=1`.
...
3.  Navigate to `Users` and create a new user (**Add Users**)
    1.  Add the username as `localemu-testing`.
...
-   Use the client `fixtures` and other fixtures for resource creation instead of methods from `aws_stack.py`
    -   While using the environment variable `TEST_TARGET=AWS_CLOUD`, the boto client will be automatically configured to target AWS instead of LocalEmu.
...
Once you verified that your test is running against AWS, you can record snapshots for the test run. A snapshot records the response from AWS and can be later on used to compare the response of LocalEmu.

Snapshot tests help to increase the parity with AWS and to raise the confidence in the service implementations. Therefore, snapshot tests are preferred over normal integration tests.
...
When running test with `TEST_TARGET=AWS_CLOUD`, by default, no LocalEmu instance will be created. This can be bypassed by also setting `TEST_FORCE_LOCALEMU_START=1`.
...
local_client = aws_client_factory(
    endpoint_url=f"http://{localemu_host()}",
