...
LocalEmu has multi-account and multi-region support. This document contains some tips to make sure that your contributions are compatible with this functionality.
...
For example:
The test case [`test_apigateway_with_step_function_integration`](https://github.com/localemu/localemu/blob/628b96b44a4fc63d880a4c1238a4f15f5803a3f2/tests/aws/services/apigateway/test_apigateway_basic.py#L999) specifies a [role](https://github.com/localemu/localemu/blob/628b96b44a4fc63d880a4c1238a4f15f5803a3f2/tests/aws/services/apigateway/test_apigateway_basic.py#L1029-L1034) which has permissions to access the target step function account.
```python
...
This can be seen in a case where `account_id` was [added](https://github.com/localemu/localemu/blob/ae31f63bb6d8254edc0c85a66e3c36cd0c7dc7b0/localemu/utils/aws/message_forwarding.py#L42) to [send events to the target](https://github.com/localemu/localemu/blob/ae31f63bb6d8254edc0c85a66e3c36cd0c7dc7b0/localemu/utils/aws/message_forwarding.py#L31) service like SQS, SNS, Lambda, etc.
...
A [scheduled GitHub Actions workflow](https://github.com/localemu/localemu/actions/workflows/aws-tests-mamr.yml) runs on working days at 01:00 UTC, executing the tests with randomized account IDs and regions.
If you have the necessary permissions, you can also manually trigger the [workflow](https://github.com/localemu/localemu/actions/workflows/aws-tests-mamr.yml) directly from GitHub.
...
Importing and using `localemu.constants.TEST_` values is not advised.
