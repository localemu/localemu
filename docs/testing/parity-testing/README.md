...
Parity tests (also called snapshot tests) are a special form of integration tests that should verify and improve the correctness of LocalEmu compared to AWS.

Initially, the integration test is executed against AWS and collects responses of interest. Those responses are called "snapshots" and will be used later on to compare the results from AWS with the ones from LocalEmu.
Those responses aka "snapshots" are stored in a **snapshot.json** file.

Once the snapshot is recorded, the test can be executed against LocalEmu. During this “normal” test execution, the test runs against LocalEmu and compares the LocalEmu responses with the recorded content.

In theory, every integration test can be converted to a parity conform snapshot test.

This guide assumes you are already familiar with writing [integration tests](../integration-tests/README.md) for LocalEmu in general.
...
2.  Add the `snapshot` fixture to your test and identify which responses you want to collect and compare against LocalEmu.
...
3.  Run the test against LocalEmu.
...
The [TransformerUtility](https://github.com/localemu/localemu/blob/main/src/localemu/testing/snapshots/transformer_utility.py) already provides some collections of transformers for specific service APIs.
...
Hint: There are also some simplified transformers in [TransformerUtility](https://github.com/localemu/localemu/blob/main/src/localemu/testing/snapshots/transformer_utility.py).
...
Getting the transformations right can be a tricky task and we appreciate the time you spend on writing parity snapshot tests for LocalEmu! We are aware that it might be challenging to implement transformers that work for AWS and LocalEmu responses.
...
> If specific paths are skipped from the verification, it means LocalEmu does not have parity yet.
...
```bash
localemu.testing.snapshots.transformer: Registering regex pattern '000000000000' in snapshot with '111111111111'
localemu.testing.snapshots.transformer: Registering regex pattern 'us-east-1' in snapshot with '<region>'localemu.testing.snapshots.transformer: Replacing JsonPath '$.json_encoded_delivery..Body.Signature' in snapshot with '<signature>'
localemu.testing.snapshots.transformer: Registering reference replacement for value: '1ad533b5-ac54-4354-a273-3ea885f0d59d' -> '<uuid:1>'
localemu.testing.snapshots.transformer: Replacing JsonPath '$.json_encoded_delivery..MD5OfBody' in snapshot with '<md5-hash>'
localemu.testing.snapshots.transformer: Replacing regex '000000000000' with '111111111111'
localemu.testing.snapshots.transformer: Replacing regex 'us-east-1' with '<region>'
localemu.testing.snapshots.transformer: Replacing '1ad533b5-ac54-4354-a273-3ea885f0d59d' in snapshot with '<uuid:1>'
```
