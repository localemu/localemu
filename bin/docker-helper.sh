...
function _enforce_no_fork() {
    REMOTE_ORIGIN=$(git remote -v | grep 'localemu/' | grep origin | grep push | awk '{print $2}')
    if [[ "$REMOTE_ORIGIN" != 'https://github.com/localemu/'* ]] &&  [[ "$REMOTE_ORIGIN" != 'git@github.com:localemu/'* ]]; then
...
function cmd-build() {
    ...
    DOCKER_BUILDKIT=1 BUILDX_NO_DEFAULT_ATTESTATIONS=1 docker buildx build --platform linux/$PLATFORM --pull --progress=plain \
      ...
      --build-arg LOCALEMU_PRE_RELEASE=$(_is_release_commit && echo "0" || echo "1") \
      --build-arg LOCALEMU_BUILD_GIT_HASH=$(git rev-parse --short HEAD) \
      --build-arg=LOCALEMU_BUILD_DATE=$(date -u +"%Y-%m-%d") \
      --build-arg=LOCALEMU_BUILD_VERSION=$IMAGE_TAG \
...
function cmd-save() {
    ...
    if [ -z "$IMAGE_FILENAME" ]; then
      _enforce_platform
      IMAGE_FILENAME="localemu-docker-image-$PLATFORM.tar.gz";
    fi
