...
from localemu.aws.mocking import Instance, generate_request
from localemu.aws.spec import ServiceCatalog
...
response = requests.get("http://localhost:4566/_localemu/health").content.decode("utf-8")
...
def simulate_call(service: str, op: str) -> RowEntry:
    ...
    if result.get("error_code", "") == "InternalError":
        ...
        if service == "apigateway" and "Unexpected HTTP method" in error_msg:
            ...
            "localemu.aws.protocol.parser.ProtocolParserError: Unable to parse request (not well-formed (invalid token)"
            in error_msg
        ):
            ...
            # sometimes LocalEmu's autogenerated API is not yet up-to-date, which could cause
