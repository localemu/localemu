...
- *origin*: indicates how the request was triggered, valid values are: `internal` and `external` . Internal means that the request was made from localemu, external means that it was triggered explicitly during the test.
...
In CircleCI, we have two jobs that test for implemented/not implemented APIs for each service. One job is running for LocalEmu Community version, the other one for LocalEmu Pro.
...
Currently, the coverage is used to generate the [docs coverage page](https://localemu.cloud/docs/services), which is updated once a week.
...
def send_metadata_for_build(build_id: str, timestamp: str):
    ...
    CIRCLE_PULL_REQUESTS=https://github.com/localemu/localemu/pull/7324
    CIRCLE_BRANCH=coverage-tinybird
    CIRCLE_BUILD_NUM=78206
    CIRCLE_BUILD_URL=https://circleci.com/gh/localemu/localemu/78206
