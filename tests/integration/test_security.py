...
from localemu.aws.handlers import cors as cors_handler
from localemu.aws.handlers.cors import _get_allowed_cors_origins
...
class TestCSRF:
    ...
    def test_default_cors_headers(self):
        headers = {"Origin": "https://app.localemu.cloud"}
        response = requests.get(
            f"{config.internal_service_url()}/2015-03-31/functions/", headers=headers
        )
        assert response.status_code == 200
        assert response.headers["access-control-allow-origin"] == "https://app.localemu.cloud"
        assert "GET" in response.headers["access-control-allow-methods"].split(",")
        assert response.headers["Vary"] == "Origin"
        assert response.headers["Access-Control-Allow-Credentials"] == "true"

    @pytest.mark.parametrize("path", ["/_localemu/health"])
    ...
    def test_disable_cors_checks(self, monkeypatch):
        ...
        headers = {"Origin": "https://invalid.localemu.cloud"}
    ...
    def test_cors_apigw_not_applied(self, aws_client):
        ...
        response = requests.get(
            ...
            verify=False,
            headers={
                "Origin": "https://app.localemu.cloud",
            },
        )
        ...
        host_header = "myapiid.execute-api.localhost:4566"
        response = requests.get(
            rest_api_url_host,
            verify=False,
            headers={
