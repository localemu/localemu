...
from localemu.utils import docker_utils
from localemu.utils.common import is_ipv4_address, save_file, short_uid, to_str
...
from localemu.utils.container_utils.docker_sdk_client import SdkDockerClient
from localemu.utils.docker_utils import (
...
from localemu.utils.net import (
    Port,
...
def authenticated_registry(docker_client: ContainerClient, create_container):
    ...
    registry_container = create_container(
        ...
        env_vars={
            ...
            "REGISTRY_HTTP_SECRET": "localemu-registry-secret",
        },
        command=[
            "sh",
            "-c",
...
class TestDockerClient:
    ...
    def test_pull_non_existent_docker_image(self, docker_client: ContainerClient):
        with pytest.raises(NoSuchImage):
            docker_client.pull_image("localemu_non_existing_image_for_tests")
    ...
    def test_push_non_existent_docker_image(self, docker_client: ContainerClient):
        with pytest.raises(NoSuchImage):
            docker_client.push_image("localemu_non_existing_image_for_tests")
    ...
    def test_push_invalid_registry(self, docker_client: ContainerClient):
        image_name = f"localhost:{get_free_tcp_port()}/localemu_dummy_image"
        try:
            docker_client.tag_image("alpine", image_name)
    ...
    def test_tag_image(self, docker_client: ContainerClient):
        ...
        _pull_image_if_not_exists(docker_client, "alpine")
        img_refs = [
            "localemu_dummy_image",
            "localemu_dummy_image:latest",
            "localemu_dummy_image:test",
            "docker.io/localemu_dummy_image:test2",
            "example.com:4510/localemu_dummy_image:test3",
        ]
        try:
    ...
    def test_tag_non_existing_image(self, docker_client: ContainerClient):
        with pytest.raises(NoSuchImage):
            docker_client.tag_image(
                "localemu_non_existing_image_for_tests", "localemu_dummy_image"
            )
    ...
    def test_run_container_non_existent_image(self, docker_client: ContainerClient):
        ...
        with pytest.raises(NoSuchImage):
            stdout, _ = docker_client.run_container(
                "localemu_non_existing_image_for_tests", command=["echo", "test"], remove=True
            )
...
class TestRunWithAdditionalArgs:
    ...
    def test_run_with_additional_arguments_add_host(self, docker_client: ContainerClient):
        additional_flags = "--add-host sometest.localemu.cloud:127.0.0.1"
        stdout, _ = docker_client.run_container(
            "alpine",
            remove=True,
            command=["getent", "hosts", "sometest.localemu.cloud"],
            additional_flags=additional_flags,
        )
        stdout = to_str(stdout)
        assert "127.0.0.1" in stdout
        assert "sometest.localemu.cloud" in stdout
...
class TestDockerNetworking:
    ...
    def test_connect_container_to_network_with_alias_and_disconnect(
        ...
        network_name = f"ls_test_network_{short_uid()}"
        container_alias = f"test-container-{short_uid()}.localemu.cloud"
        create_network(network_name)
        container = create_container("alpine", command=["sh", "-c", "while true; do sleep 1; done"])
        docker_client.start_container(container.container_id)
        docker_client.connect_container_to_network(
...
def set_ports_check_image_alpine(monkeypatch):
    """Set the ports check Docker image to 'alpine', to avoid pulling the larger localemu image in the tests"""
