...
from localemu.http.websocket import WebSocketRequest
from localemu.services.edge import ROUTER


class TestExceptionHandlers:
    def test_internal_failure_handler_http_errors(self):
        response = requests.delete(config.internal_service_url() + "/_localemu/health")
...
class TestHttps:
    def test_default_cert_works(self):
        response = requests.get(
            config.internal_service_url(host="localhost", protocol="https")
            + "/_localemu/health",
        )
        assert response.ok
...
class TestWebSocketIntegration:
    ...
    def test_ssl_websockets(self, cleanups):
        ...
        url = (
            config.internal_service_url(host="localhost", protocol="wss")
            + "/_ws/world"
        )
        socket = websocket.WebSocket()
        socket.connect(url)
...
class TestHTTP2Support:
    ...
    def _fix_proxy(self, monkeypatch):
        ...
        monkeypatch.setenv("no_proxy", "localhost,localhost,127.0.0.1")

    def test_http2_http(self):
        host = config.internal_service_url(host="localhost", protocol="http")
        with httpx.Client(http1=False, http2=True) as client:
            assert client.get(f"{host}/_localemu/health").status_code == 200
