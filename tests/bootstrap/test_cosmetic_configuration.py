...
from localemu.testing.pytest.container import ContainerFactory, LogStreamFactory
from localemu.testing.scenario.cdk_lambda_helper import load_python_lambda_to_s3
...
@pytest.fixture(scope="class")
def chosen_localemu_host() -> str:
    """
    Choose a domain name that is guaranteed never to resolve
...
def container(
    ...
    chosen_localemu_host,
):
    ls_container = class_container_factory(
        configurators=[
            ContainerConfigurators.mount_localemu_volume(),
            ...
            ContainerConfigurators.env_vars(
                {
                    "LOCALEMU_HOST": chosen_localemu_host,
                }
            ),
        ]
    )
...
class TestLocalEmuHost:
    """
    Scenario test that runs LocalEmu in a docker container with `LOCALEMU_HOST` set to a
    non-default value. This ensures that setting the cosmetic "LOCALEMU_HOST" does not affect
    the internal functionality of LocalEmu.
    """
    ...
    def infrastructure(
        ...
        infra: InfraProvisioner = infrastructure_setup(
            namespace="LocalEmuHostBootstrap",
            port=port,
        )
        ...
        def create_lambda_function(
            ...
            **kwargs,
            ...
            base_environment = {"CUSTOM_LOCALEMU_HOSTNAME": chosen_localemu_host}
    ...
    def test_scenario(
        self, port, infrastructure, aws_client_factory, chosen_localemu_host, region_name
    ):
        """
        Scenario:
        ...
        assert chosen_localemu_host in stack_outputs["DomainEndpoint"]
        health_url = stack_outputs["DomainEndpoint"].replace(
            chosen_localemu_host, constants.LOCALHOST_HOSTNAME
        )
        # we only have a route matcher for the domain with localemu_host in the URL,
        ...
        assert chosen_localemu_host in stack_outputs["ApiUrl"]
        api_url = (
            stack_outputs["ApiUrl"]
            .rstrip("/")
            .replace(chosen_localemu_host, constants.LOCALHOST_HOSTNAME)
        )
