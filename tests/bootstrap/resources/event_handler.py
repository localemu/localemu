...
def handler(event, context):
    custom_localemu_hostname = os.environ["CUSTOM_LOCALEMU_HOSTNAME"]
    domain_endpoint = os.environ["DOMAIN_ENDPOINT"]
    results_bucket = os.environ["RESULTS_BUCKET"]
    results_key = os.environ["RESULTS_KEY"]
    assert custom_localemu_hostname in domain_endpoint, (
        f"{custom_localemu_hostname} not in {domain_endpoint}"
    )
    ...
    for record in event["Records"]:
        ...
        assert custom_localemu_hostname in body["UnsubscribeURL"]
