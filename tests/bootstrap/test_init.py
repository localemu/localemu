...
class TestInitHooks:
    def test_shutdown_hooks(
        ...
        shutdown_00.write_text("#!/bin/bash\necho 'foobar' > /var/lib/localemu/shutdown_00.log")
        shutdown_01.write_text(
            "#!/bin/bash\ncurl -s localhost:4566/_localemu/init &> /var/lib/localemu/shutdown_01.log"
        )
        ...
        container = container_factory(
            configurators=[
                ...
                ContainerConfigurators.volume(
                    BindMount(str(shutdown_hooks), "/etc/localemu/init/shutdown.d")
                ),
            ]
        )
        ...
        r = requests.get("http://127.0.0.1:4566/_localemu/init")
