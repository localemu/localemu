...
import localemu.aws.accounts
from localemu.aws.api import RequestContext, ServiceException, handler
from localemu.aws.forwarder import NotImplementedAvoidFallbackError
...
def test_call_s3_with_streaming_trait(payload, monkeypatch):
    monkeypatch.setenv("MOTO_S3_CUSTOM_ENDPOINTS", "s3.localhost:4566")

    # In this test we use low-level interface with Moto and skip the standard setup
    # In the absence of below patch, Moto and LocalEmu uses difference AWS Account IDs causing the test to fail
    monkeypatch.setattr(localemu.aws.accounts, "DEFAULT_AWS_ACCOUNT_ID", DEFAULT_MOTO_ACCOUNT_ID)
...
def test_call_with_sns_with_full_uri():
    ...
    # see https://github.com/localemu/localemu/pull/8962
