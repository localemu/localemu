...
from localemu.utils.files import new_tmp_file, save_file
from localemu.utils.strings import short_uid


class TestOpenSearch:
    """
    OpenSearch does not respect any customisations and just returns a domain with localhost in.
    """
    ...
    * LOCALEMU_HOST
    ...
    def test_off_strategy_with_external_port(
        ...
        monkeypatch.setattr(
            config,
            "LOCALEMU_HOST",
            config.HostAndPort(host=localemu_host().host, port=external_port),
        )
