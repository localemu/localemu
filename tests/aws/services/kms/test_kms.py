...
from localemu.services.kms.utils import get_hash_algorithm
from localemu.testing.aws.util import in_default_partition, is_aws_cloud
from localemu.testing.pytest import markers
from localemu.utils.crypto import encrypt
...
class TestKMS:
    ...
    def test_decrypt_recipient(self, kms_create_key, aws_client):
        ...
        as an only-Localemu test because of the difficulty of spinning up a true Nitro enclave to generate a real
...
class TestKMSGenerateKeys:
    ...
    def test_generate_data_key(self, kms_key, aws_client, snapshot):
        ...
        key_id = kms_key["KeyId"]
        # LocalEmu currently doesn't act on KeySpec or on NumberOfBytes params, but one of them has to be set.
