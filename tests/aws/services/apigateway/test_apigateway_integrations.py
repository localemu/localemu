...
from localemu.aws.api.apigateway import IntegrationType
from localemu.aws.api.ec2 import VpcEndpoint
...
@pytest.mark.skipif(
    ...
    reason="calling the vpce from a lambda requires LocalEmu to run in docker",
)
def test_create_execute_api_vpc_endpoint(
    ...
    # LocalEmu
    #   url: http://localhost:4566/{stage}
    #   host: {rest-api-id}-{vpce-id}.execute-api.localhost
    ...
    # LocalEmu
    #   url: http://{public-dns-hostname}.execute-api.{region}.vpce.{localemu-host}/{stage}
    #   x-apigw-api-id: {rest-api-id}
    ...
    #   host: {rest-api-id}.execute-api.{region}.amazonaws.com
    # LocalEmu
    #   url: http://{public-dns-hostname}.execute-api.{region}.vpce.{localemu_host}/{stage}
    #   host: {rest-api-id}.execute-api.{region}.{localemu-host}
        ...
        def _invoke_api():
            response = requests.get(
                ...
                headers={
                    ...
                    "Upgrade": "HTTP/2.0",
                    "User-Agent": "localEmu/0.0",
                    "Via": "p.example.net",
                    "Warn": "299 localEmu/0.0",
