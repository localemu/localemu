...
from localemu.aws.api.apigateway import Resources
...
def apigateway_placeholder_authorizer_lambda_invocation_arn(
    ...
    LocalEmu does not validate the ARN here, so we can simply return a placeholder
    """
    if not is_aws_cloud():
...
class TestApiGatewayImportRestApi:
    ...
    def test_import_rest_apis_with_base_path_swagger(
        ...
        if is_aws_cloud():
            ...
            # then you realize LocalEmu is needed!
            time.sleep(20)
    ...
    @markers.snapshot.skip_snapshot_verify(
        paths=[
            ...
            "$..name",
            "$..rootResourceId",
            # not returned even if empty in LocalEmu
            "$.get-rest-api.tags",
        ]
    )
