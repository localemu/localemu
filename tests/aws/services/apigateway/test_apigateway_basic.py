...
from localemu.aws.handlers import cors
from localemu.constants import TAG_KEY_CUSTOM_ID
from localemu.services.apigateway.helpers import (
    host_based_url,
    localemu_path_based_url,
    path_based_url,
)
...
from localemu.testing.aws.util import in_default_partition
...
from localemu.utils.aws import resources as resource_util
...
class TestAPIGateway:
    ...
    @pytest.mark.parametrize(
        "url_function", [path_based_url, host_based_url, localemu_path_based_url]
    )
    ...
    def test_create_rest_api_with_custom_id(self, create_rest_apigw, url_function, aws_client):
        if not is_next_gen_api() and url_function == localemu_path_based_url:
    ...
    def _get_invoke_endpoint(
        ...
        if use_hostname:
            host = f"{api_id}.execute-api.{localemu_host().host}"
    ...
    @markers.snapshot.skip_snapshot_verify(
        paths=[
            ...
            "$..log.line12",
            # LocalEmu does not setup headers the same way when invoking the lambda (Token, additional headers...)
            "$..log.line08",
        ]
    )
    @markers.snapshot.skip_snapshot_verify(
