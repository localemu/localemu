...
from localemu.aws.api.stepfunctions import InspectionLevel
...
class TestStateCaseScenarios:
    ...
    @markers.aws.validated
    def test_localemu_blogpost_scenario(
        self,
        aws_client,
        aws_client_no_sync_prefix,
        sfn_snapshot,
        ...
        template = TSMT.load_sfn_template(TSMT.LOCALEMU_BLOGPOST_SCENARIO_STATE_MACHINE)
    ...
    def test_state_with_variables(
        ...
        )  # roleArn is null in AWS but is not null in LocalEmu, see comment in provider implementation. If this transformer is not added then roleArn is picked up as another resource name in LocalEmu, causing resource placeholders mismatch, e.g. <resource:3> where <resource:2> is expected.
        sfn_snapshot.add_transformer(sfn_snapshot.transform.resource_name())
