...
s3_control_endpoint = f"https://{localemu_host().host_and_port()}"
...
def s3control_snapshot(snapshot):
    snapshot.add_transformers_list(
        [
            ...
            snapshot.transform.regex(localemu_host().host_and_port(), "<endpoint-host>"),
...
@pytest.mark.skip("Not implemented yet in LocalEmu")
...
@pytest.mark.skip("Not implemented yet in LocalEmu, missing link with S3")
...
@markers.snapshot.skip_snapshot_verify(
    paths=[
        # FIXME: this needs to be updated in the serializer, see https://github.com/localemu/localemu/pull/9730
        "$..HostId",
    ]
)
