...
from localemu.utils.strings import checksum_crc32, short_uid
...
def snapshot_headers(snapshot):
    # should remove localemu specific headers as well
...
@markers.snapshot.skip_snapshot_verify(
    paths=["$..x-amz-id-2"]  # we're currently using a static value in LocalEmu
)
class TestS3Cors:
    ...
    def test_cors_no_config_localemu_allowed(self, s3_bucket, aws_client):
        key = "test-cors-get-no-config"
        body = "cors-test"
        response = aws_client.s3.put_object(Bucket=s3_bucket, Key=key, Body=body, ACL="public-read")
        assert response["ResponseMetadata"]["HTTPStatusCode"] == 200
    ...
    def test_cors_list_buckets(self, region_name):
        ...
        # it should follow the default rules of LocalEmu
