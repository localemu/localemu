...
from localemu.testing.snapshots.transformer import RegexTransformer
from urllib3 import HTTPHeaderDict

from localemu import config, constants
from localemu.aws.api.lambda_ import Runtime
from localemu.aws.api.s3 import StorageClass, TransitionDefaultMinimumObjectSize
from localemu.config import S3_VIRTUAL_HOSTNAME
from localemu.constants import (
    AWS_REGION_US_EAST_1,
    LOCALHOST_HOSTNAME,
)
from localemu.services.s3 import constants as s3_constants
from localemu.services.s3.headers import encode_header_rfc2047
...
from localemu.testing.aws.util import create_client_with_keys, in_default_partition, is_aws_cloud
from localemu.testing.config import (
    SECONDARY_TEST_AWS_ACCESS_KEY_ID,
    SECONDARY_TEST_AWS_SECRET_ACCESS_KEY,
    TEST_AWS_ACCESS_KEY_ID,
)
from localemu.testing.pytest import markers
from localemu.testing.snapshots.transformer_utility import TransformerUtility
from localemu.utils import testutil
from localemu.utils.aws.arns import get_partition
from localemu.utils.aws.request_context import mock_aws_request_headers
from localemu.utils.aws.resources import create_s3_bucket
from localemu.utils.files import load_file
from localemu.utils.run import run
from localemu.utils.strings import (
    checksum_crc32,
    checksum_crc32c,
    checksum_crc64nvme,
    hash_sha1,
...
from localemu.utils.testutil import check_expected_lambda_log_events_length
from localemu.utils.urls import localemu_host as get_localemu_host
...
class TestS3:
    ...
    def test_metadata_header_character_decoding(self, s3_bucket, snapshot, aws_client):
        ...
        # https://github.com/localemu/localemu/issues/1790
        # put object
    ...
    def test_delete_bucket_no_such_bucket(self, snapshot, aws_client):
        with pytest.raises(ClientError) as e:
            aws_client.s3.delete_bucket(Bucket="does-not-exist-localemu-test")
    ...
    def test_range_header_body_length(self, s3_bucket, snapshot, aws_client):
        # Test for https://github.com/localemu/localemu/issues/1952
    ...
    def test_put_object_chunked_newlines(self, s3_bucket, aws_client, region_name):
        ...
        # Test for https://github.com/localemu/localemu/issues/1571
        object_key = "data"
        body = "Hello\r\n\r\n\r\n\r\n"
        headers = {
    ...
    def test_put_object_chunked_newlines_with_trailing_checksum(
        ...
        # Boto still does not support chunk encoding, which means we can't test with the client nor
        # aws_http_client_factory. See open issue: https://github.com/boto/boto3/issues/751
        # Test for https://github.com/localemu/localemu/issues/6659
        object_key = "data"
        body = "Hello Blob"
    ...
    def test_upload_part_chunked_newlines_valid_etag(self, s3_bucket, aws_client, region_name):
        # Boto still does not support chunk encoding, which means we can't test with the client nor
        # aws_http_client_factory. See open issue: https://github.com/boto/boto3/issues/751
        # Test for https://github.com/localemu/localemu/issues/8703
        body = "Hello Blob"
    ...
    def test_upload_part_chunked_cancelled_valid_etag(self, s3_bucket, aws_client, region_name):
        ...
        This test is to avoid regression for https://github.com/localemu/localemu/issues/9851
        """
        body = "Hello Blob"
        precalculated_etag = hashlib.md5(body.encode()).hexdigest()
        headers = {
    ...
    def test_put_object_with_md5_and_chunk_signature(self, s3_bucket, aws_client):
        # Boto still does not support chunk encoding, which means we can't test with the client nor
        # aws_http_client_factory. See open issue: https://github.com/boto/boto3/issues/751
        # Test for https://github.com/localemu/localemu/issues/4987
    ...
    @markers.aws.validated
    @markers.requires_in_process  # Patches LOCALEMU_HOST
    def test_set_external_hostname(
        ...
        if not is_aws_cloud():
            expected_url = (
                f"{_bucket_url(bucket_name=s3_bucket, localemu_host=custom_hostname)}/{key}"
            )
        ...
        download_url = response["Location"].replace(
            f"{get_localemu_host().host}:", "localhost:"
        )
    ...
    def test_s3_hostname_with_subdomain(self, aws_http_client_factory, aws_client):
        """
        This particular test validates the fix for localemu#7424
    ...
    def test_create_bucket_aws_global(
        ...
        from localemu.aws.handlers.region import DefaultRegionRewriterStrategy
    ...
    def test_create_bucket_head_bucket(
        ...
        snapshot.add_transformers_list(
            [
                ...
                snapshot.transform.regex(r"s3\.localhost\.localemu\.cloud:4566", "<host>"),
                snapshot.transform.regex(r"s3\.localhost\.localemu\.cloud:443", "<host>"),
    ...
    def test_s3_batch_delete_objects_using_requests_with_acl(
        ...
        url = f"{_bucket_url(s3_bucket, localemu_host=get_localemu_host().host)}?delete"
    ...
    def test_s3_raw_request_routing(self, s3_bucket, aws_client):
        ...
        """
        default_endpoint = f"http://{get_localemu_host().host_and_port()}"
...
class TestS3PresignedUrl:
    ...
    def test_presign_with_additional_query_params(
        ...
        """related to issue: https://github.com/localemu/localemu/issues/4133"""
    ...
    def test_put_url_metadata_with_sig_s3v4(
        ...
        # https://github.com/localemu/localemu/issues/544
        metadata = {"foo": "bar"}
        object_key = "key-by-hostname"
        ...
        # if we skip validation, it should work for LocalEmu
        if not verify_signature and not is_aws_cloud():
            assert response.ok, f"response returned {response.status_code}: {response.text}"
            # response body should be empty, see https://github.com/localemu/localemu/issues/1317
    ...
    def test_pre_signed_url_forward_slash_bucket(
        ...
        # however the signature will not match afterward (in AWS or with LocalEmu)
...
class TestS3DeepArchive:
    ...
    def test_s3_get_deep_archive_object_restore(self, s3_create_bucket, snapshot, aws_client):
        ...
        if 'ongoing-request="false"' in response.get("Restore", ""):
            # if the restoring happens in LocalEmu (or was fast in AWS) we can retrieve the object
...
class TestS3StaticWebsiteHosting:
    ...
    def test_routing_rules_conditions(self, s3_bucket, aws_client, allow_bucket_acl):
        # https://github.com/localemu/localemu/issues/6308
...
# TODO: maybe we can fake the IAM role as it's not needed in LocalEmu
...
class TestS3PresignedPost:
    ...
    def test_post_object_with_file_as_string(self, s3_bucket, aws_client, snapshot):
        # this is a test for https://github.com/localemu/localemu/issues/10309
...
# LocalEmu does not apply encryption, so the ETag is different
@markers.snapshot.skip_snapshot_verify(paths=["$..ETag"])
...
def _endpoint_url(region: str = "", localemu_host: str = None) -> str:
    if not region:
    ...
    if region == "us-east-1":
        return f"{config.internal_service_url(host=localemu_host or S3_VIRTUAL_HOSTNAME)}"
    return config.internal_service_url(host=f"s3.{region}.{LOCALHOST_HOSTNAME}")


def _bucket_url(bucket_name: str, region: str = "", localemu_host: str = None) -> str:
    return f"{_endpoint_url(region, localemu_host)}/{bucket_name}"
...
def _website_bucket_url(bucket_name: str):
    ...
    return _bucket_url_vhost(bucket_name, localemu_host=config.S3_STATIC_WEBSITE_HOSTNAME)


def _bucket_url_vhost(bucket_name: str, region: str = "", localemu_host: str = None) -> str:
    if not region:
        region = AWS_REGION_US_EAST_1
    if is_aws_cloud():
        if region == "us-east-1":
    ...
    host_definition = get_localemu_host()
    if localemu_host:
        host_and_port = f"{localemu_host}:{config.GATEWAY_LISTEN[0].port}"
    else:
        host_and_port = (
