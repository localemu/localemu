...
from localemu.services.dynamodbstreams.dynamodbstreams_api import get_kinesis_stream_name
from localemu.testing.aws.lambda_utils import _await_dynamodb_table_active
...
from localemu.utils.aws import arns, queries, resources
from localemu.utils.aws.arns import kinesis_stream_arn
from localemu.utils.aws.resources import create_dynamodb_table
from localemu.utils.common import json_safe, long_uid, retry, short_uid
from localemu.utils.sync import poll_condition, wait_until
...
class TestDynamoDB:
    ...
    @markers.aws.only_localemu(reason="AWS has a 20 GSI limit")
    ...
    # and the ones of the replicas. In LocalEmu, it does not differ. The only difference in the stream ARNs is the
    ...
    def test_streams_on_global_tables(
        ...
        """
        This test exposes an issue in LocalEmu with Global tables and streams. In AWS, each regional replica should
        get a separate DynamoDB Stream. This does not happen in LocalEmu since DynamoDB Stream does not have any
    ...
    def test_batch_write_items_streaming(
        ...
        # because LocalEmu is multithreaded, it's not guaranteed those requests are going to be executed in order
    ...
    def test_nosql_workbench_localhost_region(self, dynamodb_create_table, aws_client_factory):
        ...
        LocalEmu must assume "us-east-1" region in such cases.
        """
