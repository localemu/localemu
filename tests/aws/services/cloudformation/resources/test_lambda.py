...
from localemu.aws.api.lambda_ import InvocationType, Runtime, State
...
from localemu.utils.common import short_uid
...
from localemu.utils.testutil import create_lambda_archive, get_lambda_log_events
...
@markers.snapshot.skip_snapshot_verify(
    [
        # TODO: Fix flaky ESM state mismatch upon update in LocalEmu (expected Enabled, actual Disabled)
...
@markers.aws.only_localemu(reason="This is functionality specific to Localemu")
def test_lambda_cfn_run_with_empty_string_replacement_deny_list(
    ...
    # URLs that match regex to capture AWS URLs gets Localemu port appended - non-matching URLs remain unchanged.
...
class TestCfnLambdaIntegrations:
    ...
    @markers.snapshot.skip_snapshot_verify(
        paths=[
            ...
            "$..StackResources..PhysicalResourceId",  # TODO: compatibility between AWS URL and localemu URL
        ]
    )
    @markers.aws.validated
