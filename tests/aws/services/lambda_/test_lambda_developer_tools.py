...
from localemu.utils.container_networking import get_main_container_network
from localemu.utils.docker_utils import DOCKER_CLIENT, get_host_path_for_path_in_docker
from localemu.utils.files import load_file, mkdir, rm_rf
...
class TestLambdaNetworking:
    @markers.aws.only_localemu
    def test_lambda_localhost_gateway_connectivity(
        ...
        result = aws_client.lambda_.invoke(
            ...
            Payload=json.dumps(
                {
                    "url": f"http://localhost:{config.GATEWAY_LISTEN[0].port}/_localemu/health"
                }
            ),
        )
