...
from localemu.testing.snapshots.transformer import KeyValueBasedTransformer

from localemu import config
from localemu.aws.api.lambda_ import Architecture, InvocationType, InvokeMode, Runtime
...
from localemu.services.lambda_.runtimes import RUNTIMES_AGGREGATED
...
from localemu.testing.aws.util import (
    create_client_with_keys,
    is_aws_cloud,
)
from localemu.testing.pytest import markers
from localemu.testing.snapshots.transformer_utility import PATTERN_UUID
from localemu.utils import files, platform, testutil
from localemu.utils.aws import arns
from localemu.utils.aws.arns import get_partition, lambda_function_name
from localemu.utils.files import load_file
from localemu.utils.http import safe_requests
from localemu.utils.platform import Arch, standardized_arch
from localemu.utils.strings import short_uid, to_bytes, to_str
from localemu.utils.sync import retry, wait_until
...
class TestLambdaBaseFeatures:
    ...
    def test_assume_role(
        ...
        due to an invalid role ARN: https://github.com/localemu/localemu/issues/9016
...
class TestLambdaBehavior:
    ...
    def test_ignore_architecture(self, create_lambda_function, monkeypatch, aws_client):
        ...
        # Assumes that LocalEmu runs on native Docker host architecture
    ...
    @markers.snapshot.skip_snapshot_verify(
        paths=[
            ...
            "$..Payload.environment.LOCALEMU_HOSTNAME",
    ...
    def test_lambda_host_prefix_api_operation(self, create_lambda_function, aws_client, snapshot):
        """Ensure that API operations with a hostPrefix are forwarded to the LocalEmu instance. Examples:
...
class TestLambdaURL:
    ...
    def test_lambda_url_persists_after_alias_delete(
        ...
        # TODO: The below will fail on LocalEmu since URL deletions are async.
...
class TestLambdaErrors:
    ...
    @markers.requires_in_process
    @markers.aws.only_localemu(
        reason="Can only induce Lambda-internal Docker error in LocalEmu"
    )
    def test_lambda_runtime_startup_timeout(
        ...
        monkeypatch.setattr(
            config, "LAMBDA_DOCKER_FLAGS", "-e LOCALEMU_RUNTIME_ENDPOINT=http://somehost.invalid"
        )
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            # aws lambda invoke --debug --function-name localemu-lambda-url-example \
...
class TestLambdaCleanup:
    ...
    def test_recreate_function(
        ...
        Reproduces a GitHub issue: https://github.com/localemu/localemu/issues/10280"""
        function_name = f"test-function-{short_uid()}"
