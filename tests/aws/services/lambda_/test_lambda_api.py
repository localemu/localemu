...
from localemu.testing.snapshots.transformer import SortingTransformer
...
from localemu.services.lambda_.api_utils import ARCHITECTURES
from localemu.services.lambda_.provider import TAG_KEY_CUSTOM_URL
from localemu.services.lambda_.provider_utils import LambdaLayerVersionIdentifier
from localemu.services.lambda_.runtimes import (
    ALL_RUNTIMES,
    DEPRECATED_RUNTIMES,
    SNAP_START_SUPPORTED_RUNTIMES,
)
from localemu.testing.aws.lambda_utils import (
...
from localemu.utils.strings import long_uid, short_uid, to_str
from localemu.utils.sync import ShortCircuitWaitException, wait_until
from localemu.utils.testutil import create_lambda_archive
...
class TestLambdaFunction:
    ...
    def test_function_name_and_qualifier_validation(
        ...
        if (
            ...
            in (
                ...
                # TODO: both are 'valid' but LocalEmu does not include the version qualifier '$LATEST' in raised NotFound exception
    ...
    def test_invalid_vpc_config_security_group(
        ...
        """
        # TODO: maybe add validation of security group id, not currently validated in LocalEmu
    ...
    def test_lambda_concurrent_code_updates(
        ...
        # to be able to reliably capture the in-progress update state in LocalEmu
        from localemu.services.lambda_.invocation import docker_runtime_executor
        from localemu.services.lambda_.invocation.docker_runtime_executor import (
...
class TestLambdaProvisionedConcurrency:
    ...
    def test_provisioned_concurrency_limits(
        ...
        # Adjust limits in LocalEmu to avoid creating a Lambda fork-bomb
        monkeypatch.setattr(config, "LAMBDA_LIMITS_CONCURRENT_EXECUTIONS", 5)
        monkeypatch.setattr(config, "LAMBDA_LIMITS_MINIMUM_UNRESERVED_CONCURRENCY", 3)
    ...
    def test_lambda_provisioned_lifecycle(
        ...
        # TODO: implement updates while IN_PROGRESS in LocalEmu (currently not supported)
...
class TestLambdaLayer:
    ...
    @markers.requires_in_process
    @markers.aws.only_localemu(reason="Deterministic id generation is LS only")
    def test_layer_deterministic_version(
        ...
        Ensuring we can control the version of the layer created through the LocalemuIdManager
        """
        layer_name = f"testlayer-{short_uid()}"
...
class TestLambdaSnapStart:
    ...
    def test_snapstart_lifecycle(self, multiruntime_lambda, snapshot, aws_client):
        """Test the API of the SnapStart feature. The optimization behavior is not supported in LocalEmu.
    ...
    def test_snapstart_exceptions(self, lambda_su_role, snapshot, aws_client):
        ...
        with pytest.raises(ClientError) as e:
            aws_client.lambda_.create_function(
                FunctionName=function_name,
                Handler="cloud.localemu.sample.LambdaHandlerWithLib",
                Code={"ZipFile": zip_file_bytes},
                PackageType="Zip",
                Role=lambda_su_role,
...
class TestLambdaEndpoints:
    ...
    def test_s3_code_url(
        self, aws_client, create_lambda_function_aws, lambda_su_role, monkeypatch, localemu_host
    ):
        monkeypatch.setattr(config, "LOCALEMU_HOST", localemu_host)
        function_name = f"function-{short_uid()}"
        zip_file_bytes = create_lambda_archive(load_file(TEST_LAMBDA_PYTHON_ECHO), get_content=True)
        create_lambda_function_aws(
            FunctionName=function_name,
        ...
        assert s3_code_url.startswith(f"http://{localemu_host}/")
