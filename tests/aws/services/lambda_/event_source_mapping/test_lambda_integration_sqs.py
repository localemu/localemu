...
from localemu.testing.snapshots.transformer import KeyValueBasedTransformer, SortingTransformer

from localemu.aws.api.lambda_ import InvalidParameterValueException, Runtime
from localemu.config import is_env_true
from localemu.testing.aws.lambda_utils import _await_event_source_mapping_enabled
...
def test_redrive_policy_with_failing_lambda(
    ...
    https://github.com/localemu/localemu/issues/5283)"""
...
def test_fifo_message_group_parallelism(
    ...
    # https://github.com/localemu/localemu/issues/7036
...
class TestSQSEventSourceMapping:
    ...
    def test_sqs_event_source_mapping_batch_size(
        ...
        # Intentional parity difference to speed up testing in LocalEmu
        snapshot.add_transformer(
            snapshot.transform.key_value(
        ...
        create_event_source_mapping_response = aws_client.lambda_.create_event_source_mapping(
            ...
            FunctionName=function_name,
            # Speed up testing in LocalEmu by waiting only up to 2s instead of up to 10s; AWS is slower.
