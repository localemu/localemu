...
from localemu.testing.snapshots.transformer import GenericTransformer

from localemu import config
from localemu.aws.api.lambda_ import Runtime
from localemu.services.sqs.constants import DEFAULT_MAXIMUM_MESSAGE_SIZE, SQS_UUID_STRING_SEED
from localemu.services.sqs.models import sqs_stores
from localemu.services.sqs.provider import MAX_NUMBER_OF_MESSAGES
from localemu.services.sqs.utils import parse_queue_url
...
from localemu.utils.common import poll_condition, retry, short_uid, short_uid_from_seed, to_str
from localemu.utils.strings import token_generator
...
class TestSqsProvider:
    ...
    def test_get_queue_url_contains_localemu_host(
        self,
        sqs_create_queue,
        monkeypatch,
        aws_sqs_client,
    ...
    def test_list_queues_pagination(self, sqs_create_queue, aws_client, snapshot):
        ...
        snapshot.add_transformer(
            snapshot.transform.regex(
                ...
                r"http://sqs.\1.localhost:4566",
            )
        )

        url = f"http://sqs.<region>.localhost:4566/111111111111/{test_output_identifier}-test-queue-{max_result_1 - 1}"
        snapshot.add_transformer(
            snapshot.transform.regex(
                r'("NextToken":\s*")[^"]*(")',
    ...
    def test_create_queue_recently_deleted_cache(
        ...
        from localemu.services.sqs import constants
    ...
    def test_send_message_with_delay_0_works_for_fifo(self, sqs_create_queue, aws_sqs_client):
        # see issue https://github.com/localemu/localemu/issues/6612
        queue_name = f"queue-{short_uid()}.fifo"
        attributes = {"FifoQueue": "true"}
        queue_url = sqs_create_queue(QueueName=queue_name, Attributes=attributes)
    ...
    @markers.aws.validated
    @pytest.mark.skip(reason="see https://github.com/localemu/localemu/issues/5938")
    ...
    def test_fifo_empty_message_groups_added_back_to_queue(
        ...
        # https://github.com/localemu/localemu/issues/10107
        queue_name = f"queue-{short_uid()}.fifo"
        queue_url = sqs_create_queue(
            QueueName=queue_name,
            Attributes={
    ...
    @pytest.mark.skip("Needs AWS fixing and is now failing against LocalEmu")
    ...
    def test_external_endpoint(self, monkeypatch, sqs_create_queue, aws_sqs_client):
        ...
        monkeypatch.setattr(config, "SQS_ENDPOINT_STRATEGY", "off")
        monkeypatch.setattr(
            config, "LOCALEMU_HOST", config.HostAndPort(host=external_host, port=external_port)
        )
    ...
    def test_marker_serialization_json_protocol(
        ...
        # TODO: this is an error in LocalEmu. Usually it should be messages[0]['Body']
        else:
            assert (
    ...
    def test_fifo_receive_message_visibility_timeout_shared_in_group(
        ...
        assert ordering in (
            ["g3-m1", "g1-m1", "g1-m2", "g1-m3", "g1-m4", "g2-m1"],  # aws and localemu
            ["g3-m1", "g2-m1", "g1-m1", "g1-m2", "g1-m3", "g1-m4"],  # localemu
        )
    ...
    def test_fifo_message_group_visibility_after_delete(self, sqs_create_queue, aws_sqs_client):
        ...
        assert ordering in (
            ...
            ["g2-m1", "g2-m2", "g3-m1", "g1-m3", "g1-m4"],  # localemu
        )
    ...
    @markers.aws.validated
    @pytest.mark.skip(reason="not implemented in localemu at the moment")
    ...
    def test_fifo_content_based_message_deduplication_arrives_once(
        ...
        # created for https://github.com/localemu/localemu/issues/6327
        queue_url = sqs_create_queue(
            QueueName=f"test-queue-{short_uid()}.fifo",
            Attributes={"FifoQueue": "true", "ContentBasedDeduplication": "true"},
        )
    ...
    @markers.aws.validated
    @pytest.mark.skip(
        reason="localemu allows queue names with slashes, but this should be deprecated"
    )
    ...
    @pytest.mark.skip(reason="validation currently not implemented in localemu")
    @markers.aws.validated
...
class TestSqsQueryApi:
    ...
    def test_queue_url_format_path_strategy(
        ...
        queue_url = sqs_create_queue(QueueName=queue_name)
        assert (
            f"localhost:4566/queue/{region_name}/{account_id}/{queue_name}"
            in queue_url
        )
...
class TestSQSMultiAccounts:
    ...
    def test_cross_account_access(
        ...
        # but this is not enforced in LocalEmu
        # - AddPermission
        # - CreateQueue
        # - DeleteQueue
        # - ListQueues
