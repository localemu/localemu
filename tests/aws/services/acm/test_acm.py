...
class TestACM:
    ...
    def test_certificate_for_subdomain_wildcard(
        ...
        domain_name = f"test-domain-{short_uid()}.localhost"
        ...
        if is_aws_cloud():
            ...
            # Note: When running parity tests against AWS, we need to add the CNAME record to the
            #  DNS provider hosting the domain, to enable validation of the certificate.
            prompt = (
                f"Please add the following CNAME entry to your DNS provider, then hit [ENTER] once "
