...
from localemu.config import external_service_url
...
from localemu.testing.config import TEST_AWS_ACCESS_KEY_ID, TEST_AWS_SECRET_ACCESS_KEY
...
from localemu.utils.aws.arns import get_partition, parse_arn, sqs_queue_arn
from localemu.utils.net import wait_for_port_closed, wait_for_port_open
from localemu.utils.strings import short_uid, to_bytes, to_str
from localemu.utils.sync import poll_condition, retry
...
class TestSNSSubscriptionCrud:
    ...
    def test_subscribe_with_invalid_protocol(self, sns_create_topic, sns_subscription, snapshot):
        ...
        with pytest.raises(ClientError) as e:
            sns_subscription(
                TopicArn=topic_arn, Protocol="test-protocol", Endpoint="localemu@yopmail.com"
            )
    ...
    def test_subscribe_with_invalid_topic(self, sns_create_topic, sns_subscription, snapshot):
        with pytest.raises(ClientError) as e:
            sns_subscription(
                TopicArn="randomstring", Protocol="email", Endpoint="localemu@yopmail.com"
            )
        ...
        with pytest.raises(ClientError) as e:
            sns_subscription(
                TopicArn=bad_topic_arn, Protocol="email", Endpoint="localemu@yopmail.com"
            )
...
class TestSNSSubscriptionCrudV2:
    ...
    # FIXME
    @pytest.mark.skip(
        reason="Https for aws lambda URL required but not implemented in localemu lambda"
    )
    @markers.aws.validated
...
class TestSNSSubscriptionSQS:
    ...
    def test_attribute_raw_subscribe(
        ...
        # https://github.com/localemu/localemu/issues/2432
        aws_client.sns.publish(
            TopicArn=topic_arn,
            Message=message,
...
class TestSNSSubscriptionSQSFifo:
    ...
    def test_fifo_topic_to_regular_sqs(
        ...
        # republish the message, to check deduplication
        # TODO: not implemented in LocalEmu yet, only deduplication with FIFO SQS queues
        aws_client.sns.publish(TopicArn=topic_arn, Message=message, **kwargs)
        response = aws_client.sqs.receive_message(
            QueueUrl=queue_url,
            WaitTimeSeconds=3,
...
class TestSNSSubscriptionSES:
    ...
    def test_email_sender(
        ...
        assert messages[0]["Source"] == "admin@localemu.com"
        requests.delete("http://localhost:4566/_aws/ses")

        sender_address = "no-reply@sns.localemu.cloud"
...
class TestSNSPlatformEndpoint:
    ...
    @markers.aws.only_localemu  # needs real credentials for GCM/FCM
...
class TestSNSMultiAccounts:
    ...
    def test_cross_account_publish_to_sqs(
        ...
        Note: we are not setting Queue policies here as it's only in localemu and IAM is not enforced, for the sake
        of simplicity
        """
...
class TestSNSRetrospectionEndpoints:
    ...
    def test_subscription_tokens_can_retrospect(
        ...
        # we won't confirm the subscription, to simulate an external provider that wouldn't be able to access LocalEmu
