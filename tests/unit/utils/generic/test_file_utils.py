...
from localemu.testing.pytest.util import run_as_os_user
from localemu.utils.common import new_tmp_file, save_file
from localemu.utils.files import idempotent_chmod, new_tmp_dir, parse_config_file, rm_rf
...
@pytest.mark.skipif(
    ...
    reason="requires `localemu` user switch, running only in Docker",
)
def test_idempotent_chmod(file_type):
    ...
    # run chmod tests in subprocess (`localemu` user should be available in our Docker container)
    run_as_os_user(_test_chmod, "localemu")
