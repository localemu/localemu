...
from localemu.testing.testselection.matching import (
    MATCHING_RULES,
    SENTINEL_ALL_TESTS,
    Matchers,
...
from localemu.testing.testselection.testselection import get_affected_tests_from_changes
...
def test_generic_service_matching_rule():
    assert generic_service_test_matching_rule("localemu/aws/api/cloudformation/__init__.py") == {
        "tests/aws/services/cloudformation/",
    }
    assert generic_service_test_matching_rule(
        "localemu/services/cloudformation/test_somefile.py"
    ) == {
        "tests/aws/services/cloudformation/",
    }
    assert generic_service_test_matching_rule(
...
def test_generic_service_matching_rule_with_dependencies():
    ...
    assert generic_service_test_matching_rule(
        "localemu/aws/api/s3/__init__.py", api_dependencies
    ) == {
        "tests/aws/services/cloudformation/",
...
def test_generic_service_matching_rule_defaults_to_api_deps():
    ...
    result = generic_service_test_matching_rule("localemu/services/opensearch/test_somefile.py")
...
def test_directory_rules_with_paths():
    feature_path = "localemu/my_feature"
...
def test_directory_rules_no_match():
    ...
    matcher = Matchers.glob(f"{feature_path}/**").directory(paths=[test_path])
    selected_tests = get_affected_tests_from_changes(
        ["localemu/not_my_feature/__init__.py"], [matcher]
    )
