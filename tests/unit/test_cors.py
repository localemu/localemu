...
def test_allowed_cors_origins_different_ports_and_protocols(monkeypatch):
    ...
    assert "https://localhost:443" not in origins
    ...
    assert "http://localhost:4566" in origins
    assert "https://localhost:443" in origins


def test_dynamic_allowed_cors_origins(monkeypatch):
    assert _origin_allowed("http://test.s3-website.localhost")
    assert _origin_allowed("https://test.s3-website.localhost")
    assert _origin_allowed("http://test.cloudfront.localhost")

    assert not _origin_allowed("https://test.appsync.localhost")
    assert not _origin_allowed("https://testcloudfront.localhost")
...
def test_dynamic_allowed_cors_origins_different_ports(monkeypatch):
    ...
    assert _origin_allowed("http://test.s3-website.localhost:4566")
    assert _origin_allowed("http://test.s3-website.localhost")
    assert _origin_allowed("https://test.s3-website.localhost:4566")
    assert _origin_allowed("https://test.s3-website.localhost")
    assert _origin_allowed("http://test.cloudfront.localhost")

    assert not _origin_allowed("https://test.cloudfront.localhost:443")
    assert not _origin_allowed("http://test.cloudfront.localhost:123")

    # test allowed origins for extended config (:4566,:443)
    monkeypatch.setattr(config, "GATEWAY_LISTEN", default_gateway_listen_ext)
    monkeypatch.setattr(cors, "_ALLOWED_INTERNAL_PORTS", cors._get_allowed_cors_ports())

    assert _origin_allowed("https://test.cloudfront.localhost:443")
