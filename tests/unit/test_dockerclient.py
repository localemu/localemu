...
from localemu.utils.bootstrap import extract_port_flags
...
class TestDockerClient:
    ...
    @patch("localemu.utils.container_utils.docker_cmd_client.run")
    def test_list_containers(self, run_mock):
        mock_container = {
            "ID": "00000000a1",
            "Image": "localemu/localemu",
            "Names": "localemu-main",
            "Labels": "authors=LocalEmu Contributors",
            "State": "running",
        }
        return_container = {
    ...
    def test_container_status(self, run_mock):
        test_output = "Up 2 minutes - localemu-main"
        run_mock.return_value = test_output
        docker_client = CmdDockerClient()
        status = docker_client.get_container_status("localemu-main")
        assert status == DockerContainerStatus.UP
        run_mock.return_value = "Exited (0) 1 minute ago - localemu-main"
        status = docker_client.get_container_status("localemu-main")
...
class TestGetRegistryFromImageName:
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            ("localemu/localemu", "docker.io"),
            ("localemu/localemu:latest", "docker.io"),
            # Images with explicit registry
            ("private-registry.com/localemu/localemu", "private-registry.com"),
            ("private-registry.com/localemu", "private-registry.com"),
            ...
            # LocalEmu ECR
            (
                "000000000000.dkr.ecr.us-east-1.localhost:4566/repo",
