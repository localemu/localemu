...
from localemu.aws.protocol.service_router import (
    ProtocolError,
...
from localemu.utils.run import to_str
...
def _botocore_request_to_localemu_request(request_object: AWSRequest) -> Request:
...
def test_service_router_works_for_every_service(
    ...
    # The endpoint URL is mandatory here, just set a dummy (doesn't _need_ to be localemu specific)
    request_dict = client._convert_to_request_dict(
        request_args, operation, "http://localhost", request_context
    )
    request_object = create_request_object(request_dict)
    client._request_signer.sign(operation.name, request_object)
    request: Request = _botocore_request_to_localemu_request(request_object)
...
def test_endpoint_prefix_based_routing():
    # TODO could be generalized using endpoint resolvers and replacing "amazonaws.com" with "localhost"
    detected_service_model = determine_aws_service_model(
        Request(method="GET", path="/", headers={"Host": "kms.localhost"})
    )
    ...
    detected_service_model = determine_aws_service_model(
        Request(
            method="POST",
            path="/app-instances",
            headers={"Host": "identity-chime.localhost"},
        )
    )
...
def test_endpoint_prefix_based_routing_s3_virtual_host():
    detected_service_model = determine_aws_service_model(
        Request(method="GET", path="/", headers={"Host": "pictures.s3.localhost"})
    )
...
def test_endpoint_prefix_based_routing_for_sqs():
    ...
    detected_service_model = determine_aws_service_model(
        Request(method="GET", path="/", headers={"Host": "sqs.localhost"})
    )
