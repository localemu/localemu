...
from localemu.aws.gateway import Gateway
from localemu.aws.handlers import add_internal_request_params, add_region_from_header
...
from localemu.http.duplex_socket import enable_duplex_socket
from localemu.http.hypercorn import GatewayServer
from localemu.testing.config import TEST_AWS_ACCESS_KEY_ID
...
class TestClientFactory:
    ...
    def test_resolve_dns_from_upstream_uses_system_resolver(self):
        # Regression: the function used to reference an undefined
        # ``upstream_dns`` symbol, so the first call would raise
        # NameError. The fix reads the host's configured nameservers via
        # dnspython's default resolver. This test pins both the lookup
        # path (no NameError) and that the answer addresses are
        # returned.
        from localemu.aws import connect

        fake_answer = MagicMock()
        fake_answer.match.return_value = True
        fake_addr = MagicMock()
        fake_addr.address = "203.0.113.10"
        fake_answer.items = {fake_addr: None}

        fake_response = MagicMock()
        fake_response.answer = [fake_answer]

        fake_resolver = MagicMock()
        fake_resolver.nameservers = ["198.51.100.53"]

        with patch.object(connect.dns.resolver, "get_default_resolver",
                          return_value=fake_resolver), \
             patch.object(connect.dns.query, "udp",
                          return_value=fake_response) as udp_mock:
            assert connect.resolve_dns_from_upstream("example.com") == "203.0.113.10"
            args, kwargs = udp_mock.call_args
            assert args[1] == "198.51.100.53"

    def test_resolve_dns_from_upstream_raises_when_no_resolver_configured(self):
        from localemu.aws import connect

        empty_resolver = MagicMock()
        empty_resolver.nameservers = []

        with patch.object(connect.dns.resolver, "get_default_resolver",
                          return_value=empty_resolver):
            with pytest.raises(RuntimeError, match="No upstream DNS resolver"):
                connect.resolve_dns_from_upstream("example.com")
