...
from localemu.aws.api.sqs import SendMessageRequest
...
def _get_sqs_request_headers():
    return {
        ...
        "Content-Length": "169",
        "x-localemu-request-url": "http://localhost:4566/",
...
@pytest.mark.parametrize(
    "api_class, expected_message_fragment",
    [
        (
            TestSqsApiNotImplemented(),
            "has not yet been implemented in LocalEmu",
        ),
        (
...
def test_skeleton_e2e_sqs_send_message_not_implemented(api_class, expected_message_fragment):
    sqs_service = load_service("sqs-query")
    ...
    assert parsed_response["Error"]["Code"] == "InternalFailure"
    assert expected_message_fragment in parsed_response["Error"]["Message"]
...
def test_dispatch_missing_method_returns_internal_failure():
    ...
    assert "Error" in parsed_response
    assert parsed_response["Error"]["Code"] == "InternalFailure"
    assert "has not yet been implemented in LocalEmu" in parsed_response["Error"]["Message"]
