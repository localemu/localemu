...
from localemu.aws.handlers.service import ServiceExceptionSerializer, ServiceResponseParser
from localemu.aws.protocol.serializer import create_serializer
...
class TestServiceResponseHandler:
    ...
    def test_service_exception(self, service_response_handler_chain):
        from localemu.aws.api.opensearch import ResourceAlreadyExistsException
    ...
    def test_service_exception_with_code_from_spec(self, service_response_handler_chain):
        from localemu.aws.api.sqs import QueueDoesNotExist
    ...
    def test_nothing_set_does_nothing(self, service_response_handler_chain):
        context = RequestContext(request=Request("GET", "/_localemu/health"))
...
class TestServiceExceptionSerializer:
    def test_not_implemented_error_uses_generic_message_when_message_is_empty(self):
        context = create_aws_request_context(
            "opensearch", "DescribeDomain", "rest-json", {"DomainName": "foobar"}
        )
        ...
        assert "has not yet been implemented in LocalEmu" in context.service_exception.message
        assert "opensearch" in context.service_exception.message
