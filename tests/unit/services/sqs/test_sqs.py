...
import localemu.services.sqs.exceptions
import localemu.services.sqs.models
from localemu.services.sqs import provider
from localemu.services.sqs.constants import DEFAULT_MAXIMUM_MESSAGE_SIZE
...
from localemu.utils.common import convert_to_printable_chars
...
def test_parse_max_receive_count_string_in_redrive_policy():
    ...
    # fmt: on
    queue = localemu.services.sqs.models.SqsQueue("TestQueue", "us-east-1", "123456789", policy)
...
def test_except_check_message_max_size():
    ...
    with pytest.raises(localemu.services.sqs.exceptions.InvalidParameterValueException):
...
def test_except_check_message_min_size():
    message_body = ""
    with pytest.raises(localemu.services.sqs.exceptions.MissingRequiredParameterException):
...
def test_parse_queue_url_valid():
    ...
    assert parse_queue_url(
        "http://queue.localhost:4566/000000000001/my-queue"
    ) == (
        "000000000001",
        "us-east-1",
        "my-queue",
    ...
    assert parse_queue_url(
        "http://eu-central-2.queue.localhost:4566/000000000001/my-queue"
    ) == (
        "000000000001",
        "eu-central-2",
        "my-queue",
    ...
    assert parse_queue_url(
        "http://eu-central-2.foobar.localhost:4566/000000000001/my-queue"
    ) == (
        "000000000001",
        None,
        "my-queue",
...
def test_parse_queue_url_invalid():
    ...
    with pytest.raises(ValueError):
        parse_queue_url(
            "http://eu-central-2.queue.localhost:4566/000000000001/my-queue/foobar"
        )

    with pytest.raises(ValueError):
        parse_queue_url(
            "http://queue.localhost:4566/us-east-1/000000000001/my-queue"
        )

    with pytest.raises(ValueError):
        assert parse_queue_url("queue.localhost:4566/000000000001/my-queue")

    with pytest.raises(ValueError):
        assert parse_queue_url(
            "http://foo.bar.queue.localhost:4566/000000000001/my-queue"
        )


def test_is_sqs_queue_url():
    # General cases
    assert is_sqs_queue_url("http://localemu.cloud") is False
    assert is_sqs_queue_url("https://localemu.cloud:4566") is False
    assert is_sqs_queue_url("local.localemu.cloud:4566") is False

    # Without proto prefix
    assert (
        is_sqs_queue_url("sqs.us-east-1.localhost:4566/111111111111/foo") is True
    )
    assert (
        is_sqs_queue_url("us-east-1.queue.localhost:4566/111111111111/foo") is True
    )
    ...
    assert (
        is_sqs_queue_url(
            "http://sqs.us-east-1.localhost:4566/111111111111/foo.fifo"
        )
        is True
    )
    assert (
        is_sqs_queue_url("http://us-east-1.queue.localhost:4566/111111111111/foo1")
        is True
    )
    ...
    assert is_sqs_queue_url("http://queue.localhost:4566/111111111111/foo") is True
...
def test_guess_endpoint_strategy_and_host():
    ...
    assert guess_endpoint_strategy_and_host("queue.localhost") == (
        "domain",
        "localhost",
    )
    assert guess_endpoint_strategy_and_host("us-east-1.queue.localhost") == (
        "domain",
        "localhost",
    )
