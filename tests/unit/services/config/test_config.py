...
from localemu.config import HostAndPort, external_service_url, internal_service_url
...
class TestEdgeVariablesDerivedCorrectly:
    """We are deriving GATEWAY_LISTEN and LOCALEMU_HOST from provided environment variables.
    ...
    # input variable (gateway_listen, localemu_host) has four unique
    ...
    @pytest.mark.parametrize(
        [
            ...
            "expected_localemu_host",
        ],
        [
            ###
            (None, None, [f"{ip()}:4566"], "localhost:4566"),
            ("1.1.1.1", None, ["1.1.1.1:4566"], "localhost:4566"),
            (":5555", None, [f"{ip()}:5555"], "localhost:5555"),
            ("1.1.1.1:5555", None, ["1.1.1.1:5555"], "localhost:5555"),
            ###
            ...
            ###
            (None, ":7777", [f"{ip()}:4566"], "localhost:7777"),
            ("1.1.1.1", ":7777", ["1.1.1.1:4566"], "localhost:7777"),
            (":5555", ":7777", [f"{ip()}:5555"], "localhost:7777"),
            ("1.1.1.1:5555", ":7777", ["1.1.1.1:5555"], "localhost:7777"),
            ###
    ...
    def test_edge_configuration(
        ...
        localemu_host: str | None,
        expected_gateway_listen: list[str],
        expected_localemu_host: str,
    ):
        environment = {}
        if gateway_listen is not None:
            environment["GATEWAY_LISTEN"] = gateway_listen
        if localemu_host is not None:
            environment["LOCALEMU_HOST"] = localemu_host
        ...
        assert actual_ls_host == expected_localemu_host
    ...
    def test_legacy_variables_ignored_if_given(self):
        ...
        assert localemu_host == "localhost:4566"
        assert gateway_listen == [
...
class TestServiceUrlHelpers:
    @pytest.mark.parametrize(
        ...
        [
            # Default
            (None, None, None, None, "http://localhost:4566"),
            # Customize each part with defaults
            ("https", None, None, None, "https://localhost:4566"),
            (None, "s3", None, None, "http://s3.localhost:4566"),
            (None, None, "localemu-container", None, "http://localemu-container:4566"),
            (None, None, None, 5555, "http://localhost:5555"),
            # Multiple subdomains
            (
                None,
                ...
                None,
                None,
                "http://abc123.execute-api.lambda.localhost:4566",
            ),
            ...
            (
                ...
                "localemu-container",
                5555,
                "https://abc.execute-api.localemu-container:5555",
            ),
        ],
    )
