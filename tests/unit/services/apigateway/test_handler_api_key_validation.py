...
from localemu.aws.api.apigateway import ApiKeySourceType, Method
from localemu.http import Request, Response
from localemu.services.apigateway.models import MergedRestApi, RestApiDeployment
from localemu.services.apigateway.next_gen.execute_api.api import RestApiGatewayHandlerChain
from localemu.services.apigateway.next_gen.execute_api.context import (
    InvocationRequest,
    RestApiInvocationContext,
)
from localemu.services.apigateway.next_gen.execute_api.gateway_response import InvalidAPIKeyError
from localemu.services.apigateway.next_gen.execute_api.handlers import ApiKeyValidationHandler
from localemu.services.apigateway.next_gen.execute_api.variables import (
    ContextVariables,
    ContextVarsIdentity,
)
from localemu.testing.config import TEST_AWS_ACCOUNT_ID, TEST_AWS_REGION_NAME
...
def moto_backend():
    ...
    the tests and favor reusable fixture that could later on be replaced to populate the Localemu
    ...
    def _create_context(
        ...
        context.deployment = RestApiDeployment(
            ...
            rest_api=MergedRestApi(
                # TODO validate that this value is always populated by localemu. AWS defaults to HEADERS on all new apis
