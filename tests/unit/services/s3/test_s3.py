...
from localemu.aws.api.s3 import AccessDenied, AuthorizationQueryParametersError, InvalidArgument
from localemu.config import S3_VIRTUAL_HOSTNAME
from localemu.constants import LOCALHOST
from localemu.http import Request
from localemu.services.s3 import presigned_url
from localemu.services.s3 import utils as s3_utils
from localemu.services.s3.codec import AwsChunkedDecoder
from localemu.services.s3.constants import S3_CHUNK_SIZE
from localemu.services.s3.exceptions import MalformedXML
from localemu.services.s3.headers import (
    decode_header_rfc2047,
    encode_header_rfc2047,
    replace_non_iso_8859_1_characters,
)
from localemu.services.s3.models import S3Multipart, S3Object, S3Part
from localemu.services.s3.storage.ephemeral import EphemeralS3ObjectStore
from localemu.services.s3.validation import validate_canned_acl
...
class TestS3Utils:
    @pytest.mark.parametrize(
        ...
        [
            ...
            ("/", {"host": "bucket.s3.localhost"}, "bucket", None),
            ("/", {"host": "bucket-1.s3-website.localhost"}, "bucket-1", None),
            ("/", {"host": "bucket.localhost"}, None, None),
    ...
    # path style format example: https://s3.{region}.localhost:4566/{bucket-name}/{key-name}
    # hosted style format example: http://{bucket-name}.s3.{region}localhost:4566/
    # region is optional in localemu
    def test_uses_virtual_host_addressing(self):
        addresses = [
            ...
            ({"host": "bucket.s3.localhost"}, "bucket"),
            ({"host": "bucket.s3.eu-west-1.amazonaws.com"}, "bucket"),
            ({"host": "s3.eu-west-1.localhost/bucket"}, None),
            ({"host": "s3.localhost"}, None),
            ({"host": "s3.localhost:4566"}, None),
            ({"host": "bucket.s3.eu-west-1.localhost"}, "bucket"),
            ({"host": "bucket.s3.localhost/key"}, "bucket"),
            ({"host": "bucket.s3.eu-west-1.amazonaws.com"}, "bucket"),
    ...
    def test_virtual_host_matching(self):
        hosts = [
            ("bucket.s3.localhost", "bucket", None),
            ("bucket.s3.eu-west-1.amazonaws.com", "bucket", "eu-west-1"),
            ("test-bucket.s3.eu-west-1.localhost", "test-bucket", "eu-west-1"),
            ("bucket.s3.notrealregion-west-1.localhost", "bucket", None),
    ...
    @pytest.mark.parametrize(
        ...
        [
            pytest.param(
                "http://s3.localhost:4566/test-output-bucket-2/test-transcribe-job-e1895bdf.json?AWSAccessKeyId=000000000000&Signature=2Yc%2BvwhXx8UzmH8imzySfLOW6OI%3D&Expires=1688561914",
                "test-output-bucket-2",
                "test-transcribe-job-e1895bdf.json",
                id="output key as a single file",
            ),
            pytest.param(
                "http://s3.localhost:4566/test-output-bucket-5/test-files/test-output.json?AWSAccessKeyId=000000000000&Signature=F6bwF1M2N%2BLzEXTZnUtjE23S%2Bb0%3D&Expires=1688561920",
                "test-output-bucket-5",
                "test-files/test-output.json",
                id="output key with subdirectories",
            ),
            pytest.param(
                "http://s3.localhost:4566/test-output-bucket-2?AWSAccessKeyId=000000000000&Signature=2Yc%2BvwhXx8UzmH8imzySfLOW6OI%3D&Expires=1688561914",
                "test-output-bucket-2",
                "",
