...
from localemu.http.hypercorn import GatewayServer, ProxyServer
from localemu.utils.net import IP_REGEX, get_free_tcp_port
...
def test_gateway_server():
    ...
    with server_context(server):
        get_response = requests.get(
            f"https://localhost:{gateway_listen.port}",
...
def test_proxy_server(httpserver):
    ...
    with server_context(proxy_server):
        ...
        response = requests.get(
            f"https://localhost:{gateway_listen.port}/relative-path",
            data="data",
            verify=False,
        )
...
def test_proxy_server_properly_handles_headers(httpserver):
    ...
    def header_echo_handler(request: WerkzeugRequest) -> Response:
        ...
        assert headers["Host"] == f"localhost:{gateway_listen.port}"
...
def test_proxy_server_with_streamed_response(httpserver):
    ...
    with server_context(proxy_server):
        with requests.get(
            f"https://localhost:{gateway_listen.port}/", stream=True, verify=False
        ) as r:
            r.raise_for_status()
