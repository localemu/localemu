...
from localemu.http.router import (
    E,
    GreedyPathConverter,
    RequestArguments,
    RuleAdapter,
...
from localemu.utils.common import get_free_tcp_port
...
class TestRouter:
    ...
    def test_regex_host_dispatcher(self):
        router = Router()
        rgx = r"\.cloudfront\.(net|localhost)"
        router.add(path="/", endpoint=noop, host=f"<dist_id><regex('{rgx}'):host>:<port>")
        assert router.dispatch(
            Request(
                method="GET",
                headers={"Host": "ad91f538.cloudfront.localhost:5446"},
            )
        )
        with pytest.raises(NotFound):
            router.dispatch(
    ...
    def test_port_host_dispatcher(self):
        collector = RequestCollector()
        router = Router(dispatcher=collector)
        router.add(path="/", endpoint=noop, host="localhost<port:port>")
        ...
        with pytest.raises(NotFound):
            router.dispatch(
                Request(
                    method="GET",
                    headers={"Host": "localemu.cloud:5446"},
                )
            )
    ...
    def test_path_converter_and_regex_converter_in_host(self):
        ...
        router.add(
            path="/<path:path>",
            host="foobar.us-east-1.opensearch.localhost<regex('(?::.*)?'):port>",
        ...
        assert router.dispatch(
            Request(
                ...
                headers={"Host": "foobar.us-east-1.opensearch.localhost:4566"},
            )
    ...
    def test_path_converter_and_port_converter_in_host(self):
        ...
        router.add(
            path="/<path:path>",
            host="foobar.us-east-1.opensearch.localhost<port:port>",
            endpoint=echo_params_json,
        )
        assert router.dispatch(
            Request(
        ...
        assert router.dispatch(
            Request(
                method="GET",
                path="/_cluster/health",
                headers={"Host": "foobar.us-east-1.opensearch.localhost"},
            )
    ...
    def test_with_host_and_submount(self):
        ...
        request = Request("GET", "/my_api", server=("foo.localhost", 4566))
        assert router.dispatch(request).json == {
            "path": "/my_api",
            "host": "foo.localhost:4566",
        }

        request = Request("GET", "/my_api", server=("localhost", 4566))
        with pytest.raises(NotFound):
