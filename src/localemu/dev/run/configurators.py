"""
Several ContainerConfigurator implementations to set up a development version of a localemu container.
"""
...
from localemu.utils.files import get_user_cache_dir
from localemu.utils.run import run
from localemu.utils.strings import md5
...
class PortConfigurator:
    """
    Configures the port mappings. Can be randomized to run multiple localemu instances.
    """
...
class SourceVolumeMountConfigurator:
    ...
    def __call__(self, cfg: ContainerConfiguration):
        # localemu source code if available
        source = self.host_paths.aws_community_package_dir
        if source.exists():
            cfg.volumes.add(
                BindMount(str(source), self.container_paths.localemu_source_dir, read_only=True)
            )
        ...
        # docker entrypoint
        source = self.host_paths.localemu_project_dir / "bin" / "docker-entrypoint.sh"
        if source.exists():
            cfg.volumes.add(
...
class EntryPointMountConfigurator:
    """
     Mounts ``entry_points.txt`` files of localemu and dependencies into the venv in the container.
    ...
    ``/opt/code/localemu/.venv/.../site-packages/<package-name>.dist-info/entry_points.txt``.
    """

    entry_point_glob = (
        "/opt/code/localemu/.venv/lib/python3.*/site-packages/*.dist-info/entry_points.txt"
    )
    localemu_community_entry_points = (
        "/opt/code/localemu/localemu_core.egg-info/entry_points.txt"
    )
    ...
    def __call__(self, cfg: ContainerConfiguration):
        ...
        for container_path in paths:
            ...
            if dep == "localemu_core":
                host_path = self.host_paths.localemu_project_dir / "plux.ini"
...
class DependencyMountConfigurator:
    ...
    dependency_glob = "/opt/code/localemu/.venv/lib/python3.*/site-packages/*"
...
def _list_files_in_container_image(container_client: ContainerClient, image_name: str) -> list[str]:
    ...
    the image ID into a gziped file into ``~/.cache/localemu-dev-cli`` to (significantly) speed up
    subsequent calls.
    ...
    cache_dir = get_user_cache_dir() / "localemu-dev-cli"
