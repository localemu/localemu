...
from localemu.extensions.api import Extension, http
...
class WebAppExtension(Extension):
    ...
    This will create an extension that localemu serves via:

    * Submount: https://localhost:4566/_extension/my-extension
    * Subdomain: https://my-extension.localhost:4566/
    ...
    * Submounts: for some environments, like in ephemeral instances where subdomains are harder to control,
    ...
    def __init__(
        ...
        :param mount: the "mount point" which will be used as default value for the submount and
            subdirectory, i.e., ``<mount>.localhost`` and
            ``localhost/_extension/<mount>``. Defaults to the extension name.  Note that,
    ...
    def collect_routes(self, routes: list[t.Any]):
        ...
        * ``my-extension.localhost:4566/hello``
        * ``my-extension.localhost:4566/say-hello``
        * ``my-extension.localhost:4566/empty-dict``
        * ``my-extension.localhost:4566/static``  <- automatically added static file endpoint

        With submount:

        * ``localhost:4566/_extension/my-extension/hello``
        * ``localhost:4566/_extension/my-extension/say-hello``
        * ``localhost:4566/_extension/my-extension/empty-dict``
        * ``localhost:4566/_extension/my-extension/static`` <- auto-added static file serving
    ...
    def update_gateway_routes(self, router: http.Router[http.RouteHandler]):
        from localemu.aws.handlers import preprocess_request
    ...
    def _configure_cors_for_subdomain(self):
        ...
        """
        from localemu.aws.handlers.cors import ALLOWED_CORS_ORIGINS
    ...
    def get_subdomain_url(self, protocol: str = "https") -> str:
        ...
        ``https://my-extension.localhost:4566/``.
    ...
    def get_submount_url(self, protocol: str = "https") -> str:
        ...
        ``https://localhost:4566/_extension/my-extension``.
