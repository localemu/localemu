...
from localemu.aws.spec import ProtocolName
...
class S3RequestParser(RestXMLRequestParser):
    class VirtualHostRewriter:
        ...
        def _is_vhost_address_get_bucket(request: Request) -> str | None:
            from localemu.services.s3.utils import uses_host_addressing
    ...
    def parse(self, request: Request) -> tuple[OperationModel, Any]:
        ...
        with self.VirtualHostRewriter(request):
            try:
                ...
                # HEAD / with no bucket is a Terraform/SDK health check.
                # Map to HeadBucket with a synthetic bucket placeholder.
                # The S3 provider will handle it gracefully.
                if request.method == "HEAD" and request.path.rstrip("/") in ("", "/"):
                    op = self.service.operation_model("HeadBucket")
                    return op, {"Bucket": "__health_check__"}
                raise
