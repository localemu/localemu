...
from localemu.services.sqs.utils import is_sqs_queue_url
...
def determine_aws_service_model(
    ...
    if host:
        ...
        if custom_host_match:
            # custom_host_addressing_rules returns a single
            # ServiceModelIdentifier (or None) — NOT a list. The previous
            # ``custom_host_match[0]`` subscript indexed into the
            # NamedTuple by integer and unpacked the ``name`` field as a
            # plain str, which then crashed on the ``.name`` / ``.protocol``
