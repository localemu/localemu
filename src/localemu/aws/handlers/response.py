import logging

from localemu import config, constants
from localemu.aws.api import RequestContext
from localemu.aws.chain import Handler, HandlerChain
from localemu.http import Response
from localemu.runtime import hooks

LOG = logging.getLogger(__name__)


class ResponseMetadataEnricher(Handler):
    """
    A handler that adds extra metadata to a Response.
    """

    def __call__(self, chain: HandlerChain, context: RequestContext, response: Response):
        # Currently, we just add 'x-localemu' in response headers.
        response.headers[constants.HEADER_LOCALEMU_IDENTIFIER] = "true"


@hooks.on_infra_start(should_load=config.LOCALEMU_RESPONSE_HEADER_ENABLED)
def init_response_mutation_handler():
    from localemu.aws.handlers import run_custom_response_handlers

    # inject enricher into handler chain
    enricher = ResponseMetadataEnricher()
    run_custom_response_handlers.append(enricher)
