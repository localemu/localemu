...
from localemu.constants import INTERNAL_RESOURCE_PATH
...
class OpenAPIRequestValidator(OpenAPIValidator):
    """
    Validates the requests to the LocalEmu public endpoints (the ones with a _localemu or _aws prefix) against
...
class OpenAPIResponseValidator(OpenAPIValidator):
    def __call__(self, chain: HandlerChain, context: RequestContext, response: Response):
        # The use of this flag is intended for test only. Eventual errors are due to LocalEmu implementation and not
