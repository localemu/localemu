...
LOCALEMU_BUILTIN_DATA_PATH = os.path.join(os.path.dirname(os.path.abspath(__file__)), "data")


class LocalEmuBuiltInDataLoaderMixin(Loader):
    def __init__(self, *args, **kwargs):
        # add the builtin data path to the extra_search_paths to ensure they are discovered by the loader
        super().__init__(*args, extra_search_paths=[LOCALEMU_BUILTIN_DATA_PATH], **kwargs)
...
class CustomLoader(PatchingLoader, LocalEmuBuiltInDataLoaderMixin):
    # Class mixing the different loader features (patching, localemu specific data)
    pass
