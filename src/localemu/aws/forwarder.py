"""
This module contains utilities to call a backend (e.g., an external service process or
an in-process Python backend) from a service provider.
"""
...
from localemu.aws.api.core import (
    RequestContext,
    ServiceRequest,
    ServiceRequestHandler,
    ServiceResponse,
)
from localemu.aws.client import create_http_request, parse_response, raise_service_exception
from localemu.aws.connect import connect_to
from localemu.aws.skeleton import DispatchTable, create_dispatch_table
from localemu.aws.spec import ProtocolName, load_service
from localemu.constants import AWS_REGION_US_EAST_1
from localemu.http import Response
from localemu.http.proxy import Proxy
...
def create_aws_request_context(
    ...
    :param endpoint_url: the endpoint to call (defaults to localemu)
    ...
    # The endpoint URL is mandatory here, set a dummy if not given (doesn't _need_ to be localemu specific)
    if not endpoint_url:
