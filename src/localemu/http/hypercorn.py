...
from localemu.aws.handlers.proxy import ProxyHandler
from localemu.aws.serving.asgi import AsgiGateway
from localemu.config import HostAndPort
from localemu.logging.setup import setup_hypercorn_logger
...
class GatewayServer(HypercornServer):
    ...
    def __init__(
        ...
        :param use_ssl: True if the LocalEmu cert should be loaded and HTTP/HTTPS multiplexing should be enabled.
...
class ProxyServer(GatewayServer):
    """
    Proxy server implementation which uses the localemu.http.proxy module.
