...
from localemu.constants import (
    ...
    ENV_INTERNAL_TEST_STORE_METRICS_IN_LOCALEMU,
    FALSE_STRINGS,
    LOCALHOST,
    LOCALHOST_IP,
    LOCALEMU_ROOT_FOLDER,
    LOG_LEVELS,
    TRACE_LOG_LEVELS,
    TRUE_STRINGS,
)
...
class Directories:
    """
    Holds different directories available to localemu. Some directories are shared between the host and the
    localemu container, some live only on the host and others in the container.
    ...
    Attributes:
        ...
        cache:       shared; ephemeral data that has to persist across localemu runs and reboots
        tmp:         container only; ephemeral data that has to persist across localemu runs but not reboots
        ...
        data:        shared; holds localemu state, pods, ...
        ...
        logs:        shared; log files produced by localemu
    """
    ...
    def defaults() -> "Directories":
        """Returns LocalEmu directory paths based on the localemu filesystem hierarchy."""
        return Directories(
            static_libs="/usr/lib/localemu",
            var_libs=f"{DEFAULT_VOLUME_DIR}/lib",
            cache=f"{DEFAULT_VOLUME_DIR}/cache",
            tmp=os.path.join(tempfile.gettempdir(), "localemu"),
            ...
            config="/etc/localemu/conf.d",  # for future use
            init="/etc/localemu/init",
        )

    @staticmethod
    def for_container() -> "Directories":
        """
        Returns LocalEmu directory paths as they are defined within the container. Everything shared and writable
        lives in /var/lib/localemu or {tempfile.gettempdir()}/localemu.
    ...
    def for_host() -> "Directories":
        """Return directories used for running localemu in host mode. Note that these are *not* the directories
        that are mounted into the container when the user starts localemu."""
        root = os.environ.get("FILESYSTEM_ROOT") or os.path.join(
            LOCALEMU_ROOT_FOLDER, ".filesystem"
        )
    ...
    def for_cli() -> "Directories":
        """Returns directories used for when running localemu CLI commands from the host system. Unlike
        ``for_container``, these needs to be cross-platform. Ideally, this should not be needed at all,
        because the localemu runtime and CLI do not share any control paths. There are a handful of
        ...
        from localemu.utils import files

        tmp_dir = os.path.join(tempfile.gettempdir(), "localemu-cli")
        cache_dir = (files.get_user_cache_dir()).absolute() / "localemu-cli"
        ...
        return Directories(
            ...
            mounted_tmp=tmp_dir,
            functions=None,
            data=os.path.join(tmp_dir, "state"),  # used by localemu config TODO: remove
    ...
    def mkdirs(self):
        ...
        named_dirs = {
            ...
            "var_libs": self.var_libs,
            "cache": self.cache,
            "tmp": self.tmp,
            "mounted_tmp": self.mounted_tmp,
        ...
        for name, folder in named_dirs.items():
            if folder:
                os.makedirs(folder, exist_ok=True)
            else:
                _log.debug(
                    "Directories.mkdirs: skipping unset directory %r", name
                )
...
def load_environment(profiles: str = None, env=os.environ) -> list[str]:
    """Loads the environment variables from ~/.localemu/{profile}.env, for each profile listed in the profiles.
...
def in_docker():
    ...
    for path in [
        "/usr/lib/localemu/.community-version",
        "/usr/lib/localemu/.pro-version",
        "/tmp/localemu/.marker",
    ]:
        if os.path.isfile(path):
            return True

    # details: https://github.com/localemu/localemu/pull/4352
...
CONFIG_DIR = os.environ.get("CONFIG_DIR", os.path.expanduser("~/.localemu"))
...
# whether localemu should persist service state across localemu runs
PERSISTENCE = is_env_true("PERSISTENCE")

# When enabled, VPC EC2 and RDS containers receive a private IP from
...
TMP_FOLDER = os.path.join(tempfile.gettempdir(), "localemu")

# this is exclusively for the CLI to configure the container mount into /var/lib/localemu
VOLUME_DIR = os.environ.get("LOCALEMU_VOLUME_DIR", "").strip() or TMP_FOLDER
...
# whether to enable verbose debug logging ("LOG" is used when using the CLI with LOCALEMU_LOG instead of LS_LOG)
...
if DEBUG:
    ...
    logging.getLogger("localemu").setLevel(logging.DEBUG)
...
def populate_edge_configuration(
    ...
    """Populate the LocalEmu edge configuration from environment variables."""
    localemu_host_raw = environment.get("LOCALEMU_HOST")
    ...
    # the actual value of the LOCALEMU_HOST port now depends on what gateway listen actually listens to.
    if localemu_host_raw is None:
        localemu_host = HostAndPort(
            host=constants.LOCALHOST_HOSTNAME, port=gateway_listen[0].port
        )
    else:
        localemu_host = HostAndPort.parse(
            localemu_host_raw,
    ...
    assert localemu_host is not None
...
# whether to simulate AWS API throttling (off by default)
SIMULATE_THROTTLING = is_env_true("SIMULATE_THROTTLING")
# probability (0.0–1.0) that a request is throttled when simulation is enabled
THROTTLE_RATE = float(os.environ.get("THROTTLE_RATE", "0.05"))
...
MAIN_CONTAINER_NAME = os.environ.get("MAIN_CONTAINER_NAME", "").strip() or "localemu-main"

# the latest commit id of the repository when the docker image was created
LOCALEMU_BUILD_GIT_HASH = os.environ.get("LOCALEMU_BUILD_GIT_HASH", "").strip() or None

# the date on which the docker image was created
LOCALEMU_BUILD_DATE = os.environ.get("LOCALEMU_BUILD_DATE", "").strip() or None
...
# Flag to enable the validation of the requests made to the LocalEmu internal endpoints. Active by default.
...
# Enables the automatic removal of stale KV pairs based on TTL.
...
# PUBLIC: Endpoint host under which LocalEmu APIs are accessible from Lambda Docker containers.
...
# PUBLIC: 5 (default), set to 0 for strict AWS-parity
# How long an Invoke will wait for a function in Pending state to transition
# to Active before returning ResourceConflictException. Smooths the
# create-function-then-invoke race that AWS itself exhibits (real AWS takes
# a few seconds to warm up too). 0 disables the wait entirely.
LAMBDA_INVOKE_WAIT_FOR_ACTIVE_SECONDS = float(
    os.environ.get("LAMBDA_INVOKE_WAIT_FOR_ACTIVE_SECONDS") or 5
)

# PUBLIC: base images for Lambda (default) https://docs.aws.amazon.com/lambda/latest/dg/runtimes-images.html
# localemu/services/lambda_/invocation/lambda_models.py:IMAGE_MAPPING
...
# PUBLIC: 0 (disabled by default)
# Artificial cold start delay in seconds applied to the first invocation of a function version
# (or after it has been idle longer than LAMBDA_COLD_START_IDLE_TIMEOUT).
# Set to a positive value (e.g. 3) to simulate AWS Lambda cold start latency.
LAMBDA_COLD_START_DELAY = float(os.environ.get("LAMBDA_COLD_START_DELAY", "0"))

# PUBLIC: 300 (default 5 min, matching AWS)
# Idle time in seconds after which a function version is considered cold again.
LAMBDA_COLD_START_IDLE_TIMEOUT = int(os.environ.get("LAMBDA_COLD_START_IDLE_TIMEOUT", "300"))
...
# DEV: Release version of https://github.com/localemu/lambda-runtime-init overriding the current default
...
# When true, presigned URL validation will NOT fall back to the hardcoded
# "test" access key / secret when the request's access key is not registered
# in LocalEmu. Unregistered keys will be rejected. Enable this for
# Production-parity testing .
S3_STRICT_PRESIGNED_VALIDATION = is_env_true("S3_STRICT_PRESIGNED_VALIDATION")
...
# URL to a custom OpenSearch/Elasticsearch backend cluster. If this is set to a valid URL, then localemu will not
...
# Historical compatibility shim: the DynamoDB Streams v2 provider was removed when LocalEmu
# dropped the Java DynamoDB Local backend. The flag is kept as a permanent False so older test
# expressions like `config.DDB_STREAMS_PROVIDER_V2` continue to evaluate without raising.
DDB_STREAMS_PROVIDER_V2 = False
...
# replaced in CloudFormation templates (e.g. AWS URLs that are usually edited by Localemu to point to itself if found
...
# How localemu will react to encountering unsupported resource types.
...
S3_VIRTUAL_HOSTNAME = f"s3.{LOCALEMU_HOST.host}"
S3_STATIC_WEBSITE_HOSTNAME = f"s3-website.{LOCALEMU_HOST.host}"
...
# This flag enables all responses from LocalEmu to contain a `x-localemu` HTTP header.
LOCALEMU_RESPONSE_HEADER_ENABLED = is_env_not_false("LOCALEMU_RESPONSE_HEADER_ENABLED")

# Simulate network latency for AWS API responses.
# Set to "1" or "true" for per-service realistic latency profiles, or a number for fixed ms delay.
SIMULATE_LATENCY = os.environ.get("SIMULATE_LATENCY", "").strip()

# Serialization backend for the LocalEmu internal state (`dill` is used by default`).
STATE_SERIALIZATION_BACKEND = os.environ.get("STATE_SERIALIZATION_BACKEND", "").strip() or "dill"

# List of environment variable names used for configuration that are passed from the host into the LocalEmu container.
# => Synchronize this list with the above and the configuration docs:
# https://localemu.cloud/docs/configuration
...
CONFIG_ENV_VARS = [
    ...
    "LOCALEMU_HOST",
    "LOCALEMU_RESPONSE_HEADER_ENABLED",
    ...
    "LOCALEMU_HOSTNAME",  # deprecated since 2.0.0
...
def store_test_metrics_in_local_filesystem() -> bool:
    ...
    return is_env_true(ENV_INTERNAL_TEST_STORE_METRICS_IN_LOCALEMU)


def collect_config_items() -> list[tuple[str, Any]]:
    """Returns a list of key-value tuples of LocalEmu configuration values."""
...
def populate_config_env_var_names():
    ...
    CONFIG_ENV_VARS += [
        ...
        if (key.startswith("LOCALEMU_") or key.startswith("PROVIDER_OVERRIDE_"))
        # explicitly exclude LOCALEMU_CLI (it's prefixed with "LOCALEMU_",
        # but is only used in the CLI (should not be forwarded to the container)
        and key != "LOCALEMU_CLI"
    ]

    # create variable aliases prefixed with LOCALEMU_ (except LOCALEMU_HOST)
    CONFIG_ENV_VARS += [
        "LOCALEMU_" + v for v in CONFIG_ENV_VARS if not v.startswith("LOCALEMU_")
    ]
...
def external_service_url(
    ...
    machine than LocalEmu. The configurations LOCALEMU_HOST and USE_SSL can customize these returned URLs.
    The optional parameters can be used to customize the defaults.
    Examples with default configuration:
    * external_service_url() == http://localhost:4566
    * external_service_url(subdomains="s3") == http://s3.localhost:4566
    """
    protocol = protocol or get_protocol()
    subdomains = f"{subdomains}." if subdomains else ""
    host = host or LOCALEMU_HOST.host
    port = port or LOCALEMU_HOST.port
...
def internal_service_url(
    ...
    """Returns a service URL for internal use within LocalEmu (i.e., same host).
    The configuration USE_SSL can customize these returned URLs but LOCALEMU_HOST has no effect.
    The optional parameters can be used to customize the defaults.
    Examples with default configuration:
...
def service_port(service_key: str, external: bool = False) -> int:
    """@deprecated: Use `localemu_host().port` for external and `GATEWAY_LISTEN[0].port` for
    internal use."""
    warnings.warn(
        "Deprecated: use `localemu_host().port` for external and `GATEWAY_LISTEN[0].port` for "
        "internal use.",
...
def get_edge_port_http():
    ...
    and HTTP ports anymore since LocalEmu listens to both ports."""
    warnings.warn(
        """@deprecated: Use `localemu_host().port` for external and `GATEWAY_LISTEN[0].port`
        for internal use. This function is also not needed anymore because we don't separate
        between HTTP and HTTP ports anymore since LocalEmu listens to both.""",
        DeprecationWarning,
        stacklevel=2,
    )
    return GATEWAY_LISTEN[0].port
...
def get_edge_url(localemu_hostname=None, protocol=None):
    ...
    return internal_service_url(host=localemu_hostname, protocol=protocol)
