...
from ..constants import LOCALEMU_VENV_FOLDER
...
class ArchiveDownloadAndExtractInstaller(ExecutableInstaller):
    ...
    def get_installed_dir(self) -> str | None:
        ...
        # f.e. /var/lib/localemu/lib/amazon-mq/5.16.5/apache-activemq-5.16.5/
...
class NodePackageInstaller(ExecutableInstaller):
    ...
    def _install(self, target: InstallTarget) -> None:
        ...
        # npm 9+ no longer chowns the install tree when invoked as root,
        # so root-installed package files end up owned by uid 0 and the
        # service that consumes them may not have read access. We restore
        # the historical chown manually for the root case.
        if is_root():
...
LOCALEMU_VENV = VirtualEnvironment(LOCALEMU_VENV_FOLDER)
...
class PythonPackageInstaller(PackageInstaller):
    ...
    def _prepare_installation(self, target: InstallTarget) -> None:
        ...
        if not venv.exists:
            ...
            venv.create()
            LOG.info("adding localemu venv path %s", venv.venv_dir)
            venv.add_pth("localemu-venv", LOCALEMU_VENV)
