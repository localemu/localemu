...
class PackageInstaller(abc.ABC):
    ...
    def _get_install_marker_path(self, install_dir: str) -> str:
        ...
        :param install_dir: base path for the check (e.g. /var/lib/localemu/lib/<package>/latest/)
        :return: path which should be checked to indicate if the package has been installed successfully
                 (e.g. /var/lib/localemu/lib/<package>/latest/<marker-file>)
        """
...
PLUGIN_NAMESPACE = "localemu.packages"
...
class PackagesPluginManager(PluginManager[PackagesPlugin]):  # type: ignore[misc]
    ...
    def get_packages(
        ...
        # Plugin names have the format "<package-name>" or "<package-name>/<scope>" (legacy); build a dict of
        # specs per package name for the lookup.
        plugin_specs_per_name = defaultdict(list)
        for plugin_spec in self.list_plugin_specs():
            if "/" in plugin_spec.name:
                package_name = plugin_spec.name.rpartition("/")[0]
            else:
                package_name = plugin_spec.name
...
def package(
    ...
    Methods marked with this decorator are discoverable as a PluginSpec within the namespace "localemu.packages".
    The plugin name is the package name; a scope (e.g. "community") can optionally be appended as "<name>/<scope>".
    If ``name`` is not explicitly specified, the parent module name is used.
    """
    ...
    def wrapper(fn: Callable[[], Package[Any] | list[Package[Any]]]) -> PluginSpec:
        ...
        plugin_name = f"{_name}/{scope}" if scope else _name

        @functools.wraps(fn)
        def factory() -> PackagesPlugin:
            return PackagesPlugin(name=_name, scope=scope or "", get_package=fn, should_load=should_load)

        return PluginSpec(PLUGIN_NAMESPACE, plugin_name, factory=factory)
