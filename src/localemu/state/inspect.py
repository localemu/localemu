...
from localemu.state.core import StateVisitor
...
class ServiceBackend(TypedDict, total=False):
    ...
    localemu: AccountRegionBundle | None
...
class ServiceBackendCollectorVisitor(StateVisitor):
    ...
    def collect(self) -> ServiceBackend:
        ...
        if self.store:
            service_backend.update({"localemu": self.store})
...
class ReflectionStateLocator:
    ...
    def accept_state_visitor(self, visitor: StateVisitor):
        ...
        module_name = f"localemu.services.{service_name}.models"
