...
For your convenience, you can simply call ``dumps`` or ``loads`` as you would pickle or dill::
    ...
    from localemu.state import pickle
...
# ---------------------------------------------------------------------------
# Persistence: custom dill reducers for unpicklable types
# ---------------------------------------------------------------------------
# These two reducers fix the only objects in the codebase that dill cannot
# serialize out of the box.  They are registered into the dispatch table
# at import time so they are available whenever ``localemu.state.pickle``
# is used (e.g. by SaveOrchestrator / LoadOrchestrator).

# --- Fix 1: moto SSM ParameterDict -------------------------------------------
# ParameterDict subclasses dict but its __init__ requires (account_id,
# region_name).  dill's default dict reducer loses those constructor args.
# We save them explicitly and replay them on restore.

try:
    from moto.ssm.models import ParameterDict as _ParameterDict

    def _restore_parameter_dict(cls, account_id, region_name, items):
        pd = cls(account_id, region_name)
        pd.update(items)
        return pd

    @register(_ParameterDict)
    def _pickle_parameter_dict(pickler, obj):
        pickler.save_reduce(
            _restore_parameter_dict,
            (type(obj), obj.account_id, obj.region_name, dict(obj)),
            obj=obj,
        )

except ImportError:
    # moto.ssm may not be installed in every environment
    pass


# --- Fix 2: weakref.WeakValueDictionary --------------------------------------
# WeakValueDictionary entries may vanish between serialization and
# deserialization (the referents are gone).  We save an empty snapshot and
# let the owning containers re-populate strong references on access.

import weakref as _weakref


def _restore_wvd(items):
    # Return empty — owning containers re-populate on access
    return _weakref.WeakValueDictionary()


@register(_weakref.WeakValueDictionary)
def _pickle_wvd(pickler, obj):
    # Convert to plain dict for safe serialization; items may be stale
    # after restore, but owning containers hold strong refs and re-register
    pickler.save_reduce(_restore_wvd, (dict(obj),), obj=obj)
