...
from localemu.state import Decoder, Encoder
from localemu.state.pickle import PickleDecoder, PickleEncoder
...
def create_decoder(decoder_type: str) -> Decoder:
    cls = DECODERS.get(decoder_type)
    if cls is None:
        LOG.warning("Unknown decoder type '%s', falling back to PickleDecoder", decoder_type)
        cls = PickleDecoder
    return cls()
