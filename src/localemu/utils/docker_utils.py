...
from localemu.constants import DEFAULT_VOLUME_DIR, DOCKER_IMAGE_NAME
...
from localemu.utils.net import IntOrPort, Port, PortNotAvailableException, PortRange
...
def create_docker_client() -> ContainerClient:
    ...
    if config.LEGACY_DOCKER_CLIENT or not is_docker_sdk_installed() or not config.is_in_docker:
        ...
        try:
            ...
            LOG.debug("Docker SDK client failed, falling back to CLI client")
            from localemu.utils.container_utils.docker_cmd_client import CmdDockerClient
...
def get_default_volume_dir_mount() -> VolumeInfo | None:
    """
    Returns the volume information of LocalEmu's DEFAULT_VOLUME_DIR (/var/lib/localemu), if mounted,
...
def get_host_path_for_path_in_docker(path: str) -> str:
    """
    Returns the calculated host location for a given subpath of DEFAULT_VOLUME_DIR inside the localemu container.
    ...
    if config.is_in_docker and DOCKER_CLIENT.has_docker():
        ...
        if volume:
            ...
            if not path.startswith(f"{DEFAULT_VOLUME_DIR}/") and path != DEFAULT_VOLUME_DIR:
                ...
                # Please check if the target path is indeed prefixed by /var/lib/localemu
...
def _get_ports_check_docker_image() -> str:
    ...
    if not config.is_in_docker:
        ...
        from localemu.utils.bootstrap import get_docker_image_to_start

        # Use whatever image the user is trying to run LocalEmu with, since they either have
        # it already, or need it by definition to start LocalEmu.
