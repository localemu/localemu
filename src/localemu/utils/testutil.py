...
from localemu.aws.connect import connect_externally_to, connect_to
...
from localemu.constants import (
    ...
    LOCALEMU_VENV_FOLDER,
)
from localemu.services.lambda_.lambda_utils import (
...
from localemu.utils.archives import create_zip_file_cli, create_zip_file_python
from localemu.utils.collections import ensure_list
from localemu.utils.files import (
    TMP_FILES,
    chmod_r,
    cp_r,
    is_empty_dir,
...
from localemu.utils.platform import is_debian
    ...
    with tempfile.TemporaryDirectory(prefix=ARCHIVE_DIR_PREFIX) as tmp_dir:
        ...
        for lib in libs:
            ...
            try:
                ...
                pass
            target_dir = tmp_dir
            root_folder = os.path.join(LOCALEMU_VENV_FOLDER, "lib/python*/site-packages")
            if lib == "localemu":
                paths = ["localemu/*.py", "localemu/utils"]
                root_folder = LOCALEMU_ROOT_FOLDER
...
def upload_file_to_bucket(s3_client, bucket_name, file_path, file_name=None):
    ...
    domain = "amazonaws.com" if is_aws_cloud() else localemu_host().host_and_port()
