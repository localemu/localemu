...
from localemu.config import LS_LOG
from localemu.constants import TRACE_LOG_LEVELS
from localemu.utils.collections import ensure_list
...
class SdkDockerClient(ContainerClient):
    ...
    @staticmethod
    def _create_client():
        from localemu.config import DOCKER_SDK_DEFAULT_RETRIES, DOCKER_SDK_DEFAULT_TIMEOUT_SECONDS
    ...
    def create_network(
        ...
        # The SDK path raises ``APIError`` cleanly without printing to stdout,
        # so ``print_error`` is accepted for parity with the cmd client but
        # has no observable effect here.
        del print_error
        try:
            kwargs: dict = {"name": network_name, "driver": "bridge"}
            if subnet:
                from docker.types import IPAMConfig, IPAMPool

                gateway = subnet.rsplit(".", 1)[0] + ".1"  # e.g. 10.0.0.0/16 → 10.0.0.1
                kwargs["ipam"] = IPAMConfig(pool_configs=[IPAMPool(subnet=subnet, gateway=gateway)])
            if internal:
                kwargs["internal"] = True
            if labels:
                kwargs["labels"] = labels
            return self.client().networks.create(**kwargs).id
        except APIError as e:
            raise ContainerException() from e
    ...
    def remove_volume(self, volume_name: str) -> None:
        try:
            ...
            pass  # Volume may not exist or be in use
    ...
    def connect_container_to_network(
        ...
        LOG.debug(
            "Connecting container '%s' to network '%s' (aliases=%s ipv4=%s ipv6=%s)",
        ...
        if mac_address is not None:
            # moby/moby#48192: --mac-address on network connect is silently
            # ignored. We accept the kwarg for forward-compat and to keep
            # the abstraction symmetric, but warn loudly so callers don't
            # think they have determinism when they don't.
            LOG.warning(
                "mac_address=%s passed to connect_container_to_network; "
                "Docker ignores this on post-hoc network connect (moby/moby#48192). "
                "Set the MAC at container create time for determinism.",
                mac_address,
            )
        try:
        ...
        *,
        ...
        if additional_flags:
            parsed_flags = Util.parse_additional_flags(
                additional_flags,
                env_vars=env_vars,
                # Pass the caller's labels through the parser so the merge
                # logic preserves them. Without this, any caller that supplies
                # both ``labels=...`` and ``additional_flags=...`` loses every
                # label that wasn't a ``--label`` token inside the flags
                # string. ECS task containers were the visible victim — their
                # localemu.service / cluster / task-arn labels were dropped
                # entirely and ``docker ps --filter label=localemu.service=ecs``
                # never saw the running container.
                labels=labels,
                volumes=volumes,
                network=network,
                platform=platform,
