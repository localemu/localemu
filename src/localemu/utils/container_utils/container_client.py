...
from localemu.constants import DEFAULT_VOLUME_DIR
from localemu.utils.collections import HashableList, ensure_list
from localemu.utils.files import TMP_FILES, chmod_r, rm_rf, save_file
from localemu.utils.no_exit_argument_parser import NoExitArgumentParser
...
class VolumeDirMount(Mount):
    volume_path: str
    """
    Absolute path inside /var/lib/localemu to mount into the container
    """
    container_path: str
    """
...
class ContainerClient(metaclass=ABCMeta):
    ...
    @abstractmethod
    def create_network(
        self, network_name: str, subnet: str | None = None, internal: bool = False,
        labels: dict[str, str] | None = None, print_error: bool = True,
    ) -> str:
        """
        Creates a Docker network with the given name and optional CIDR.

        :param network_name: Name of the network
        :param subnet: CIDR block for the network (e.g. "10.0.0.0/16")
        :param internal: If True, create an isolated network with no internet access
        :param labels: Optional key/value labels to attach via ``docker network create --label``
        :param print_error: If False, suppress the stderr/stdout ``ERROR: ...`` print
            when the underlying ``docker`` subprocess returns non-zero. The exception
            is still raised; only the noisy log line is hidden. Used by callers that
            expect a probe to fail (e.g. the smart subnet fallback in ``VpcNetworkManager``).
        :return: Network ID
        """
    ...
    def connect_container_to_network(
        ...
        :param ipv4_address: Optional IPv4 to pin (Docker --ip). Requires the
            network to have been created with an explicit ``--subnet``;
            on networks without a subnet Docker rejects the request.
        :param ipv6_address: Optional IPv6 to pin (Docker --ip6). Requires
            the network to have been created with ``enable_ipv6=True`` and
            an explicit ``--ipv6-subnet``.
        :param mac_address: Optional MAC. NOTE: Docker silently ignores
            ``--mac-address`` on ``network connect`` (moby/moby#48192).
            MAC determinism only works when the MAC is set at container
            create time; this parameter is accepted for forward-compat
            but produces a one-time WARN if non-None.
        """
...
class Util:
    ...
    def _read_docker_cli_env_file(env_file: str) -> dict[str, str]:
        ...
        try:
            ...
            LOG.error(
                "Specified env file '%s' not found. Please make sure the file is properly mounted into the LocalEmu container. Error: %s",
                env_file,
                e,
            )
            raise
            ...
            LOG.error(
                "Could not read env file '%s'. Please make sure the LocalEmu container has the permissions to read it. Error: %s",
                env_file,
                e,
            )
            raise
    ...
    def parse_additional_flags(
        ...
        parser.add_argument(
            "--sysctl",
            help="Sysctl options (key=value, repeatable)",
            dest="sysctls",
            action="append",
        )
