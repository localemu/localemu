...
class TCPProxy(Server):
    ...
    def do_run(self):
        ...
        # Accept timeout doubles as the shutdown-responsiveness polling interval:
        # closing the socket from ``do_shutdown`` does not always wake a blocked
        # accept() (notably on macOS / BSD), so the loop must re-check
        # ``_stopped`` periodically.
    ...
    def do_shutdown(self):
        if self._server_socket:
            # ``shutdown(SHUT_RDWR)`` on a listening socket raises ``ENOTCONN``
            # on macOS/BSD — the call is only meaningful for sockets with a
            # peer. Skip the failure: ``close()`` is enough to release the
            # bound port, and the accept loop already polls ``_stopped`` via
