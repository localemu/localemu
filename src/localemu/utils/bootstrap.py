...
from localemu.utils.files import cache_dir, mkdir
from localemu.utils.functions import call_safe
from localemu.utils.net import get_free_tcp_port, get_free_tcp_port_range
from localemu.utils.run import is_command_available, run, to_str
from localemu.utils.serving import Server
...
# - this mapping is used f.e. used for the selective test execution (localemu.testing.testselection)
...
def get_server_version_from_running_container() -> str:
    try:
        ...
        version, _ = DOCKER_CLIENT.exec_in_container(
            container_name, interactive=True, command=["bin/localemu", "--version"]
        )
...
def get_server_version() -> str:
    ...
    version_cache = cache_dir() / "image_metadata" / image_hash / "localemu_version"
    if version_cache.exists():
        cached_version = version_cache.read_text()
        return cached_version.strip()

    env_version = get_image_environment_variable("LOCALEMU_BUILD_VERSION")
...
def start_infra_locally():
    from localemu.runtime.main import main

    return main()


def validate_localemu_config(name: str):
    ...
    ls_service_name = [
        name for name, svc in services_config.items() if "localemu" in svc.get("image", "")
    ]
    if not ls_service_name:
        raise Exception(
            'No LocalEmu service found in config (looking for image names containing "localemu")'
        )
    if len(ls_service_name) > 1:
        warns.append(f"Multiple candidates found for LocalEmu service: {ls_service_name}")
...
class ContainerConfigurators:
    """
    A set of useful container configurators that are typical for starting the localemu container.
    """
    ...
    @staticmethod
    def mount_localemu_volume(host_path: str | os.PathLike = None):
    ...
    def config_env_vars(cfg: ContainerConfiguration):
        ...
        for env_var in config.CONFIG_ENV_VARS:
            ...
            if value is not None:
                if (
                    env_var != "CI"
                    and not env_var.startswith("LOCALEMU_")
                    and env_var not in profile_env
                ):
                    # Collect all env vars that are directly forwarded from the system env
                    # to the container which has not been prefixed with LOCALEMU_ here.
        ...
        if non_prefixed_env_vars:
            from localemu.utils.analytics import log

            for non_prefixed_env_var in non_prefixed_env_vars:
                # Show a deprecation warning for each individual env var collected above
                LOG.warning(
                    "Non-prefixed environment variable %(env_var)s is forwarded to the LocalEmu container! "
                    "Please use `LOCALEMU_%(env_var)s` instead of %(env_var)s to explicitly mark this environment variable to be forwarded from the CLI to the LocalEmu Runtime.",
...
def get_gateway_port(container: Container) -> int:
    ...
    :param container: the localemu container
...
def get_gateway_url(
    ...
    """
    Returns the localemu container's gateway URL reachable from the host. In most cases this will be
    ``http://localhost:4566``.

    :param container: the container
    :param hostname: the hostname to use (default localhost)
...
class Container:
    ...
    def start(self, attach: bool = False) -> RunningContainer:
        # FIXME: this is pretty awkward, but additional_flags in the LocalemuContainer API was
        ...
        try:
            ...
            LOG.error(
                "Error while starting LocalEmu container: %s\n%s",
                e.message,
                to_str(e.stderr),
...
class RunningContainer:
    """
    Represents a LocalEmu container that is running.
    """
...
class LocalemuContainerServer(Server):
    ...
    def do_run(self):
        if self.is_container_running():
            raise ContainerRunning(
                f'LocalEmu container named "{self.container.name}" is already running'
            )
        ...
        LOG.debug("starting LocalEmu container")
    ...
    def do_shutdown(self):
        try:
            ...
            LOG.info("error cleaning up localemu container %s: %s", self.container.name, e)
...
def prepare_docker_start():
    ...
    if DOCKER_CLIENT.is_container_running(container_name):
        raise ContainerRunning(f'LocalEmu container named "{container_name}" is already running')

    if container_name in DOCKER_CLIENT.get_all_container_names():
        raise ContainerExists(f'LocalEmu container named "{container_name}" already exists')
...
def configure_container(container: Container):
    """
    Configuration routine for the LocalemuContainer.
    """
    port_configuration = PortMappings(bind_host=config.GATEWAY_LISTEN[0].host)
    ...
    hooks.configure_localemu_container.run(container)
    ...
    container.configure(
        [
            ...
            ContainerConfigurators.mount_localemu_volume(config.VOLUME_DIR),
...
def prepare_host(console):
    """
    Prepare the host environment for running LocalEmu, this should be called before start_infra_*.
    """
    if os.environ.get(constants.LOCALEMU_INFRA_PROCESS) in constants.TRUE_STRINGS:
        return
...
def start_infra_in_docker(console, cli_params: dict[str, Any] = None):
    ...
    status = console.status("Starting LocalEmu container")
    status.start()
    ...
    def _init_log_printer(line):
        ...
        to print."""
        status.stop()
        console.rule("LocalEmu Runtime Log (press [bold][yellow]CTRL-C[/yellow][/bold] to quit)")
        print(line)
    ...
    #  being received (e.g., when running the localemu CLI as part of a "npm run .." script).
    ...
    # start the Localemu container as a Server
    server = LocalemuContainerServer(container)
...
def start_infra_in_docker_detached(console, cli_params: dict[str, Any] = None):
    ...
    server = LocalemuContainerServer(container_config)
    server.start()
...
def wait_container_is_ready(timeout: float | None = None):
    """Blocks until the localemu main container is running and the ready marker has been printed."""
    container_name = config.MAIN_CONTAINER_NAME
    started = time.time()
