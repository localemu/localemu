...
"""Default number of seconds a port is reserved in a PortRange."""
...
def get_docker_host_from_container() -> str:
    ...
    try:
        if not config.is_in_docker and not config.is_in_linux:
            # On macOS host mode, containers reach the host via host.docker.internal.
            # This hostname is resolved by Docker's built-in DNS inside containers.
