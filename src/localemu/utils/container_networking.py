...
from localemu.utils.net import get_docker_host_from_container
...
def get_main_container_network() -> str | None:
    """
    Gets the main network of the LocalEmu container (if we run in one, bridge otherwise)
    If there are multiple networks connected to the LocalEmu container, we choose the first as "main" network
    ...
    if config.MAIN_DOCKER_NETWORK:
        if config.is_in_docker:
            ...
            if config.MAIN_DOCKER_NETWORK not in networks:
                LOG.warning(
                    "The specified 'MAIN_DOCKER_NETWORK' is not connected to the LocalEmu container! Falling back to %s",
                    networks[0],
                )
                return networks[0]
...
def get_endpoint_for_network(network: str | None = None) -> str:
    """
    Get the LocalEmu endpoint (= IP address) on the given network.
    If a network is given, it will return the IP address/hostname of LocalEmu on that network
...
def get_main_container_id():
    """
    Return the container ID of the LocalEmu container
...
def get_main_container_name():
    """
    Returns the container name of the LocalEmu container

    :return: LocalEmu container name
    """
