...
from localemu.aws.api.cloudformation import (
...
from localemu.services.cloudformation import api_utils
from localemu.services.cloudformation.engine import parameters as param_resolver
from localemu.services.cloudformation.engine import template_deployer, template_preparer
from localemu.services.cloudformation.engine.entities import (
    Stack,
    StackChangeSet,
    StackInstance,
    StackSet,
)
from localemu.services.cloudformation.engine.parameters import mask_no_echo, strip_parameter_type
from localemu.services.cloudformation.engine.resource_ordering import (
    NoResourceInStack,
    order_resources,
)
from localemu.services.cloudformation.engine.template_deployer import (
    NoStackUpdates,
)
from localemu.services.cloudformation.engine.template_utils import resolve_stack_conditions
from localemu.services.cloudformation.engine.transformers import (
    FailedTransformationException,
)
from localemu.services.cloudformation.engine.validations import (
...
from localemu.services.cloudformation.stores import (
...
from localemu.utils.aws.arns import ARN_PARTITION_REGEX
from localemu.utils.collections import (
    remove_attributes,
    select_attributes,
    select_from_typed_dict,
)
from localemu.utils.json import clone
...
class CloudformationProvider(CloudformationApi, ServiceLifecycleHook):
    ...
    def update_stack(
        ...
        stack = find_active_stack_by_name_or_id(context.account_id, context.region, stack_name)
        if not stack:
            not_found_error(f'Unable to update non-existing stack "{stack_name}"')
    ...
    def get_template(
        ...
        **kwargs,
        ...
        if change_set_name:
            ...
            stack = find_active_stack_by_name_or_id(context.account_id, context.region, stack_name)
        if not stack:
            stack_not_found_error(stack_name)
    ...
    def list_change_sets(
        ...
        **kwargs,
        ...
        stack = find_active_stack_by_name_or_id(context.account_id, context.region, stack_name)
        if not stack:
            not_found_error(f'Unable to find stack "{stack_name}"')
    ...
    def describe_stack_set_operation(
        ...
        **kwargs,
        ...
        if not stack_set:
            not_found_error(f'Unable to find stack set "{set_name}"')
    ...
    def describe_stack_set(
        ...
        **kwargs,
        ...
        if not result:
            not_found_error(f'Unable to find stack set "{stack_set_name}"')
    ...
    def update_stack_set(
        ...
        stack_set = [sset for sset in state.stack_sets.values() if sset.stack_set_name == set_name]
        if not stack_set:
            not_found_error(f'Stack set named "{set_name}" does not exist')
        stack_set = stack_set[0]
    ...
    def delete_stack_set(
        ...
        **kwargs,
        ...
        if not stack_set:
            not_found_error(f'Stack set named "{stack_set_name}" does not exist')
