...
LOCALEMU_ROOT_DIR = Path(__file__).parent.joinpath("../../../../..").resolve()
TESTS_ROOT_DIR = LOCALEMU_ROOT_DIR.joinpath("tests/aws/services")

assert LOCALEMU_ROOT_DIR.is_dir(), f"{LOCALEMU_ROOT_DIR} does not exist"
assert TESTS_ROOT_DIR.is_dir(), f"{TESTS_ROOT_DIR} does not exist"


def root_dir() -> Path:
    return LOCALEMU_ROOT_DIR
...
class TemplateRenderer:
    ...
    def render(
        ...
        tests_output_path = root_dir().joinpath(
            f"tests/aws/{resource_name.python_compatible_service_name.lower()}/resource_providers/templates"
        )
        match file_type:
...
class FileWriter:
    ...
    def __init__(self, resource_name: ResourceName, console: Console, overwrite: bool):
        ...
        # Path inside the repo where generated provider files land.
        base_path = ["src", "localemu"]

        self.destination_files = {
            FileType.provider: root_dir().joinpath(
                *base_path,
                "services",
            ...
            FileType.provider_base: root_dir().joinpath(
                *base_path,
                "services",
                self.resource_name.python_compatible_service_name.lower(),
                "resource_providers",
            ...
            FileType.plugin: root_dir().joinpath(
                *base_path,
                "services",
                self.resource_name.python_compatible_service_name.lower(),
                "resource_providers",
            ...
            FileType.schema: root_dir().joinpath(
                *base_path,
                "services",
                self.resource_name.python_compatible_service_name.lower(),
                "resource_providers",
        ...
        for template_type in templates:
            self.destination_files[template_type] = template_path(
                self.resource_name, template_type
            )
