...
from localemu.services.cloudformation.deployment_utils import PLACEHOLDER_AWS_NO_VALUE
from localemu.services.cloudformation.engine.errors import TemplateError
from localemu.utils.urls import localemu_host

AWS_URL_SUFFIX = localemu_host().host  # value is "amazonaws.com" in real AWS
            ...
            match k:
                ...
                case "Fn::FindInMap":
                    ...
                    if isinstance(map_name, dict):
                        map_name = resolve_condition(
                            account_id, region_name, map_name, conditions, parameters, mappings, stack_name
                        )

                    if isinstance(top_level_key, dict):
                        top_level_key = resolve_condition(
                            account_id, region_name, top_level_key, conditions, parameters, mappings, stack_name
                        )

                    if isinstance(second_level_key, dict):
                        second_level_key = resolve_condition(
                            account_id, region_name, second_level_key, conditions, parameters, mappings, stack_name
                        )
                ...
                case "Fn::And":
                    # AWS supports 2-10 conditions for Fn::And
                    return all(
                        resolve_condition(
                            account_id, region_name, c, conditions, parameters, mappings, stack_name
                        )
                        for c in v
                    )
                case "Fn::Or":
                    # AWS supports 2-10 conditions for Fn::Or
                    return any(
                        resolve_condition(
                            account_id, region_name, c, conditions, parameters, mappings, stack_name
                        )
