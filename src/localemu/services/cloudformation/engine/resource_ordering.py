...
from localemu.services.cloudformation.engine.changes import ChangeConfig
from localemu.services.cloudformation.engine.parameters import StackParameter
from localemu.services.cloudformation.engine.template_utils import get_deps_for_resource
...
def order_resources(
    ...
    if len(sorted_logical_resource_ids) != len(nodes):
        remaining = set(nodes.keys()) - set(sorted_logical_resource_ids)
        raise Exception(
            f"Circular dependency found among resources: [{', '.join(sorted(remaining))}]"
        )
