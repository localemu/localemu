...
from localemu.services.cloudformation.analytics import track_resource_operation
from localemu.services.cloudformation.deployment_utils import (
    PLACEHOLDER_AWS_NO_VALUE,
    get_action_name_for_resource_change,
    log_not_available_message,
    remove_none_values,
)
from localemu.services.cloudformation.engine.changes import ChangeConfig, ResourceChange
from localemu.services.cloudformation.engine.entities import Stack, StackChangeSet
from localemu.services.cloudformation.engine.parameters import StackParameter
from localemu.services.cloudformation.engine.quirks import VALID_GETATT_PROPERTIES
from localemu.services.cloudformation.engine.resource_ordering import (
    order_changes,
    order_resources,
)
from localemu.services.cloudformation.engine.template_utils import (
    AWS_URL_SUFFIX,
...
from localemu.services.cloudformation.service_models import (
    DependencyNotYetSatisfied,
)
from localemu.services.cloudformation.stores import exports_map, find_stack
from localemu.utils.aws.arns import get_partition
from localemu.utils.functions import prevent_stack_overflow
from localemu.utils.json import clone_safe
from localemu.utils.strings import to_bytes, to_str
from localemu.utils.threads import start_worker_thread

from localemu.services.cloudformation.models import *  # noqa: F401, F403, isort:skip
...
# Cache for Fn::GetAZs results per region to avoid repeated API calls
_az_cache: dict[str, list[str]] = {}
...
def get_attr_from_model_instance(
    ...
    if resource["PhysicalResourceId"] == MOCK_REFERENCE:
        raise Exception(
            f"Cannot resolve attribute '{attribute_name}' for unsupported resource "
            f"'{resource_id}' of type '{resource_type}'. The resource type has no provider "
            f"and was assigned a mock PhysicalResourceId. Either implement the resource "
            f"provider or set CFN_IGNORE_UNSUPPORTED_RESOURCE_TYPES=0 to fail early."
        )
    ...
    if attribute_candidate is None:
        LOG.warning(
            "Fn::GetAtt resolved to None for %s: %s.%s — resource may not be fully deployed yet",
            resource_type,
            resource_id,
            attribute_name,
        )
        ...
        if dynamic_ref_match:
            ...
            if service_name == "ssm":
                ...
                try:
                    ...
                    LOG.error("client error while trying to access key '%s'", secret_id, exc_info=True)
                    raise
...
def _resolve_refs_recursively(
    ...
    if isinstance(value, dict):
        ...
        if stripped_fn_lower == "findinmap":
            ...
            if isinstance(mapping_id, dict):
                mapping_id = resolve_refs_recursively(
                    account_id,
                    region_name,
                    stack_name,
                    resources,
        ...
        if stripped_fn_lower == "importvalue":
            ...
            if not stack_export.get("Value"):
                ...
                raise Exception(
                    f"No export named '{import_value_key}' found. Rollback requested by user."
                )
            return stack_export["Value"]

        if stripped_fn_lower == "if":
            condition_name, option1, option2 = value[keys_list[0]]
            condition_value = conditions.get(condition_name)
            if condition_value is None:
                ...
                raise KeyError(
                    f"Cannot find condition '{condition_name}' in conditions mapping: '{conditions.keys()}'"
                )
            ...
            result = resolve_refs_recursively(
                ...
                conditions,
                parameters,
                option1 if condition_value else option2,
            )
            return result
        ...
        if stripped_fn_lower in ["and", "or"]:
            # Audit fix: renamed to fn_conditions to avoid shadowing the
            # outer `conditions` parameter needed by resolve_refs_recursively
            fn_conditions = value[keys_list[0]]
            results = [
        ...
        if stripped_fn_lower == "select":
            ...
            try:
                ...
                if idx < 0 or idx >= len(values):
                    raise IndexError(
                        f"Fn::Select cannot select nonexistent value at index {idx}"
                    )
                return values[idx]
            except (TypeError, ValueError) as e:
                raise Exception(
                    f"Template error: Fn::Select index must be an integer, got {index!r}"
                ) from e
        ...
        if stripped_fn_lower == "getazs":
            ...
            if region in _az_cache:
                return _az_cache[region]
...
def resolve_placeholders_in_string(
    ...
    def _validate_result_type(value: str):
        # Fn::Sub always returns a string per AWS specification.
        # Do not convert numeric strings to int/float.
        return value
        ...
        if len(parts) == 1:
            if parts[0] in resources or parts[0].startswith("AWS::"):
                ...
                if parameter_type in ["CommaDelimitedList"] or parameter_type.startswith("List<"):
                    return ",".join(p.strip() for p in parameter_value.split(","))
...
class TemplateDeployer:
    ...
    def apply_changes(
        ...
        # Only set changed resources to UPDATE_IN_PROGRESS, not all resources (CF-32)
        changed_resource_ids = {
            change["ResourceChange"]["LogicalResourceId"] for change in changes
        }
        for resource_id in changed_resource_ids:
            if resource_id in old_resources:
                existing_stack.set_resource_status(resource_id, f"{action}_IN_PROGRESS")
    ...
    def delete_stack(self):
        ...
        with self.stack.lock:
            self._delete_stack_impl()

    def _delete_stack_impl(self):
            ...
            try:
                ...
                match event.status:
                    ...
                    case OperationStatus.PENDING:
                        ...
                        continue
                    case OperationStatus.FAILED:
                        deletion_failures.append(resource_id)
                        LOG.error(
        ...
        if deletion_failures:
            self.stack.set_stack_status(
                "DELETE_FAILED",
                status_reason=f"The following resource(s) failed to delete: [{', '.join(deletion_failures)}].",
            )
        else:
