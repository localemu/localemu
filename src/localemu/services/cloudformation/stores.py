...
from localemu.aws.api.cloudformation import Export, StackStatus
from localemu.services.cloudformation.engine.entities import Stack, StackChangeSet, StackSet
from localemu.services.cloudformation.v2.entities import ChangeSet as ChangeSetV2
from localemu.services.cloudformation.v2.entities import Stack as StackV2
from localemu.services.cloudformation.v2.entities import StackSet as StackSetV2
...
class CloudFormationStore(BaseStore):
    ...
    # exports cache for V1 stacks
    _exports_cache: dict[str, Export] | None = None
    _exports_cache_stack_versions: dict[str, str] | None = None

    def invalidate_exports_cache(self):
        self._exports_cache = None
        self._exports_cache_stack_versions = None

    @property
    def exports(self) -> dict[str, Export]:
        # Build a version fingerprint based on stack statuses and output counts
        current_versions = {}
        for stack_id, stack in self.stacks.items():
            current_versions[stack_id] = f"{stack.status}:{len(stack.resolved_outputs)}"

        if (
            self._exports_cache is not None
            and self._exports_cache_stack_versions == current_versions
        ):
        ...
        self._exports_cache = exports
        self._exports_cache_stack_versions = current_versions
        return exports
