...
from localemu.aws.connect import InternalClientFactory, ServiceLevelClientFactory
...
from localemu.services.cloudformation.engine.quirks import PHYSICAL_RESOURCE_ID_SPECIAL_CASES
from localemu.services.cloudformation.provider_utils import convert_request_kwargs
from localemu.services.cloudformation.service_models import KEY_RESOURCE_STATE

if TYPE_CHECKING:
    from localemu.services.cloudformation.engine.types import (
        FuncDetails,
        FuncDetailsValue,
        ResourceDefinition,
    )
...
class CloudFormationResourceProviderPlugin(Plugin):
    ...
    namespace = "localemu.cloudformation.resource_providers"
...
def resolve_json_pointer[Properties](resource_props: Properties, primary_id_path: str) -> str:
    ...
    if not parts:
        raise Exception(f"Cannot resolve empty JSON pointer path for resource properties: {primary_id_path}")
...
class ResourceProviderExecutor:
        ...
        match change_type:
            ...
            case "Dynamic" | "Modify":
                try:
                    ...
                    feature_request_url = "https://github.com/localemu/localemu/issues/new?template=feature-request.yml"
                    LOG.warning(
    ...
    def try_load_resource_provider(resource_type: str) -> ResourceProvider | None:
        ...
        # try to load the resource provider
        try:
