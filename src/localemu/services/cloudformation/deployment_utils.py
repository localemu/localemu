...
from localemu.utils.aws import aws_stack
from localemu.utils.common import select_attributes, short_uid
...
from localemu.utils.strings import is_string
from localemu.services.cloudformation.provider_utils import (  # noqa: F401
...
def remove_none_values(params):
    ...
    def remove_nones(o, **kwargs):
        ...
        if isinstance(o, list):
            o[:] = [v for v in o if v is not None and v != PLACEHOLDER_AWS_NO_VALUE]
        return o
...
# generate_default_name and generate_default_name_without_stack are imported from
# provider_utils at the top of this module (CF-25: consolidated duplicate definitions)
...
def fix_boto_parameters_based_on_report(original_params: dict, report: str) -> dict:
    ...
    for found in param_validation.findall(report):
        ...
        if cast_class is bool and str(old_value).lower() in ["true", "false"]:
...
def log_not_available_message(resource_type: str, message: str):
    LOG.warning(
        "%s. The %s resource is not currently emulated by LocalEmu. "
        "Open an issue at https://github.com/localemu/localemu/issues if you need it.",
        message,
        resource_type,
    )
