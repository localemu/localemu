...
from localemu.aws.api.transcribe import (
    BadRequestException,
    ConflictException,
...
from localemu.constants import HUGGING_FACE_ENDPOINT
...
from localemu.services.transcribe.models import TranscribeStore, transcribe_stores
from localemu.services.transcribe.packages import vosk_package
from localemu.state import StateVisitor
from localemu.utils.files import new_tmp_file
from localemu.utils.http import download
...
class TranscribeProvider(TranscribeApi):
    ...
    def download_model(name: str) -> str:
        """
        Download a Vosk language model to LocalEmu cache directory. Do nothing if model is already downloaded.
        ...
        with _DL_LOCK:
            ...
            if not download_succeeded:
                # Remove the empty model directory so a future job can retry.
                try:
                    model_path.rmdir()
                except OSError:
                    pass
                raise RuntimeError(
                    f"Failed to download Vosk language model '{model_path.name}' from any configured URL"
                ) from last_error
    ...
    def _run_transcription_job(self, args: tuple[TranscribeStore, str]) -> None:
        ...
        job["StartTime"] = datetime.datetime.now(datetime.timezone.utc)
        ...
        try:
            ...
            if format not in SUPPORTED_FORMAT_NAMES:
            ...
            # Check if file is valid wav — wrap in try/finally to guarantee the
            # file handle is closed even if validation or transcription fails.
            ...
            # Update job details
            job["CompletionTime"] = datetime.datetime.now(datetime.timezone.utc)
