...
from localemu.aws.api.config import TagsList
from localemu.aws.api.events import (
    Action,
...
from localemu.aws.api.events import ApiDestination as ApiTypeApiDestination
from localemu.aws.api.events import Archive as ApiTypeArchive
from localemu.aws.api.events import Connection as ApiTypeConnection
from localemu.aws.api.events import EventBus as ApiTypeEventBus
from localemu.aws.api.events import Replay as ApiTypeReplay
from localemu.aws.api.events import Rule as ApiTypeRule
from localemu.services.events.api_destination import (
    APIDestinationService,
    ApiDestinationServiceDict,
)
from localemu.services.events.archive import ArchiveService, ArchiveServiceDict
from localemu.services.events.connection import (
    ConnectionService,
    ConnectionServiceDict,
)
from localemu.services.events.event_bus import EventBusService, EventBusServiceDict
from localemu.services.events.models import (
    ApiDestination,
    ApiDestinationDict,
    Archive,
    ArchiveDict,
...
from localemu.services.events.replay import ReplayService, ReplayServiceDict
from localemu.services.events.rule import RuleService, RuleServiceDict
from localemu.services.events.scheduler import JobScheduler
from localemu.services.events.target import (
    TargetSender,
    TargetSenderDict,
    TargetSenderFactory,
)
from localemu.services.events.utils import (
    TARGET_ID_PATTERN,
...
from localemu.utils.common import truncate
from localemu.utils.event_matcher import matches_event
from localemu.utils.strings import long_uid
from localemu.utils.threads import start_worker_thread
from localemu.utils.time import TIMESTAMP_FORMAT_TZ, timestamp
...
class EventsProvider(EventsApi, ServiceLifecycleHook):
    ...
    # ------------------------------------------------------------------
    # State persistence (EB-5)
    #
    # EventBridge holds a second, non-persistable layer of in-memory
    # *service* objects (_event_bus_services_store, _rule_services_store,
    # _target_sender_store, _archive_service_store, _replay_service_store,
    # _connection_service_store, _api_destination_service_store) that wrap
    # the domain objects stored inside `events_stores`. Only the stores are
    # visited by the state visitor — so on a snapshot restore the service
    # wrappers are missing and subsequent API calls (PutEvents, RemoveTargets,
    # DeleteRule, etc.) operate on an inconsistent map.
    #
    # The hooks below rebuild the service layer from the persisted stores
    # after a state load/reset and clear it before a reset. They are
    # intentionally conservative: when some wrapper can't be rebuilt we log
    # and skip rather than abort the whole load.
    # ------------------------------------------------------------------
    def on_before_state_reset(self) -> None:
        self._clear_service_stores()

    def on_after_state_reset(self) -> None:
        self._clear_service_stores()

    def on_after_state_load(self) -> None:
        self._rebuild_service_stores()

    def _clear_service_stores(self) -> None:
        self._event_bus_services_store.clear()
        self._rule_services_store.clear()
        self._target_sender_store.clear()
        self._archive_service_store.clear()
        self._replay_service_store.clear()
        self._connection_service_store.clear()
        self._api_destination_service_store.clear()

    def _rebuild_service_stores(self) -> None:
        """Rebuild the per-request in-memory service wrappers from events_stores.

        Only rebuilds what we can safely reconstruct (event buses, rules, target
        senders). Archive/Replay/Connection/ApiDestination services are best-effort
        and silently skipped if their constructors require data that isn't stored.
        """
        self._clear_service_stores()
        try:
            for account_id, region_store in events_stores.items():
                for region, store in region_store.items():
                    for event_bus in store.event_buses.values():
                        try:
                            # Wrap the persisted EventBus as-is; EventBusService only
                            # stores a reference to it so identities (rules, policy,
                            # last_modified_time, etc.) are preserved verbatim.
                            service = EventBusService(event_bus)
                            self._event_bus_services_store[event_bus.arn] = service
                        except Exception as exc:  # pragma: no cover - best-effort
                            LOG.debug(
                                "events: could not rebuild event bus service %s: %s",
                                event_bus.arn,
                                exc,
                            )
                            continue
                        ...
                        for rule in event_bus.rules.values():
                            try:
                                rule_service = RuleService(rule)
                                self._rule_services_store[rule.arn] = rule_service
                            except Exception as exc:  # pragma: no cover
                                LOG.debug(
                                    "events: could not rebuild rule service %s: %s",
                                    rule.arn,
                                    exc,
                                )
                                continue
                            for target_id, target in rule.targets.items():
                                try:
                                    self.create_target_sender(
                                        target, rule.arn, rule.name, region, account_id
                                    )
                                except Exception as exc:  # pragma: no cover
                                    LOG.debug(
                                        "events: could not rebuild target sender %s/%s: %s",
                                        rule.arn,
                                        target_id,
                                        exc,
                                    )
        except Exception as exc:  # pragma: no cover
            LOG.warning("events: unexpected error rebuilding service stores: %s", exc)
    ...
    def delete_event_bus(self, context: RequestContext, name: EventBusName, **kwargs) -> None:
        ...
        account_id = context.account_id
        store = self.get_store(region, account_id)
        # EB-1: do NOT return the exception object — the framework treats that as a
        # successful response and silently drops the real error. Let
        # ResourceNotFoundException propagate so clients see the correct 404.
        # EB-2: when deleting a bus, also remove every target_sender that belongs
        # to any rule attached to the bus, so _target_sender_store doesn't leak.
        # EB-3: TaggingService has no ``.pop()`` — previous code raised
        # ``AttributeError`` partway through the delete, leaving the store in an
        # inconsistent state. botocore then retried, got ResourceNotFoundException
        # on the second pass, and surfaced that as the operation error.
        event_bus = self.get_event_bus(name, store)
        for rule in list(event_bus.rules.values()):
            for target_id in list(rule.targets.keys()):
                target_unique_id = f"{rule.arn}-{target_id}"
                self._target_sender_store.pop(target_unique_id, None)
        self._event_bus_services_store.pop(event_bus.arn, None)
        if rules := event_bus.rules:
            self._delete_rule_services(rules)
        del store.event_buses[name]
        store.TAGS.del_resource(event_bus.arn)
    ...
    def delete_rule(
        ...
        **kwargs,
        ...
        event_bus_name = extract_event_bus_name(event_bus_name)
        event_bus = self.get_event_bus(event_bus_name, store)
        # EB-1: do not swallow ResourceNotFoundException by `return error`-ing it;
        # let it propagate to the caller. ValidationException must also propagate.
        rule = self.get_rule(name, event_bus)
        if rule.targets and not force:
            raise ValidationException("Rule can't be deleted since it has targets.")
        # EB-2 companion: clean up any target senders belonging to this rule.
        for target_id in list(rule.targets.keys()):
            self._target_sender_store.pop(f"{rule.arn}-{target_id}", None)
        self._delete_rule_services(rule)
        del event_bus.rules[name]
        store.TAGS.del_resource(rule.arn)
    ...
    def put_targets(
        ...
        **kwargs,
        ...
        # EB-4: validate every target_id BEFORE mutating any state. The previous
        # implementation called add_targets first, which meant an invalid id in
        # the middle of the batch left earlier targets persisted while raising
        # ValidationException — a torn-write that AWS never produces.
    ...
    def delete_archive(
        ...
        if archive := self.get_archive(archive_name, store):
            # EB-1: propagate errors rather than returning the exception object,
            # which the framework would otherwise treat as a 200 OK response.
            archive_service = self._archive_service_store.pop(archive.arn, None)
    ...
    def _process_entries(
        ...
        # EB-3: work on per-entry shallow copies so we never mutate the caller's
        # request payload (e.g. _process_entry injects TraceHeader). Keeping the
        # caller's list/entries immutable avoids surprising the framework
        # (and any future retry/replay logic) with rewritten entries.
        for event in entries:
            entry_copy = dict(event) if isinstance(event, dict) else event
            self._process_entry(entry_copy, processed_entries, failed_entry_count, context)
