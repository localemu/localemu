...
from localemu.aws.api.kms import (
    AlgorithmSpec,
...
from localemu.services.kms.exceptions import ValidationException
from localemu.services.kms.models import (
...
from localemu.services.kms.utils import (
...
from localemu.utils.aws.arns import get_partition, kms_alias_arn, parse_arn
from localemu.utils.collections import PaginatedList
from localemu.utils.common import select_attributes
from localemu.utils.crypto import pkcs7_envelope_encrypt
...
class KmsProvider(KmsApi, ServiceLifecycleHook):
    """
    The LocalEmu Key Management Service (KMS) provider.
    ...
    # in LocalEmu, so this function is limited to them.
    #
    ...
    def create_grant(
        ...
        # KMS-3: grants live alongside the key they govern, not the caller. When a
        # cross-account principal creates a grant on another account's key, the grant
        # must be stored in (and listable from) the key's owning account/region,
        # matching AWS behaviour. Using the caller's store here made the grant invisible
        # to the key owner and to ListGrants in that account.
        store = self._get_store(key_account_id, key_region_name)
    ...
    # Currently LocalEmu only calculates SHA256 digests no matter what the signing algorithm is.
    ...
    def re_encrypt(
        ...
        **kwargs,
        ...
        # SourceKeyId is optional on the AWS API — for symmetric ciphertexts
        # the source key is encoded in the LocalEmu envelope, so the caller
        # only needs to specify DestinationKeyId. Without the guard below
        # _parse_key_id(None) crashed with the bare-regex
        # "expected string or bytes-like object, got 'NoneType'", surfacing
        # as a 500 InternalError for the most common ReEncrypt invocation
        # (re-key a DEK without naming the source key).
        if source_key_id is None:
            try:
                ciphertext = deserialize_ciphertext_blob(
                    ciphertext_blob=ciphertext_blob,
                )
                source_key_id = ciphertext.key_id
            except Exception:
                raise InvalidCiphertextException(
                    "LocalEmu is unable to deserialize the ciphertext blob. Perhaps the "
                    "blob didn't come from LocalEmu",
                )
    ...
    def encrypt(
        ...
        **kwargs,
        ...
        # For compatibility, we return EncryptionAlgorithm values expected from AWS. But LocalEmu currently always
    ...
    def decrypt(
        ...
        **kwargs,
        ...
        # Since LocalEmu doesn't currently do asymmetrical encryption, there is a question of modeling here: we
        ...
        if key_id:
            ...
            try:
                ...
                # KMS-5: Failing to deserialize when a key_id is supplied is the
                # expected path for *asymmetric* decrypt — the ciphertext is raw
                # RSA-encrypted bytes, not a LocalEmu envelope. Fall through with
                # ciphertext=None so the RSA branch below can handle it. Log at
                # debug level (not error) to avoid noisy log spam on a normal code
                # path, and drop the bare `pass`.
                logging.debug(
                    "Ciphertext not in LocalEmu envelope format; will attempt asymmetric "
                    "decrypt path (key_id=%s): %s",
                    key_id,
                    e,
                )
                ciphertext = None
        ...
        # localemu as a drop-in replacement for AWS when testing without having to skip the secondary decryption
        # when using localemu.
        ...
        try:
            # KMS-1: dispatch on the actual key type rather than assuming symmetric.
            # - RSA asymmetric keys without a LocalEmu envelope: use RSA decrypt.
            # - Symmetric keys (SYMMETRIC_DEFAULT): envelope-based AES-GCM decrypt.
            # - Asymmetric keys where the caller happened to wrap with a LocalEmu
            #   envelope: fall back to the envelope path to preserve back-compat.
            key_spec = key.crypto_key.key_spec
            is_rsa = self._is_rsa_spec(key_spec)
            if is_rsa and ciphertext is None:
                plaintext = key.decrypt_rsa(ciphertext_blob)
                # Surface the algorithm that was actually used when the caller
                # didn't specify one, so clients see a faithful echo.
                if not encryption_algorithm:
                    encryption_algorithm = EncryptionAlgorithmSpec.RSAES_OAEP_SHA_256
            else:
                if ciphertext is None:
                    raise InvalidCiphertextException()
                plaintext = key.decrypt(ciphertext, encryption_context)
                if not encryption_algorithm:
                    encryption_algorithm = EncryptionAlgorithmSpec.SYMMETRIC_DEFAULT
        except InvalidTag:
            raise InvalidCiphertextException()
    ...
    def get_parameters_for_import(
        ...
        **kwargs,
        ...
        # are valid for 24 hours." — KMS-4: honour the documented 24h expiry so clients
        # that rely on ParametersValidTo behave as they do on real AWS.
    ...
    def delete_alias(self, context: RequestContext, request: DeleteAliasRequest) -> None:
        ...
        # LocalEmu, are supported by this operation.
        store = self._get_store(context.account_id, context.region)
        alias_name = request["AliasName"]
        if alias_name not in store.aliases:
    ...
    def _validate_grant_request(self, data: dict):
        """Validate an incoming CreateGrant payload.

        KMS-2 (documented scope): this performs the validations that matter for
        LocalEmu correctness — presence of KeyId/GranteePrincipal/Operations and the
        enum check against ``VALID_OPERATIONS``. It intentionally does NOT re-validate:
          - ARN well-formedness of GranteePrincipal / RetiringPrincipal (delegated to
            IAM resolution at consumption time),
          - length limits on Name (AWS rejects >256 chars; we don't enforce this since
            upstream typed dispatch already bounds it elsewhere),
          - the shape of Constraints.EncryptionContext{Equals,Subset}; AWS accepts any
            string→string map here and so do we.
        Add more checks here if AWS parity becomes load-bearing for a test.
        """
...
# APIs to do that. Functions here are supposed to be used by other LocalEmu services to have similar integrations
# with KMS in LocalEmu. As such, they are supposed to be proper APIs (as in error and security handling),
