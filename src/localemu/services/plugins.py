...
from localemu.config import ServiceProviderConfig
from localemu.runtime import hooks
from localemu.state import StateLifecycleHook, StateVisitable, StateVisitor
from localemu.utils.bootstrap import get_enabled_apis, is_api_enabled, log_duration
from localemu.utils.functions import call_safe
from localemu.utils.sync import SynchronizedDefaultDict, poll_condition

# set up logger
LOG = logging.getLogger(__name__)

# namespace for AWS provider plugins
PLUGIN_NAMESPACE = "localemu.aws.provider"
...
class Service:
    ...
    FIXME: this has become frankenstein's monster, and it has to go. once we've rid ourselves of the legacy edge
     ...
     ``start_<whatever>``, ``check_<whatever>``, ``check_<whatever>``, etc. methods, to make all of those integral part
...
def aws_provider(api: str = None, name="default", should_load: Callable[[], bool] = None):
    ...
    decorator are discoverable as a PluginSpec within the namespace "localemu.aws.provider", with the name
...
def eager_load_services():
    from localemu.utils.bootstrap import get_preloaded_services
