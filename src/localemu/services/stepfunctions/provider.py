...
from localemu.aws.api.stepfunctions import (
...
from localemu.services.stepfunctions.asl.component.state.state_execution.state_map.iteration.itemprocessor.map_run_record import (
    MapRunRecord,
)
from localemu.services.stepfunctions.asl.eval.callback.callback import (
...
from localemu.services.stepfunctions.asl.eval.event.logging import (
    CloudWatchLoggingConfiguration,
    CloudWatchLoggingSession,
)
from localemu.services.stepfunctions.asl.parse.asl_parser import (
    ASLParserException,
)
from localemu.services.stepfunctions.asl.static_analyser.express_static_analyser import (
    ExpressStaticAnalyser,
)
from localemu.services.stepfunctions.asl.static_analyser.static_analyser import (
    StaticAnalyser,
)
from localemu.services.stepfunctions.asl.static_analyser.test_state.test_state_analyser import (
    TestStateStaticAnalyser,
)
from localemu.services.stepfunctions.asl.static_analyser.usage_metrics_static_analyser import (
    UsageMetricsStaticAnalyser,
)
from localemu.services.stepfunctions.backend.activity import Activity, ActivityTask
from localemu.services.stepfunctions.backend.alias import Alias
from localemu.services.stepfunctions.backend.execution import Execution, SyncExecution
from localemu.services.stepfunctions.backend.models import SFNStore, sfn_stores
from localemu.services.stepfunctions.backend.state_machine import (
    StateMachineInstance,
    StateMachineRevision,
    StateMachineVersion,
    TestStateMachine,
)
from localemu.services.stepfunctions.backend.test_state.execution import (
    TestStateExecution,
)
from localemu.services.stepfunctions.backend.test_state.test_state_mock import TestStateMock
from localemu.services.stepfunctions.local_mocking.mock_config import (
    LocalMockTestCase,
    load_local_mock_test_case_for,
)
from localemu.services.stepfunctions.stepfunctions_utils import (
...
from localemu.utils.collections import PaginatedList
from localemu.utils.strings import long_uid, short_uid

LOG = logging.getLogger(__name__)

# Module-level reentrant lock that serialises mutations of the state machine / alias / execution
# stores. The underlying dicts are thread-safe for individual operations but compound
# check-then-act sequences (e.g. "look up by name, create if missing") require explicit
# synchronisation to prevent two concurrent CreateStateMachine calls from both deciding the
# state machine does not yet exist and each inserting their own copy (SF-4).
_SF_STORE_LOCK = threading.RLock()
...
class StepFunctionsProvider(StepfunctionsApi, ServiceLifecycleHook):
    ...
    def _find_idempotent_or_conflicting_revision(
        self,
        context: RequestContext,
        name: str,
    ...
    ) -> tuple[StateMachineRevision | None, StateMachineRevision | None]:
        """Scan the state machine store ONCE and return ``(idempotent, conflicting)``.

        Previously ``create_state_machine`` iterated all state machines twice — first to look
        for an idempotent match and then again to check for a name collision. We now do both in
        a single pass (SF-1):

        * ``idempotent`` — an existing revision with the same name *and* identical
          definition/type/logging/tracing configuration. Callers should return the existing ARN.
        * ``conflicting`` — an existing revision with the same name but different attributes.
          Callers must raise ``StateMachineAlreadyExists``.
        """
        idempotent: StateMachineRevision | None = None
        conflicting: StateMachineRevision | None = None
        with _SF_STORE_LOCK:
            state_machines = list(self.get_store(context).state_machines.values())
        for state_machine in state_machines:
            ...
            if all(
                ...
                idempotent = state_machine
                break
            conflicting = state_machine
            break
        return idempotent, conflicting
    ...
    def _idempotent_revision(
        ...
        # Retained for backwards compatibility with any external callers; delegates to the
        # single-pass helper above (SF-1).
        idempotent, _ = self._find_idempotent_or_conflicting_revision(
            context=context,
            name=name,
            definition=definition,
    ...
    def _idempotent_start_execution(
        ...
        # StartExecution is idempotent for STANDARD workflows only. For STANDARD:
        #   - Same name + same input + currently RUNNING  -> return the existing execution.
        #   - Otherwise                                   -> ExecutionAlreadyExists.
        # EXPRESS workflows do NOT support idempotent StartExecution: any name collision with a
        # live execution (or a completed one within the retention window) must fail with
        # ExecutionAlreadyExists regardless of whether the input matches (SF-3).

        if not execution:
            return None

        if state_machine.sm_type == StateMachineType.EXPRESS:
            raise CommonServiceException(
                code="ExecutionAlreadyExists",
                message=f"Execution Already Exists: '{execution.exec_arn}'",
            )

        # STANDARD workflow — apply idempotent match.
        if (
            name == execution.name
            and input_data == execution.input_data
            and execution.exec_status == ExecutionStatus.RUNNING
        ):
            return execution
    ...
    def create_state_machine(
        ...
        # resource if it was already created. Do the idempotency check and the name-collision
        # check in a SINGLE pass over the store (SF-1) and under the store lock so concurrent
        # callers can't race past the check (SF-4).
        with _SF_STORE_LOCK:
            idem_state_machine, conflicting = self._find_idempotent_or_conflicting_revision(
                context=context,
            ...
            if conflicting is not None:
                raise StateMachineAlreadyExists(
                    f"State Machine Already Exists: '{conflicting.arn}'"
                )
        ...
        # Create the state machine and add it to the store under the store lock so the insert is
        # atomic with the idempotency / name-collision check performed above (SF-4).
    ...
    def _validate_state_machine_alias_routing_configuration(
        ...
        for routing_configuration_arn in routing_configuration_arn_list:
            ...
            if state_machine_revision_arn != shared_state_machine_revision_arn:
                # All routing targets in a single alias must point at versions of the SAME
                # state machine revision. Previously this branch raised ``ValidationException("TODO")``
                # with an empty placeholder message (SF-2).
                raise ValidationException(
                    "Routing configuration must reference versions of the same state machine. "
                    f"Received versions from '{shared_state_machine_revision_arn}' and "
                    f"'{state_machine_revision_arn}'."
                )
    ...
    def delete_state_machine(
        ...
        self._validate_state_machine_arn(state_machine_arn)
        # Serialise deletion against concurrent create / update / delete on the same store so the
        # read-then-pop sequence below is atomic (SF-4).
        with _SF_STORE_LOCK:
            state_machines = self.get_store(context).state_machines
    ...
    def update_state_machine(
        ...
        **kwargs,
        ...
        self._validate_state_machine_arn(state_machine_arn)
        # All reads/mutations of the state_machines store for this update run under the store
        # lock so that concurrent updates / deletes cannot interleave (SF-4).
        with _SF_STORE_LOCK:
            state_machines = self.get_store(context).state_machines
