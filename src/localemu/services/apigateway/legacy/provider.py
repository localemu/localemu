...
from localemu.aws.api import CommonServiceException, RequestContext, ServiceRequest, handler
from localemu.aws.api.apigateway import (
    Account,
    ApigatewayApi,
    ApiKey,
    ApiKeys,
...
from localemu.services.apigateway.exporter import OpenApiExporter
from localemu.services.apigateway.helpers import (
    EMPTY_MODEL,
    ERROR_MODEL,
...
from localemu.services.apigateway.legacy.helpers import multi_value_dict_for_list
from localemu.services.apigateway.legacy.invocations import invoke_rest_api_from_request
from localemu.services.apigateway.legacy.router_asf import ApigatewayRouter, to_invocation_context
from localemu.services.apigateway.models import (
    ApiGatewayStore,
    RestApiContainer,
    apigateway_stores,
)
from localemu.services.apigateway.next_gen.execute_api.router import (
    ApiGatewayRouter as ApiGatewayRouterNextGen,
)
from localemu.services.apigateway.patches import apply_patches
...
from localemu.utils.aws.arns import InvalidArnException, get_partition, parse_arn
from localemu.utils.collections import (
    DelSafeDict,
    PaginatedList,
    ensure_list,
    select_from_typed_dict,
)
from localemu.utils.json import parse_json_or_yaml
from localemu.utils.strings import md5, short_uid, str_to_bool, to_bytes, to_str
from localemu.utils.time import TIMESTAMP_FORMAT_TZ, now_utc, timestamp

LOG = logging.getLogger(__name__)

# Store VPC_LINK connection_ids in a proper dict keyed by (rest_api_id, resource_id, http_method)
# instead of monkey-patching dynamic attributes on moto model objects.
_integration_connection_ids: dict[tuple[str, str, str], str] = {}
...
class ApigatewayProvider(ApigatewayApi, ServiceLifecycleHook):
    ...
    def test_invoke_method(
        ...
        return TestInvokeMethodResponse(
            ...
            log=log,
            latency=int((req_end_time - req_start_time).total_seconds() * 1000),
    ...
    def put_method(
        ...
        **kwargs,
        ...
        if need_authorizer_id and (
            ...
            # TODO: will be cleaner with https://github.com/localemu/localemu/pull/7750
            raise BadRequestException(
    ...
    def _generate_self_signed_pem() -> str:
        """Generate a self-signed PEM certificate for client certificate emulation."""
        try:
            ...
            subject = issuer = x509.Name([
                x509.NameAttribute(NameOID.COMMON_NAME, "localemu.cloud"),
                x509.NameAttribute(NameOID.ORGANIZATION_NAME, "LocalEmu"),
            ])
            ...
            LOG.debug("cryptography library not available, using placeholder certificate")
            return "-----BEGIN CERTIFICATE-----\nMIIBkTCB+wIJALRiMLAh...(localemu-placeholder)\n-----END CERTIFICATE-----"
    ...
    def generate_client_certificate(
        ...
        **kwargs,
        ...
        pem_certificate = self._generate_self_signed_pem()
        entry = {
            "description": description,
            "tags": tags,
            "clientCertificateId": cert_id,
            "createdDate": creation_time,
            "expirationDate": creation_time + 60 * 60 * 24 * 365,  # 365 days validity
            "pemEncodedCertificate": pem_certificate,
        }
    ...
    def get_integration(
        ...
        **kwargs,
        ...
        if response.get("connectionType") == "VPC_LINK":
            # Look up the connection_id from our proper dict store instead of moto model attributes
    ...
    def put_integration(
        ...
        # TODO: moto does not save the connection_id
            ...
            # store the connection id in a proper dict instead of monkey-patching moto objects
            _key = (request.get("restApiId"), request.get("resourceId"), request.get("httpMethod"))
            _integration_connection_ids[_key] = connection_id
            response["connectionId"] = connection_id
