...
from localemu.constants import (
    ...
    HEADER_LOCALEMU_EDGE_URL,
    PATH_USER_REQUEST,
)
from localemu.services.apigateway.helpers import REQUEST_TIME_DATE_FORMAT
from localemu.services.apigateway.legacy.context import ApiInvocationContext
from localemu.utils import common
from localemu.utils.aws import resources as resource_utils
from localemu.utils.aws.arns import get_partition, parse_arn
from localemu.utils.aws.aws_responses import requests_error_response_json, requests_response
from localemu.utils.json import try_json
from localemu.utils.numbers import is_number
from localemu.utils.strings import canonicalize_bool_to_str, long_uid, to_str
...
class ResponseParametersResolver:
    def resolve(self, context: ApiInvocationContext) -> dict[str, str]:
        ...
        # Use the actual response status code instead of always defaulting to "200"
        actual_status = str(getattr(context.response, "status_code", 200))
        response_params = integration_responses.get(actual_status, {}).get("responseParameters", {})
        if not response_params:
            # Fall back to "200" for backwards compatibility if actual status not found
...
def get_cors_response(headers, gateway_responses=None):
    """Build CORS response using configured gateway response parameters if available,
    otherwise fall back to restrictive defaults.
    See: http://docs.aws.amazon.com/apigateway/latest/developerguide/how-to-cors.html
    """
    response = Response()
    response.status_code = 200

    # Try to extract CORS settings from the gateway response configuration
    cors_origin = None
    cors_methods = None
    cors_headers = None
    if gateway_responses:
        # Look for DEFAULT_4XX or DEFAULT_5XX responses that may have CORS response parameters
        for resp_type in gateway_responses.values():
            resp_params = resp_type.get("responseParameters", {})
            for param_key, param_value in resp_params.items():
                param_key_lower = param_key.lower()
                if "access-control-allow-origin" in param_key_lower:
                    cors_origin = param_value.strip("'\"")
                elif "access-control-allow-methods" in param_key_lower:
                    cors_methods = param_value.strip("'\"")
                elif "access-control-allow-headers" in param_key_lower:
                    cors_headers = param_value.strip("'\"")

    request_origin = headers.get("Origin", "")
    if cors_origin:
        # Use configured origin, or echo back Origin if configured value is the request origin
        response.headers["Access-Control-Allow-Origin"] = cors_origin
    elif request_origin:
        # If no explicit config, echo back the request Origin (safer than *)
        response.headers["Access-Control-Allow-Origin"] = request_origin
    else:
        response.headers["Access-Control-Allow-Origin"] = ""

    response.headers["Access-Control-Allow-Methods"] = (
        cors_methods or "GET, POST, PUT, DELETE, PATCH, OPTIONS"
    )
    response.headers["Access-Control-Allow-Headers"] = (
        cors_headers
        or "Content-Type, X-Amz-Date, Authorization, X-Api-Key, X-Amz-Security-Token"
    )
    response._content = ""
    return response
...
def set_api_id_stage_invocation_path(
    ...
    host_header = headers.get(HEADER_LOCALEMU_EDGE_URL, "") or headers.get("Host") or ""
...
def extract_api_id_from_hostname_in_url(hostname: str) -> str:
    """Extract API ID 'id123' from URLs like https://id123.execute-api.localhost:4566"""
