...
from localemu.services.apigateway.legacy.integration import (
...
from localemu.services.apigateway.models import ApiGatewayStore
from localemu.utils.aws.arns import ARN_PARTITION_REGEX
from localemu.utils.aws.aws_responses import requests_response
    ...
    def _get_missing_required_parameters(self, resource) -> list[str]:
        ...
        for request_parameter, required in sorted(request_parameters.items()):
            ...
            match param_type:
                ...
                case "path":
                    is_missing = param_value not in (self.context.path_params or {})
                case "querystring":
    ...
    for usage_plan_id in usage_plan_ids:
        ...
        for key in usage_plan_keys.get("items", []):
            if key.get("value") == api_key:
                ...
                api_key_details = client.get_api_key(apiKey=key.get("id"))
                return api_key_details.get("enabled") in ("true", True)
...
_MAX_PAYLOAD_SIZE_BYTES = 10 * 1024 * 1024  # AWS API Gateway 10 MB limit
...
def invoke_rest_api(invocation_context: ApiInvocationContext):
    ...
    # Enforce AWS API Gateway 10 MB request payload size limit
    payload_size = len(invocation_context.data or b"")
    if payload_size > _MAX_PAYLOAD_SIZE_BYTES:
        return requests_response(
            {"message": "Request body must be less than 10485760 bytes"}, status_code=413
        )
    ...
    if not method_integration:
        ...
        # AWS returns 403 with "Missing Authentication Token" when the path exists but the method
        # is not configured, matching real AWS behavior
        return requests_response(
            {"message": "Missing Authentication Token"}, status_code=403
        )
