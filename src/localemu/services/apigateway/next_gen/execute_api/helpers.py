...
from localemu.services.apigateway.models import MergedRestApi, RestApiContainer, RestApiDeployment
...
def freeze_rest_api(
    account_id: str, region: str, moto_rest_api: MotoRestAPI, localemu_rest_api: RestApiContainer
) -> RestApiDeployment:
    """
    Snapshot a REST API in time to create a deployment
    This will merge the Moto and LocalEmu data into one `MergedRestApi`
    """
    moto_resources = get_resources_from_moto_rest_api(moto_rest_api)

    rest_api = MergedRestApi.from_rest_api_container(
        rest_api_container=localemu_rest_api,
...
# AWS stage variable values must match this pattern (alphanumeric, hyphens, underscores, periods, slashes, colons)
_STAGE_VAR_VALUE_PATTERN = re.compile(r"^[a-zA-Z0-9._\-:/]*$")


def _sanitize_stage_variable_value(value: str) -> str:
    """Sanitize stage variable values to prevent injection when rendered into URIs.
    AWS restricts stage variable values to alphanumeric characters and a limited set of
    special characters. Values that do not match are rejected.
    """
    if _STAGE_VAR_VALUE_PATTERN.match(value):
        return value
    LOG.warning("Stage variable value contains invalid characters, sanitizing: %s", value)
    # Strip any characters that are not in the allowed set
...
def render_uri_with_stage_variables(
    ...
    def replace_match(match_obj: re.Match) -> str:
        value = stage_vars.get(match_obj.group("varName"), "")
        return _sanitize_stage_variable_value(value)
