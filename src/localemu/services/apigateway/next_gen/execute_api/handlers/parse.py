...
from localemu.utils.time import timestamp

from ..api import RestApiGatewayHandler, RestApiGatewayHandlerChain
from ..context import InvocationRequest, RestApiInvocationContext
from ..gateway_response import RequestTooLargeError
...
# AWS API Gateway enforces a 10 MB request payload limit
...
class InvocationRequestParser(RestApiGatewayHandler):
    def __call__(
        ...
        # Enforce AWS API Gateway 10 MB payload size limit
        content_length = context.request.content_length or 0
        if content_length > _MAX_PAYLOAD_SIZE_BYTES:
            raise RequestTooLargeError("Request body must be less than 10485760 bytes")
    ...
    def _enrich_with_raw_path(
        ...
        # if the request comes from the LocalEmu only `_user_request_` route, we need to remove this prefix from the
    ...
    def create_context_variables(context: RestApiInvocationContext) -> ContextVariables:
        ...
        context_variables = ContextVariables(
            ...
            identity=ContextVarsIdentity(
                ...
                principalOrgId=None,
                sourceIp=context.request.remote_addr or "127.0.0.1",
                user=None,
