...
from localemu.aws.api.apigateway import Stage
from localemu.constants import APPLICATION_JSON, AWS_REGION_US_EAST_1, DEFAULT_AWS_ACCOUNT_ID
from localemu.deprecations import deprecated_endpoint
from localemu.http import Response
from localemu.services.apigateway.models import ApiGatewayStore, apigateway_stores
...
class ApiGatewayEndpoint:
    ...
    def create_response(request: Request) -> Response:
        ...
        if not (connection := request.headers.get("Connection")) or connection != "close":
            ...
            # There appears to be in issue in Localemu, where setting "close" will result in "close, close"
...
class ApiGatewayRouter:
    ...
    def register_routes(self) -> None:
        ...
        rules = [
            ...
            # add the deprecated localemu-specific _user_request_ routes
            self.router.add(
            ...
            # add the localemu-specific so-called "path-style" routes when DNS resolving is not possible
            self.router.add(
