...
class RestApiMockIntegration(RestApiIntegration):
    ...
    def parse_invalid_json(self, body: str) -> dict:
        ...
        try:
            ...
            if not match:
                raise ValueError("Body does not match expected pattern: {key: value, ...}")
            ...
            for key_value in key_values:
                ...
                leading_key_char = key[0]
                if leading_key_char in "[{":
                    raise ValueError(f"Invalid leading character in key: {leading_key_char}")
                if leading_key_char in "'\"":
                    if len(key) < 2 or key[-1] != leading_key_char:
                        raise ValueError(f"Mismatched quotes in key: {key}")

                if len(value) >= 2 and (leading_value_char := value[0]) in "[{'\"":
                    if leading_value_char == "{":
                        # TODO reparse objects
                        if value[-1] != "}":
                            raise ValueError(f"Mismatched braces in value: {value}")
                    elif leading_value_char == "[":
                        # TODO validate arrays
                        if value[-1] != "]":
                            raise ValueError(f"Mismatched brackets in value: {value}")
                    else:
                        if value[-1] != leading_value_char:
                            raise ValueError(f"Mismatched quotes in value: {value}")
