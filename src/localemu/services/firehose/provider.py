...
from localemu.aws.api.firehose import (
...
from localemu.services.firehose.mappers import (
...
from localemu.services.firehose.models import FirehoseStore, firehose_stores
...
from localemu.utils.common import (
...
from localemu.utils.run import run_for_max_seconds
from localemu.utils.tagging import tag_list_to_map, tag_map_to_list
...
def get_search_db_connection(endpoint: str, region_name: str):
    ...
    if "https://" in endpoint:
        use_ssl = True
        # TODO remove this condition once ssl certs are available for .es.localhost domains
        endpoint_netloc = urlparse(endpoint).netloc
        if not re.match(r"^.*(localhost(\.localemu\.cloud)?)(:\d+)?$", endpoint_netloc):
            verify_certs = True
...
class FirehoseProvider(FirehoseApi):
    ...
    def __init__(self) -> None:
        ...
        # cache of OpenSearch/Elasticsearch clients keyed by (endpoint, region)
        # to avoid rebuilding the client (and re-doing AWS4Auth signing) on
        # every batch delivery.
        self._search_db_connections: dict[tuple[str, str], object] = {}
        self._search_db_connections_lock = threading.RLock()

    def _get_search_db_connection(self, endpoint: str, region_name: str):
        key = (endpoint, region_name)
        client = self._search_db_connections.get(key)
        if client is not None:
            return client
        with self._search_db_connections_lock:
            client = self._search_db_connections.get(key)
            if client is None:
                client = get_search_db_connection(endpoint, region_name)
                self._search_db_connections[key] = client
            return client
    ...
    def list_delivery_streams(
        ...
        **kwargs,
        ...
        store = self.get_store(context.account_id, context.region)
        # Apply the optional DeliveryStreamType filter
        all_names: list[str] = []
        for _, stream in store.delivery_streams.items():
            if delivery_stream_type and stream.get("DeliveryStreamType") != delivery_stream_type:
                continue
            all_names.append(stream["DeliveryStreamName"])
        # Stable ordering for deterministic pagination
        all_names.sort()
        # Apply ExclusiveStartDeliveryStreamName
        if exclusive_start_delivery_stream_name:
            try:
                start_idx = all_names.index(exclusive_start_delivery_stream_name) + 1
            except ValueError:
                start_idx = 0
            all_names = all_names[start_idx:]
        # Apply Limit (AWS default 10, max 10000)
        page_size = limit if limit else len(all_names)
        page = all_names[:page_size]
        has_more = len(all_names) > len(page)
        return ListDeliveryStreamsOutput(
            DeliveryStreamNames=page, HasMoreDeliveryStreams=has_more
        )
    ...
    def _put_records(
        ...
        for destination in delivery_stream_description.get("Destinations", []):
            ...
            if (
                "S3DestinationDescription" in destination
                or "ExtendedS3DestinationDescription" in destination
            ):
                s3_dest_desc = destination.get(
                    "S3DestinationDescription"
                ) or destination.get("ExtendedS3DestinationDescription")
                if s3_dest_desc:
            ...
            if "HttpEndpointDestinationDescription" in destination:
                ...
                try:
                    # AWS Firehose HTTP endpoint delivery has a per-request
                    # timeout (configurable, default 60s) — mirror that here so
                    # a hung endpoint cannot block the handler indefinitely.
    ...
    def _put_to_search_db(
        ...
        db_connection = self._get_search_db_connection(cluster_endpoint, region_name)
    ...
    def _preprocess_records(self, processor: dict, records: list[Record]) -> list[dict]:
        ...
        if proc_type == "Lambda":
            ...
            # AWS sets FunctionError=Handled|Unhandled when the Lambda fails.
            # Previously we ignored it and silently dropped the batch; surface
            # the error instead so operators can diagnose data loss.
            function_error = response.get("FunctionError")
            if function_error:
                LOG.error(
                    "Firehose preprocessor Lambda %s returned FunctionError=%s payload=%s; aborting batch.",
                    lambda_arn,
                    function_error,
                    truncate(str(result), max_length=300),
                )
                raise Exception(
                    f"Firehose preprocessor Lambda {lambda_arn} failed with FunctionError={function_error}"
                )
    ...
    def _put_records_to_s3_bucket(
        ...
        batched_data = b"".join(
            [base64.b64decode(data) for r in records if (data := r.get("Data") or r.get("data"))]
        )
    ...
    def _prepare_records_for_redshift(self, record: dict) -> list[dict]:
        ...
        for key, value in data.items():
            ...
            # required to work with execute_statement in community (moto) and ext (localemu native)
