...
from localemu.aws.forwarder import (
    ForwardingFallbackDispatcher,
    create_aws_request_context,
    dispatch_to_backend,
)
from localemu.aws.skeleton import DispatchTable
from localemu.constants import DEFAULT_AWS_ACCOUNT_ID
from localemu.constants import VERSION as LOCALEMU_VERSION
from localemu.http import Response
from localemu.http.request import Request, get_full_raw_path, get_raw_current_url
...
def MotoOnlyDispatcher(service_model) -> DispatchTable:
    """
    Creates a dispatch table that routes ALL operations for a service directly to Moto.
    Used for services where Moto has a full implementation and no custom provider is needed.

    :param service_model: a botocore ServiceModel
    :return: a DispatchTable mapping every operation to the Moto proxy
    """
