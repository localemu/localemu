...
from localemu.aws.api.cloudwatch import MetricData, MetricDataQuery, ScanBy
...
class CloudwatchDatabase:
    ...
    def _ensure_ready(self) -> None:
        ...
        Originally this ran once in ``__init__`` and early-returned if
        the file existed. In practice the metrics directory can get
        pruned between the first call and subsequent ``PutMetricData``
        calls (state resets, persistence cleanup, fresh volume mounts).
        When that happens, ``sqlite3.connect`` fails with the misleading
        ``unable to open database file`` because the parent dir is gone.

        Solution: every writer re-checks the directory and creates
        tables with IF NOT EXISTS. Cheap when the DB is healthy, fixes
        the rare "disappeared underneath me" case automatically.
        """
        ...
        with self.DATABASE_LOCK, sqlite3.connect(self.METRICS_DB) as conn:
            ...
            cur.execute(
                f"""
                CREATE TABLE IF NOT EXISTS "{self.TABLE_SINGLE_METRICS}" (
                    {common_columns},
                    "value"	                NUMERIC,
                    PRIMARY KEY("id")
                );
            ...
            cur.execute(
                f"""
                CREATE TABLE IF NOT EXISTS "{self.TABLE_AGGREGATED_METRICS}" (
                    {common_columns},
            ...
            cur.executescript(
                """
                CREATE INDEX IF NOT EXISTS idx_single_metrics_comp
                    ON SINGLE_METRICS (metric_name, namespace);
                CREATE INDEX IF NOT EXISTS idx_aggregated_metrics_comp
                    ON AGGREGATED_METRICS (metric_name, namespace);
                """
            )
            conn.commit()
    ...
    def add_metric_data(
        ...
        # Re-verify the DB is usable on every write. Cheap when healthy
        # (pure check of an existing directory + IF NOT EXISTS DDL on an
        # already-created schema). Fixes the rare case where the parent
        # directory was pruned between the singleton init and this call.
        self._ensure_ready()
