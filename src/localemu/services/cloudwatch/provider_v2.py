...
from localemu.services.cloudwatch.alarm_scheduler import AlarmScheduler
from localemu.services.cloudwatch.cloudwatch_database_helper import CloudwatchDatabase
from localemu.services.cloudwatch.models import (
    CloudWatchStore,
    LocalEmuAlarm,
    LocalEmuCompositeAlarm,
    LocalEmuDashboard,
    LocalEmuMetricAlarm,
    cloudwatch_stores,
)
from localemu.services.edge import ROUTER
from localemu.services.plugins import SERVICE_PLUGINS, ServiceLifecycleHook
from localemu.state import AssetDirectory, StateVisitor
from localemu.utils.aws import arns
from localemu.utils.aws.arns import extract_account_id_from_arn, lambda_function_name
from localemu.utils.collections import PaginatedList
from localemu.utils.json import CustomEncoder as JSONEncoder
from localemu.utils.strings import camel_to_snake_case
...
class CloudwatchProvider(CloudwatchApi, ServiceLifecycleHook):
    ...
    def get_raw_metrics(self, request: Request):
        """this feature was introduced with https://github.com/localemu/localemu/pull/3535
    ...
    def put_metric_alarm(self, context: RequestContext, request: PutMetricAlarmInput) -> None:
        ...
        with _STORE_LOCK:
            store = self.get_store(context.account_id, context.region)
            metric_alarm = LocalEmuMetricAlarm(context.account_id, context.region, {**request})
    ...
    def put_composite_alarm(self, context: RequestContext, request: PutCompositeAlarmInput) -> None:
        with _STORE_LOCK:
            store = self.get_store(context.account_id, context.region)
            composite_alarm = LocalEmuCompositeAlarm(
    ...
    def describe_alarms_for_metric(
        ...
        **kwargs,
        ...
        alarms = [
            ...
            if isinstance(a, LocalEmuMetricAlarm)
    ...
    def put_dashboard(
        ...
        **kwargs,
        ...
        store = self.get_store(context.account_id, context.region)
        store.dashboards[dashboard_name] = LocalEmuDashboard(
    ...
    def _update_state(
        ...
        if state_reason_data and isinstance(alarm, LocalEmuMetricAlarm):
        ...
        alarm_history_item = AlarmHistoryItem(
            ...
            if isinstance(alarm, LocalEmuMetricAlarm)
            else "CompositeAlarm",
        )
    ...
    def _evaluate_composite_alarms(self, context: RequestContext, triggering_alarm):
        ...
        store = self.get_store(context.account_id, context.region)
        alarms = list(store.alarms.values())
        composite_alarms = [a for a in alarms if isinstance(a, LocalEmuCompositeAlarm)]
        ...
        for alarm_condition in alarms_conditions:
            if not alarm_condition.startswith("ALARM"):
                validation_result.append(
                    f"Unsupported expression in alarm rule condition {alarm_condition}: Only ALARM expression is supported by Localemu as of now"
                )
...
def create_metric_data_query_from_alarm(alarm: LocalEmuMetricAlarm):
...
def create_message_response_update_state_lambda(
    alarm: LocalEmuMetricAlarm, old_state, old_state_reason, old_state_timestamp
):
    _alarm = alarm.alarm
    response = {
...
def create_message_response_update_state_sns(alarm: LocalEmuMetricAlarm, old_state: StateValue):
    _alarm = alarm.alarm
    response = {
...
def create_message_response_update_composite_alarm_state_sns(
    composite_alarm: LocalEmuCompositeAlarm,
    triggering_alarm: LocalEmuMetricAlarm,
