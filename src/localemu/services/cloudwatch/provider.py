...
def _to_iso_8601_datetime_with_nanoseconds(date: datetime | None) -> str | None:
    if date is None:
        return None
    # Python's datetime only holds microsecond precision. Express the value as
    # microseconds; any sub-microsecond ("nanosecond") component we don't track
    # is explicitly padded with zeros to preserve the AWS wire shape.
    return date.strftime("%Y-%m-%dT%H:%M:%S.%f") + "000Z"
...
class CloudwatchProvider(CloudwatchApi, ServiceLifecycleHook):
    ...
    def put_metric_alarm(
        ...
        if tags:
            self.tags.tag_resource(arn, tags)
