...
from localemu.aws.api.dynamodbstreams import (
...
from localemu.services.dynamodb.utils import change_region_in_ddb_stream_arn
from localemu.services.dynamodbstreams.dynamodbstreams_api import (
...
class DynamoDBStreamsProvider(DynamodbstreamsApi, ServiceLifecycleHook):
    ...
    def __init__(self):
        self.shard_to_region = collections.OrderedDict()
        self._shard_to_region_lock = threading.Lock()
    ...
    def describe_stream(
        ...
        **kwargs,
        ...
        # Extract the table name from the requested stream ARN so we can
        # match even when the stream label (timestamp) differs between
        # what DynamoDB reports and what was stored at creation time.
        requested_table = table_name_from_stream_arn(stream_arn)
        for stream in store.ddb_streams.values():
            ...
            stored_table = table_name_from_stream_arn(stream_description["StreamArn"])
            # Prefer an exact ARN match. Only fall back to a table-name match
            # when the stream is still ACTIVE (i.e. not being torn down) to
            # avoid returning a stale stream record that happens to share the
            # same table name as the requested one.
            stream_is_current = stream_description.get("StreamStatus") not in (
                StreamStatus.DISABLED,
                StreamStatus.DISABLING,
            )
            if stream_description["StreamArn"] == _stream_arn or (
                stored_table == requested_table and stream_is_current
            ):
                # get stream details
                dynamodb = connect_to(
    ...
    def get_shard_iterator(
        ...
        **kwargs,
        ...
        with self._shard_to_region_lock:
            ...
            # Always enforce the cache cap on every iterator creation, so
            # entries from earlier cross-region calls cannot grow unbounded
            # even when the current call is same-region.
            while len(self.shard_to_region) > self._MAX_SHARD_ENTRIES:
                self.shard_to_region.popitem(last=False)
    ...
    def list_streams(
        ...
        **kwargs,
        ...
        if table_name:
            ...
            # If this is a stream from a table replica, we need to change the region in the stream ARN, as LocalEmu
        ...
        if exclusive_start_stream_arn:
            start_idx = 0
            for i, s in enumerate(result):
                if s.get("StreamArn") == exclusive_start_stream_arn:
                    start_idx = i + 1
                    break
            result = result[start_idx:]

        # Apply Limit (AWS default is 100, hard-cap to client-provided limit)
        last_evaluated_arn = None
        if limit and len(result) > limit:
            result = result[:limit]
            last_evaluated_arn = result[-1].get("StreamArn") if result else None

        output: ListStreamsOutput = ListStreamsOutput(Streams=result)
        if last_evaluated_arn:
            output["LastEvaluatedStreamArn"] = last_evaluated_arn
        return output
