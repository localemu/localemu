...
# Lazy import to avoid circular dependency (dynamodb.provider ↔ dynamodbstreams)
# DynamoDBProvider is used only in get_original_region() at the bottom of this file
from localemu.services.dynamodbstreams.models import (
    DynamoDbStreamsStore,
    StreamWrapper,
    dynamodbstreams_stores,
)
from localemu.utils.aws import arns, resources
from localemu.utils.common import now_utc
...
def _process_forwarded_records(
    ...
    if stream_type.is_kinesis and stream_type.stream_view_type != StreamViewType.NEW_AND_OLD_IMAGES:
        ...
        for record in records:
            ...
            kinesis_records.append({"Data": dumps(record), "PartitionKey": table_name or "default"})
