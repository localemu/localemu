...
from localemu.aws.api.dynamodb import (
...
from localemu.aws.api.dynamodbstreams import StreamStatus
from localemu.aws.connect import connect_to
from localemu.config import is_persistence_enabled
from localemu.constants import (
    AUTH_CREDENTIAL_REGEX,
    AWS_REGION_US_EAST_1,
    INTERNAL_AWS_SECRET_ACCESS_KEY,
)
from localemu.http import Request, Response, route
from localemu.services.dynamodb.models import (
    DynamoDBStore,
    RecordsMap,
    StreamRecord,
    StreamRecords,
...
from localemu.services.dynamodb.utils import (
    ItemFinder,
    ItemSet,
    SchemaExtractor,
    de_dynamize_record,
...
from localemu.services.dynamodbstreams import dynamodbstreams_api
from localemu.services.dynamodbstreams.models import dynamodbstreams_stores
...
from localemu.utils.aws.aws_stack import get_valid_regions_for_service
from localemu.utils.aws.request_context import (
    extract_account_id_from_headers,
    extract_region_from_headers,
)
from localemu.utils.collections import select_attributes, select_from_typed_dict
from localemu.utils.common import short_uid, to_bytes
from localemu.utils.files import cp_r, rm_rf
from localemu.utils.json import BytesEncoder, canonical_json
from localemu.utils.scheduler import Scheduler
from localemu.utils.strings import long_uid, md5, to_str
from localemu.utils.threads import FuncThread, start_thread
...
MANAGED_KMS_KEYS: dict[tuple[str, str], str] = {}  # (account_id, region) → key_id
MANAGED_KMS_KEYS_LOCK = threading.Lock()
...
class EventForwarder:
        ...
        for table_name, table_records in records_map.items():
            ...
            for record in records:
                ...
                if hash_keys:
                    ...
                    kinesis_partition_key = md5(table_name)
    ...
    def is_kinesis_stream_exists(cls, stream_arn):
        ...
        while has_more:
            kwargs = {}
            if exclusive_start:
                kwargs["ExclusiveStartStreamName"] = exclusive_start
            response = kinesis.list_streams(**kwargs)
            stream_names = response.get("StreamNames", [])
            if stream_name_from_arn in stream_names:
                return True
...
class SSEUtils:
    ...
    def get_sse_kms_managed_key(cls, account_id: str, region_name: str):
        from localemu.services.kms import provider

        cache_key = (account_id, region_name)
        with MANAGED_KMS_KEYS_LOCK:
            existing_key = MANAGED_KMS_KEYS.get(cache_key)
            if existing_key:
                return existing_key
        ...
        with MANAGED_KMS_KEYS_LOCK:
            MANAGED_KMS_KEYS[cache_key] = key_id
        return key_id
...
_EXPR_ATTR_REF = re.compile(r"(?<![A-Za-z0-9_:#])([#:]?[A-Za-z_][A-Za-z0-9_]*)")

# Expression reserved words that must be allowed through. They look like attribute
# references in the regex but are operators. Match is case-insensitive.
...
def _referenced_attributes(expr: str, expr_attr_names: dict | None) -> set[str]:
    """Return the set of actual attribute names referenced in a DynamoDB expression,
    resolving any ``#alias`` placeholders through ``ExpressionAttributeNames``.
    """
    if not expr:
        return set()
    names = expr_attr_names or {}
    out = set()
    for token in _EXPR_ATTR_REF.findall(expr):
        if token.startswith(":"):
            # value placeholder, not an attribute ref
            continue
        if token.lower() in _EXPR_RESERVED:
            continue
        resolved = names.get(token, token) if token.startswith("#") else token
        out.add(resolved)
    return out


def _validate_filter_expression_not_using_keys(
    filter_expr: str | None,
    expr_attr_names: dict | None,
    key_attr_names: set[str],
) -> None:
    """Reject FilterExpression references to primary-key attributes, matching real-AWS
    DynamoDB behaviour. A filter expression can only reference non-key attributes; key
    attributes belong in the KeyConditionExpression.
    """
    if not filter_expr or not key_attr_names:
        return
    for attr in _referenced_attributes(filter_expr, expr_attr_names):
        if attr in key_attr_names:
            raise ValidationException(
                "Filter Expression can only contain non-primary key attributes: "
                f"Primary key attribute: {attr}"
            )


def _table_key_attrs(
    account_id: str, region: str, table_name: str, index_name: str | None = None
) -> set[str]:
    """Return the attribute names that make up the table's primary key, or the key of
    the named GSI/LSI when ``index_name`` is provided.
    """
    try:
        schema = SchemaExtractor.get_table_schema(table_name, account_id, region)
    except Exception:
        return set()
    table = (schema or {}).get("Table", {})
    if index_name:
        for idx_group in ("GlobalSecondaryIndexes", "LocalSecondaryIndexes"):
            for idx in table.get(idx_group) or []:
                if idx.get("IndexName") == index_name:
                    return {k["AttributeName"] for k in idx.get("KeySchema", [])}
        return set()
    return {k["AttributeName"] for k in table.get("KeySchema", [])}
            ...
            if ttl_spec.get("Enabled", False):
                ...
                try:
                    ...
                    while True:
                        ...
                        items_to_delete.extend(result.get("Items", []))
...
class DynamoDBProvider(DynamodbApi, ServiceLifecycleHook):
    ...
    """Persistent state directory for snapshot save/load."""
    tmp_asset_directory = f"{config.dirs.tmp}/dynamodb"
    """Temporary directory used by the MANUAL snapshot strategy."""
    ...
    def on_before_state_save(self) -> None:
        ...
        # - awsemu dynamodb create-table table1
        # - curl -X POST http://localhost:4566/_localemu/state/save
        # - awsemu dynamodb create-table table2
    ...
    def on_after_init(self):
        # Fix up ARN account_id and region in responses that come back from Moto.
    ...
    def _forward_request(
        ...
        """
        Modify the context region and then forward the request to the Moto backend.

        Used for operations impacted by global tables. LocalEmu keeps a single copy
        of the global table state, and any requests to replicated tables are forwarded
    ...
    def _endpoint_url(self) -> str:
        """Return the DynamoDB endpoint URL for internal API calls."""
        from localemu import config
    ...
    def forward_request(
        ...
        """
        Forward a request to the Moto backend.

        Raising NotImplementedError here would cause MotoFallbackDispatcher to handle
        the call, which bypasses all post-processing in the calling handler method
        (stream record creation, ARN fixes, table definitions, etc.). So we call
        Moto directly.

        When the caller mutated the body (for example, PutItem and DeleteItem rewrite
        the request to add ``ReturnValues=ALL_OLD`` so the stream record builder can
        fill in ``OldImage``), we forward the mutated body. Without that, Moto would
        read ``context.request`` (the original) and silently return no Attributes,
        and DeleteItem would bail before emitting the REMOVE stream record.
        """
        from localemu.services.moto import call_moto, call_moto_with_request

        if service_request is not None:
            return call_moto_with_request(context, service_request)
    ...
    def delete_table(
        ...
        # This is not the case for LocalEmu, where all replicated tables will also be removed if source is deleted.
        ...
        store.table_properties.pop(table_name, None)
        store.TABLE_REGION.pop(table_name, None)
    ...
    def describe_table(
        ...
        # Update table properties from LocalEmu stores
    ...
    def update_table(
        ...
        try:
            ...
            # Moto refuses to update certain table params and raises ValidationException,
            # but we still need to update this info in the LocalEmu stores.
        ...
        for replicated_table, replications in replicas.items():
            for replica_region, replica_description in replications.items():
                if context.region == replica_region:
                    if replicated_table not in response["TableNames"]:
                        response["TableNames"].append(replicated_table)

        # Apply pagination to the full list including replicas
        table_names = sorted(response["TableNames"])
        if exclusive_start_table_name:
            try:
                idx = table_names.index(exclusive_start_table_name)
                table_names = table_names[idx + 1:]
            except ValueError:
                table_names = [t for t in table_names if t > exclusive_start_table_name]

        if limit and len(table_names) > limit:
            response["TableNames"] = table_names[:limit]
            response["LastEvaluatedTableName"] = table_names[limit - 1]
        else:
    ...
    def query(self, context: RequestContext, query_input: QueryInput) -> QueryOutput:
        ...
        if index_name:
            if not is_index_query_valid(context.account_id, context.region, query_input):
                raise ValidationException(
                    f"One or more parameter values were invalid: Select type ALL_ATTRIBUTES "
                    f"is not supported for global secondary index {index_name} because its projection "
                    f"type is not ALL",
                )

        table_name = query_input["TableName"]
        # Real DynamoDB rejects FilterExpression references to primary-key attributes
        # of the table (or of the queried index); those belong in KeyConditionExpression.
        _validate_filter_expression_not_using_keys(
            query_input.get("FilterExpression"),
            query_input.get("ExpressionAttributeNames"),
            _table_key_attrs(context.account_id, context.region, table_name, index_name),
        )
    ...
    def scan(self, context: RequestContext, scan_input: ScanInput) -> ScanOutput:
        ...
        _validate_filter_expression_not_using_keys(
            scan_input.get("FilterExpression"),
            scan_input.get("ExpressionAttributeNames"),
            _table_key_attrs(
                context.account_id, context.region, table_name, scan_input.get("IndexName")
            ),
        )
        global_table_region = self.get_global_table_region(context, table_name)
        result = self._forward_request(context=context, region=global_table_region)
    ...
    def transact_write_items(
        ...
        for item in transact_items:
            ...
            for key in ["Put", "Update", "Delete", "ConditionCheck"]:
                ...
                if inner_item:
                    # Extract the table name from the ARN; the backend expects plain names here.
                    ...
                    continue
        # Normalise the request shape for the Moto backend.
    ...
    def execute_transaction(
        ...
        for stmt_entry in transact_statements:
            ...
            if not table_name or table_name in table_existing_items:
                continue
            stream_type = get_table_stream_type(context.account_id, context.region, table_name)
            if stream_type:
                table_stream_types[table_name] = stream_type
                table_existing_items[table_name] = ItemFinder.list_existing_items_for_statement(
        ...
        # Forward stream records for each affected table
        for table_name, stream_type in table_stream_types.items():
            records = get_updated_records(
                account_id=context.account_id,
                region_name=context.region,
                table_name=table_name,
                existing_items=table_existing_items.get(table_name, []),
                server_url=self._endpoint_url,
    ...
    def execute_statement(
        ...
        if stream_type:
            ...
            # TODO: hook into PartiQL update execution to extract updated items without re-scanning.
        ...
        **kwargs,
        ...
        if next_token:
            try:
                ...
                offset = 0
        page_size = 100
        page = all_tags[offset: offset + page_size]
        result = ListTagsOfResourceOutput(Tags=page)
        if offset + page_size < len(all_tags):
            result["NextToken"] = str(offset + page_size)
        return result
    ...
    def list_global_tables(
        ...
        **kwargs,
        ...
        all_tables = sorted(
            ...
            key=lambda t: t.get("GlobalTableName", ""),
        )

        if exclusive_start_global_table_name:
            all_tables = [
                t for t in all_tables
                if t.get("GlobalTableName", "") > exclusive_start_global_table_name
            ]

        result = ListGlobalTablesOutput(GlobalTables=all_tables)
        if limit and len(all_tables) > limit:
            result["GlobalTables"] = all_tables[:limit]
            result["LastEvaluatedGlobalTableName"] = all_tables[limit - 1].get("GlobalTableName")
        return result
    ...
    def ensure_table_exists(
        ...
        :raise: ResourceNotFoundException if table does not exist
        """
    ...
    def get_global_table_region(context: RequestContext, table_name: str) -> str:
        ...
        Replication in LocalEmu works by keeping a single copy of a table and forwarding
    ...
    def fix_consumed_capacity(self, request: dict, result: dict):
        # Append ConsumedCapacity if Moto didn't (some endpoints leave it out).
    ...
    def fix_table_arn(self, account_id: str, region_name: str, arn: str) -> str:
        """
        Set the correct account ID and region in ARNs returned by the Moto backend.
        """
...
def get_updated_records(
    ...
    Note: This is currently a fairly expensive operation, as we need to retrieve the list of all items
          from the table and compare them against the previously available state. This is a limitation
          of treating the Moto backend as a blackbox. In future, hook into PartiQL query execution to
          extract the list of updated items directly.
    """
    result = []
