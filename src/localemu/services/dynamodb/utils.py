...
from localemu.utils.json import canonical_json
from localemu.utils.testutil import list_all_resources
...
def get_ddb_access_key(account_id: str, region_name: str) -> str:
    """
    Get the namespacing access key used for account+region isolation in the DynamoDB store.
...
class SchemaExtractor:
    ...
    def get_key_schema(
        ...
        from localemu.services.dynamodb.provider import get_store
...
class ItemFinder:
    ...
    def find_existing_item(
        ...
        from localemu.services.dynamodb.provider import ValidationException
        ...
        # Read straight from moto's backend rather than re-entering via HTTP.
        # Going through ``ddb_client.get_item`` requires synthesizing an
        # AccessKey with ``get_ddb_access_key`` (the DDB-Local namespacing
        # trick). Under the moto fallback that synthetic key resolves to
        # the wrong account and the GetItem returns ResourceNotFoundException
        # — UpdateItem / DeleteItem then bail before emitting stream
        # records (only INSERT-from-PutItem ever showed up in the stream).
        try:
            import moto.backends as moto_backends

            backend = moto_backends.get_backend("dynamodb")[account_id][region_name]
            table = backend.get_table(table_name)
            if table is None:
                return None
            # moto's Table exposes hash_key_attr / range_key_attr.
            hash_key_name = getattr(table, "hash_key_attr", None)
            range_key_name = getattr(table, "range_key_attr", None) or None
            if not hash_key_name:
                return None
            # moto's Table.get_item expects DynamoType instances, not raw
            # AttributeValue dicts. Wrap the search-key values.
            from moto.dynamodb.models.dynamo_type import DynamoType

            hash_raw = search_key.get(hash_key_name)
            if hash_raw is None:
                return None
            hash_val = DynamoType(hash_raw)
            range_val = (
                DynamoType(search_key[range_key_name])
                if range_key_name and search_key.get(range_key_name) is not None
                else None
            )
            item = (
                table.get_item(hash_val, range_val)
                if range_val is not None
                else table.get_item(hash_val)
            )
            if item is None:
                return None
            return item.to_json().get("Attributes") if hasattr(item, "to_json") else None
        except Exception as e:
            LOG.warning(
                "Unable to read item from DynamoDB table '%s' via moto backend: %s",
                table_name, e,
            )
            return None
...
def modify_ddblocal_arns(chain, context: RequestContext, response: Response):
    ...
    if response_content := response.get_data(as_text=True):
        ...
        # fix the table and latest stream ARNs (the backend uses a placeholder region internally)
