...
from localemu.utils.strings import base64_decode, long_uid, to_bytes, to_str
...
def guess_endpoint_strategy_and_host(
    ...
    For instance:
      ...
      * ``sqs.us-east-1.localhost`` -> ``standard, localhost``
      * ``queue.localhost:4566`` -> ``domain, localhost:4566``
