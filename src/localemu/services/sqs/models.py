...
from localemu.services.sqs.constants import DYNAMIC_ATTRIBUTES
from localemu.services.sqs.exceptions import (
    InvalidAttributeValue,
    InvalidParameterValueException,
    MissingRequiredParameterException,
)
from localemu.services.sqs.queue import InterruptiblePriorityQueue, InterruptibleQueue
...
def to_sqs_api_message(
    ...
    """
    Utility function to convert an SQS message from LocalEmu's internal representation to the AWS API
    concept 'Message', which is the format returned by the ``ReceiveMessage`` operation.

    :param standard_message: A LocalEmu SQS message
    ...
    # 4.1 fix: shallow copy + selective deep copy of mutable dicts only
    # (avoids expensive full deep copy for every ReceiveMessage on large payloads)
    message = dict(standard_message.message)
    message["Attributes"] = dict(message.get("Attributes", {}))
    if "MessageAttributes" in message:
        message["MessageAttributes"] = copy.deepcopy(message["MessageAttributes"])
...
class MessageMoveTask:
    ...
    def mark_started(self):
        self.started_timestamp = datetime.now(tz=__import__('datetime').timezone.utc)
...
class SqsQueue:
    ...
    def url(self, context: RequestContext) -> str:
        ...
        * (standard) http://sqs.eu-west-1.localhost:4566/000000000000/myqueue
        * (domain) http://eu-west-1.queue.localhost:4566/000000000000/myqueue
        * (path) http://localhost:4566/queue/eu-central-1/000000000000/myqueue
        * otherwise: http://localhost:4566/000000000000/myqueue
        """
        ...
        if endpoint_strategy == "standard":
            ...
            # sqs.us-east-1.localhost:4566/000000000000/my-queue
            scheme = context.request.scheme
            host_url = f"{scheme}://sqs.{self.region}.{host_and_port}"
        elif endpoint_strategy == "domain":
            # Legacy style
            # queue.localhost:4566/000000000000/my-queue (us-east-1)
            # or us-east-2.queue.localhost:4566/000000000000/my-queue
