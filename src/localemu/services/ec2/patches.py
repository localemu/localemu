...
def apply_patches():
    ...
    def ec2_create_vpc_endpoint(
        ...
        # by the individual services in LocalEmu we will be creating the following entries:
        # `<vpc-id>.<service>.<region>.vpce.<localemu host and port>`
        if service_name := kwargs.get("service_name"):
            ls_service_name = ".".join(service_name.split(".")[::-1]).replace(
                "amazonaws.com", f"vpce.{localemu_host()}"
            )
