...
class EC2VPCEndpointProvider(ResourceProvider[EC2VPCEndpointProperties]):
    ...
    def create(
        ...
        if not request.custom_context.get(REPEATED_INVOCATION):
            ...
            # Use the actual PrivateDnsEnabled value from the model,
            # defaulting to True for Interface endpoints and False for Gateway
            private_dns = model.get("PrivateDnsEnabled")
            if private_dns is None:
                private_dns = model["VpcEndpointType"] == "Interface"
            elif isinstance(private_dns, str):
                private_dns = private_dns.lower() in ("true", "1", "yes")
            model["PrivateDnsEnabled"] = bool(private_dns)
