...
from localemu.aws.api.s3 import Type as GranteeType
...
from localemu.services.s3 import checksums
...
from localemu.services.s3.headers import decode_header_rfc2047, encode_header_rfc2047
...
def get_owner_for_account_id(account_id: str):
    """
    This method returns the S3 Owner from the account id, generating a deterministic
    account-specific canonical ID by hashing the account ID with SHA-256, matching the
    64 hex-char format of real AWS canonical user IDs.
    See https://docs.aws.amazon.com/AmazonS3/latest/API/API_Owner.html
    :param account_id: the owner account id
    :return: the Owner object containing the DisplayName and owner ID
    """
    canonical_id = hashlib.sha256(account_id.encode("utf-8")).hexdigest()
    return Owner(
        ID=canonical_id,
    )
...
def get_full_default_bucket_location(bucket_name: BucketName) -> str:
    ...
    if host_definition.host != constants.LOCALHOST_HOSTNAME:
        # the user has customised their LocalEmu hostname, and may not support subdomains.
...
def is_valid_canonical_id(canonical_id: str) -> bool:
    ...
    All-zero IDs are valid canonical IDs.
    """
    if len(canonical_id) != 64:
        return False
    try:
        int(canonical_id, 16)
        return True
...
def uses_host_addressing(headers: Mapping[str, str]) -> str | None:
    ...
    # Standard virtual-host: bucket.s3.region.amazonaws.com or bucket.s3.localhost:4566
    # The .s3. pattern is the reliable detection method. Go SDK v2 style (bucket.localhost:4566)
    # is NOT detected here - clients must use s3_use_path_style=true (Terraform) or
    # Config(s3={"addressing_style": "path"}) (boto3) for local endpoints.
    if ".s3." in host and (
