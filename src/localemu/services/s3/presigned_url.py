...
from localemu.aws.protocol.op_router import RestServiceOperationRouter
from localemu.aws.spec import get_service_catalog
from localemu.constants import AWS_REGION_US_EAST_1
from localemu.http import Request, Response
from localemu.http.request import get_raw_path
...
def get_credentials_from_parameters(parameters: dict, region: str) -> PreSignedCredentials:
    ...
    if not access_key_id:
        # In strict mode reject instead of falling back to "test"
        if config.S3_STRICT_PRESIGNED_VALIDATION:
            raise SignatureDoesNotMatch(
                "Missing access key in presigned URL and S3_STRICT_PRESIGNED_VALIDATION is enabled"
            )
    ...
    if access_key_id in (constants.INTERNAL_AWS_ACCESS_KEY_ID, config.INTERNAL_RESOURCE_ACCOUNT):
        ...
        # Reject unregistered keys in strict mode
        if config.S3_STRICT_PRESIGNED_VALIDATION:
            raise SignatureDoesNotMatch(
                f"Access key {access_key_id!r} is not registered and "
                "S3_STRICT_PRESIGNED_VALIDATION is enabled"
            )
        # if we could not retrieve the secret access key, it means the access key was not registered in LocalEmu,
...
def _find_valid_signature_through_ports(context: RequestContext) -> FindSigV4Result:
    """
    Tries to validate the signature of the received request. If it fails, it will iterate through known LocalEmu
