...
from localemu.aws.handlers.cors import CorsEnforcer, CorsResponseEnricher
...
from localemu.services.s3.constants import S3_HOST_ID
from localemu.services.s3.utils import S3_VIRTUAL_HOSTNAME_REGEX
...
class S3CorsHandler(Handler):
    ...
    def handle_cors(self, chain: HandlerChain, context: RequestContext, response: Response):
        ...
        if not bucket_name or bucket_name not in self.bucket_cors_index.cors:
            # if the origin is allowed by localemu per default, adds default LS CORS headers
