...
from localemu.aws.api.s3 import BucketName, MultipartUploadId, PartNumber
...
from localemu.services.s3.utils import ChecksumHash, ObjectRange, get_s3_checksum
...
class EphemeralS3ObjectStore(S3ObjectStore):
    ...
    def copy(
        ...
        with self.open(src_bucket, src_object, mode="r") as src_stored_object:
            with self.open(dest_bucket, dest_object, mode="w") as dest_stored_object:
                dest_stored_object.write(src_stored_object)

        # Re-open in read mode for the caller (write lock released above)
        return self.open(dest_bucket, dest_object, mode="r")
