...
from localemu.config import S3_STATIC_WEBSITE_HOSTNAME, S3_VIRTUAL_HOSTNAME
...
from localemu.services.s3.utils import normalize_bucket_name
from localemu.utils.aws import arns
from localemu.utils.testutil import delete_all_s3_objects
...
class S3BucketProvider(ResourceProvider[S3BucketProperties]):
    ...
    def _setup_post_creation_attributes(self, model, region: str):
        ...
        model["WebsiteURL"] = (
            f"http://{model['BucketName']}.{S3_STATIC_WEBSITE_HOSTNAME}:{localemu_host().port}"
        )
