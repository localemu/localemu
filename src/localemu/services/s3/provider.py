...
from localemu.aws.api.s3 import (
    MFA,
...
from localemu.aws.api.s3 import NotImplemented as NotImplementedException
from localemu.aws.handlers import (
    modify_service_response,
    preprocess_request,
    serve_custom_service_request_handlers,
)
from localemu.constants import AWS_REGION_EU_WEST_1, AWS_REGION_US_EAST_1
from localemu.services.edge import ROUTER
from localemu.services.plugins import ServiceLifecycleHook
from localemu.services.s3.codec import AwsChunkedDecoder
from localemu.services.s3.constants import (
...
from localemu.services.s3.cors import S3CorsHandler, s3_cors_request_handler
from localemu.services.s3.exceptions import (
...
from localemu.services.s3.notifications import NotificationDispatcher, S3EventNotificationContext
from localemu.services.s3.presigned_url import validate_post_policy
from localemu.services.s3.storage.core import LimitedIterableStream, S3ObjectStore
...
from localemu.services.s3.validation import (
...
from localemu.services.s3.website_hosting import register_website_hosting_routes
from localemu.state import AssetDirectory, StateVisitor
from localemu.utils.aws.arns import s3_bucket_name
from localemu.utils.aws.aws_stack import get_valid_regions_for_service
from localemu.utils.collections import select_from_typed_dict
...
DEFAULT_S3_TMP_DIR = "/tmp/localemu-s3-storage"
...
class S3Provider(S3Api, ServiceLifecycleHook):
    ...
    def on_after_init(self):
        ...
        # Start background lifecycle expiration worker
        from localemu.services.s3.lifecycle import start_lifecycle_worker
        start_lifecycle_worker(self._storage_backend)
    ...
    def on_before_stop(self):
        from localemu.services.s3.lifecycle import stop_lifecycle_worker
        stop_lifecycle_worker()
    ...
    def _get_cross_account_bucket(
        ...
        *,
        ...
        if expected_bucket_owner and not re.fullmatch(r"\d{12}", expected_bucket_owner):
    ...
    def delete_bucket(
        ...
        **kwargs,
        ...
        # the bucket still has active multipart uploads
        if s3_bucket.multiparts:
            raise BucketNotEmpty(
    ...
    def head_bucket(
        ...
        **kwargs,
        ...
        # Health check endpoint: Terraform's S3 backend and various AWS SDKs perform a
        # HEAD on the special "__health_check__" bucket to verify connectivity before
        # issuing real requests.  Returning 200 here avoids a spurious NoSuchBucket error
        # that would break those client handshakes.  This bucket name is invalid per S3
        # naming rules, so it can never collide with a real user bucket.
        if bucket == "__health_check__":
    ...
    def get_object(
        ...
        # We deliberately do not call `.close()` on s3_stored_object to keep the read lock acquired.
        # Werkzeug calls `.close()` after finishing iterating over `__iter__`.
        # To prevent deadlocks if an exception occurs before return, we wrap in try/except.
        s3_stored_object = self._storage_backend.open(bucket_name, s3_object, mode="r")
        try:
            # Verify the object hasn't been modified between get_object and storage open
    ...
    def head_object(
        ...
        if (part_number := request.get("PartNumber")) is not None:
        ...
        **kwargs,
        ...
        if non_supported_precondition:
            LOG.warning(
                ...
                "LocalEmu does not support Directory Buckets yet."
            )
            raise NotImplementedException(
                "A header you provided implies functionality that is not implemented",
        ...
        for to_delete_object in objects:
            ...
            if s3_bucket.versioning_status is None:
                ...
                if found_object:
                    ...
                    # for a non-existing object being deleted. Use the standard empty-object etag (MD5 of empty
                    # content) to match the AWS behavior for delete notifications on non-existing keys.
                    self._notify(
                        context,
                        s3_bucket=s3_bucket,
                        s3_object=S3Object(
    ...
    def restore_object(
        ...
        **kwargs,
        ...
        if not restore_days:
            LOG.debug("LocalEmu does not support restore SELECT requests yet.")
        ...
        # But because it's instant in LocalEmu, we can directly send the Completed notification as well
    ...
    def complete_multipart_upload(
        ...
        **kwargs,
        ...
        # Try the new signature with mpu_size and validation_checksum first; fall back to the
        # legacy signature for multiparts restored from persistence that lack the new parameters.
        # TODO: remove the fallback for next major version (minor?)
        ...
        try:
            ...
            # Legacy multipart restored from persistence does not accept new kwargs
    ...
    def put_object_tagging(
        ...
        **kwargs,
        ...
        # Invalidate expiration cache for this object since tags may affect lifecycle rule matching
        self._expiration_cache.get(bucket, {}).pop(key, None)
    ...
    def get_object_torrent(
        ...
        **kwargs,
        ...
        # Torrent files are not actually generated locally.
        # Return a minimal valid response so the API call doesn't error.
        store, s3_bucket = self._get_cross_account_bucket(
            context, bucket, expected_bucket_owner=expected_bucket_owner
        )
        ...
        return GetObjectTorrentOutput(Body=b"", RequestCharged=None)
    ...
    def select_object_content(
        ...
        **kwargs,
        ...
        """S3 Select: query CSV/JSON objects with SQL expressions."""
        import csv
        import io
        import json
        ...
        # Read object data from storage backend (S3Object has no .value attribute)
        with self._storage_backend.open(bucket, s3_object, mode="r") as s3_stored_object:
            data = s3_stored_object.read() or b""
        ...
        records = []
        if "CSV" in input_serialization:
            csv_config = input_serialization.get("CSV", {})
            has_header = csv_config.get("FileHeaderInfo", "NONE") == "USE"
            ...
            for row in reader:
                ...
                if headers:
                    ...
                    records.append({"_" + str(i + 1): v for i, v in enumerate(row)})
        elif "JSON" in input_serialization:
            json_config = input_serialization.get("JSON", {})
            json_type = json_config.get("Type", "DOCUMENT")
            if json_type == "LINES":
        ...
        # Evaluate the S3 Select expression (WHERE / projection / aggregates)
        # against the parsed rows via DuckDB. Falls back to all rows if the
        # query cannot run, so a Select request never fails outright.
        from localemu.services.s3.select import evaluate_s3_select

        result_records = evaluate_s3_select(records, expression)

        # Format output
        output_lines = []
        if "JSON" in output_serialization:
            for rec in result_records:
                output_lines.append(json.dumps(rec))
            result_bytes = "\n".join(output_lines).encode("utf-8")
        elif "CSV" in output_serialization:
            out = io.StringIO()
            if result_records:
                writer = csv.DictWriter(out, fieldnames=result_records[0].keys())
                for rec in result_records:
                    writer.writerow(rec)
            result_bytes = out.getvalue().encode("utf-8")
        else:
            result_bytes = json.dumps(result_records).encode("utf-8")

        # Return as an event stream. The protocol serializer requires an
        # iterator (generator) for streaming event members, not a list.
        def _event_stream():
            yield {"Records": {"Payload": result_bytes}}
            yield {"Stats": {"Details": {
                "BytesScanned": len(data),
                "BytesProcessed": len(data),
                "BytesReturned": len(result_bytes),
            }}}
            yield {"End": {}}

        return {"Payload": _event_stream()}
    ...
    def write_get_object_response(
        ...
        **kwargs,
        ...
        """S3 Object Lambda: write a transformed response back to the caller.

        In local emulation, this is a no-op success since Object Lambda
        transformations are handled by the Lambda function itself.
        """
        # S3 Object Lambda: the Lambda function calls this API to send the
        # transformed object back. In local emulation, we accept and discard
        # since the full Object Lambda pipeline is not implemented.
        LOG.debug(
            "WriteGetObjectResponse: route=%s, token=%s, body_size=%d",
            request_route, request_token, len(body) if body else 0,
        )
        return None
