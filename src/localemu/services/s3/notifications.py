...
from localemu.aws.api.events import PutEventsRequestEntry
...
from localemu.services.s3.models import S3Bucket, S3DeleteMarker, S3Object
from localemu.utils.aws import arns
from localemu.utils.aws.arns import ARN_PARTITION_REGEX, get_partition, parse_arn, s3_bucket_arn
...
from localemu.utils.time import parse_timestamp, timestamp_millis
...
class S3EventNotificationContext:
    ...
    def from_request_context(
        ...
        return cls(
            ...
            source_ip=request_context.request.headers.get(
                "X-Forwarded-For", request_context.request.remote_addr or "127.0.0.1"
            ).split(",")[0].strip(),
...
def _matching_filter(
    ...
    """
    # Note: AWS S3 notification filtering only supports prefix and suffix rules (no wildcards)
...
class BaseNotifier:
    ...
    def _get_event_payload(
        ...
        record = EventRecord(
            ...
            userIdentity={"principalId": ctx.caller},
            requestParameters={
                "sourceIPAddress": ctx.source_ip
            },
            responseElements={
            ...
            s3=S3NotificationContent(
                ...
                bucket={
                    ...
                    "ownerIdentity": {
                        "principalId": ctx.bucket_account_id
                    },
                    "arn": f"arn:{partition}:s3:::{ctx.bucket_name}",
                },
                object={
                    "key": ctx.key_name,
                    "sequencer": f"{int(time.time() * 1000000):016X}",
                },
            ),
        )
        ...
        if "objectrestore:completed" in event_type:
            ...
            record["userIdentity"]["principalId"] = (
                f"AmazonCustomer:{ctx.bucket_account_id}"
            )
