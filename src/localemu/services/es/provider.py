...
from localemu.aws.api.es import (
    ARN,
...
from localemu.aws.api.es import BaseException as EsBaseException
from localemu.aws.api.opensearch import (
    ClusterConfig,
...
# Default Elasticsearch engine version reported when the caller does not pick one.
# Kept inline so the es provider has no dependency on the old Java-backed installer.
...
def _version_from_opensearch(
    ...
    if version is None:
        return None
    # OpenSearch uses the engine-prefixed form, e.g. "Elasticsearch_7.10" or
    # "OpenSearch_2.11". Always strip the prefix so the caller receives a bare
    # version string (ES clients cannot interpret the "OpenSearch_" prefix).
    if "_" in version:
        prefix, _, rest = version.partition("_")
        if prefix in ("Elasticsearch", "OpenSearch"):
            return rest
    return version
...
class EsProvider(EsApi):
    ...
    def update_elasticsearch_domain_config(
        ...
        if "ElasticsearchClusterConfig" in payload:
            ...
            instance_type = payload["ClusterConfig"].get("InstanceType")
            if instance_type is not None:
    ...
    def list_elasticsearch_versions(
        ...
        **kwargs,
        ...
        return ListElasticsearchVersionsResponse(
            ...
            NextToken=versions.get("NextToken"),
        )
