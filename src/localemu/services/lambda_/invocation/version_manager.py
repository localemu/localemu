...
from localemu.services.lambda_.invocation.logs import LogHandler, LogItem
from localemu.services.lambda_.invocation.metrics import (
    record_cw_metric_error,
    record_cw_metric_invocation,
)
from localemu.services.lambda_.invocation.cold_start_tracker import ColdStartTracker
from localemu.services.lambda_.invocation.runtime_executor import get_runtime_executor
from localemu.services.lambda_.ldm import LDMProvisioner
from localemu.utils.strings import long_uid, to_bytes, truncate
from localemu.utils.threads import FuncThread, start_thread

LOG = logging.getLogger(__name__)

_cold_start_tracker = ColdStartTracker()
...
class LambdaVersionManager:
    ...
    def start(self) -> VersionState:
        try:
            ...
            if (
                ...
                # InProgress to Successful happens too fast on LocalEmu (thanks to caching in prepare_version).
    ...
    def stop(self) -> None:
        ...
        _cold_start_tracker.mark_cold(self.function_version.qualified_arn)
    ...
    def update_provisioned_concurrency_config(
        ...
        with self.provisioned_state_lock:
            # LocalEmu limitation: cannot update provisioned concurrency while another update is in progress
            if (
    ...
    def invoke(self, *, invocation: Invocation) -> InvocationResult:
        ...
        # Simulate cold start delay if configured.
        # Skip for functions with provisioned concurrency (they are pre-warmed, matching AWS).
        if config.LAMBDA_COLD_START_DELAY and not (
            self.provisioned_state
            and self.provisioned_state.status == ProvisionedConcurrencyStatusEnum.READY
        ):
            qualified_arn = self.function_version.qualified_arn
            if _cold_start_tracker.should_cold_start(qualified_arn):
                LOG.info(
                    "Simulating cold start for %s: sleeping %.1fs",
                    qualified_arn,
                    config.LAMBDA_COLD_START_DELAY,
                )
                time.sleep(config.LAMBDA_COLD_START_DELAY)
