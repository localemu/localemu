...
from localemu.aws.api.lambda_ import ProvisionedConcurrencyStatusEnum, TooManyRequestsException
...
class CountingService:
    ...
    def get_invocation_lease(
        ...
        try:
            ...
            elif lease_type is not None:
                # Unknown lease type that was incremented — attempt on-demand
                # decrement to prevent permanent concurrency leak
                LOG.error(
                    "Unknown lease type for function %s: %s. "
                    "Attempting on-demand decrement to prevent concurrency leak.",
                    unqualified_function_arn,
                    lease_type,
                )
                on_demand_tracker.atomic_decrement(unqualified_function_arn)
            # lease_type is None means no lease was acquired (exception before yield)
