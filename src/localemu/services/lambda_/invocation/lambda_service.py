...
from localemu.services.lambda_.invocation.assignment import AssignmentService
from localemu.services.lambda_.invocation.counting_service import CountingService
from localemu.services.lambda_.invocation.event_manager import LambdaEventManager
...
from localemu.services.lambda_.invocation.models import LambdaStore, lambda_stores
...
from localemu.utils.archives import get_unzipped_size, is_zip_file
...
# Timeout in seconds when waiting for all shutdown futures during service stop.
SERVICE_SHUTDOWN_TIMEOUT_SECONDS = 5
...
class LambdaService:
    ...
    def stop(self) -> None:
        ...
        # 7.2 fix: scale timeout with number of active managers (min 5s, max 30s)
        scaled_timeout = max(SERVICE_SHUTDOWN_TIMEOUT_SECONDS, min(len(shutdown_futures) * 2, 30))
        _, not_done = concurrent.futures.wait(shutdown_futures, timeout=scaled_timeout)
        if not_done:
    ...
    def _await_active_version_manager(
        self, qualified_arn: str, function, version_qualifier: str
    ):
        """Poll for the version manager while the function warms up.

        Returns ``(version_manager, event_manager, None, None)`` once both
        managers exist. If the deadline expires (or the function reaches a
        terminal state like Failed), returns
        ``(None, None, current_state, captured_ValueError)`` so the caller
        can fall through to the existing ResourceConflictException path.
        """
        deadline = time.monotonic() + config.LAMBDA_INVOKE_WAIT_FOR_ACTIVE_SECONDS
        while True:
            try:
                vm = self.get_lambda_version_manager(qualified_arn)
                em = self.get_lambda_event_manager(qualified_arn)
                return vm, em, None, None
            except ValueError as wait_exc:
                live_version = function.versions.get(version_qualifier)
                state = live_version and live_version.config.state.state
                if state == State.Pending and time.monotonic() < deadline:
                    time.sleep(0.05)
                    continue
                return None, None, state, wait_exc
    ...
    def publish_version(self, function_version: FunctionVersion):
        ...
        # Successful happens too fast on LocalEmu (thanks to caching in prepare_version).
    ...
    def invoke(
        ...
        # Wait briefly for the version manager to be registered. The function
        # transitions Pending -> Active asynchronously (real AWS does the same),
        # so back-to-back create-function/invoke scripts otherwise race the
        # warm-up and get ResourceConflictException. Set
        # LAMBDA_INVOKE_WAIT_FOR_ACTIVE_SECONDS=0 for strict AWS parity.
        version_manager, event_manager, state, wait_error = (
            self._await_active_version_manager(qualified_arn, function, version_qualifier)
        )
        ...
        if wait_error is not None:
            if state == State.Failed:
                ...
                HINT_LOG.error(
                    ...
                    "Lambda requires Docker to be running. Please start Docker Desktop."
                )
                ...
                HINT_LOG.warning(
                    ...
                    f'"awsemu lambda wait function-active-v2 --function-name {function_name}" '
                    "Check out https://localemu.cloud/docs/lambda"
                )
            else:
        ...
        try:
            ...
            with self.lambda_version_manager_lock:
                ...
                if new_state.state == State.Active:
                    ...
                    # Stop the old event manager synchronously BEFORE starting the new one,
                    # to prevent both old and new pollers from processing the same event queue
                    if old_event_manager:
                        old_event_manager.stop_for_update()
                    ...
                    # Clean up the code from the failed version to prevent S3 code object leaks
                    failed_code = new_version_manager.function_version.config.code
                    if failed_code:
                        function = lambda_stores[function_version.id.account][
                            function_version.id.region
                        ].functions.get(function_version.id.function_name)
                        if function:
                            self.task_executor.submit(
                                destroy_code_if_not_used, failed_code, function
                            )
                elif (
