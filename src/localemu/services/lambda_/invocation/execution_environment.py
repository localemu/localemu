...
from localemu.aws.api.lambda_ import LogFormat
...
# Maximum STS session duration for Lambda execution role credentials (12 hours).
# AWS Lambda allows sessions up to 12 hours for the execution role.
STS_SESSION_DURATION_SECONDS = 43200
...
class ExecutionEnvironment:
    ...
    def get_environment_variables(self) -> dict[str, str]:
        ...
        env_vars = {
            ...
            # 3) Public LocalEmu endpoint
            "LOCALEMU_HOSTNAME": self.runtime_executor.get_endpoint_from_executor(),
            "EDGE_PORT": str(config.GATEWAY_LISTEN[0].port),
            # AWS_ENDPOINT_URL conditionally added below
            # 4) Internal LocalEmu runtime API
            "LOCALEMU_RUNTIME_ID": self.id,
            "LOCALEMU_RUNTIME_ENDPOINT": self.runtime_executor.get_runtime_endpoint(),
            # 5) Account of the function (necessary for extensions API)
            "LOCALEMU_FUNCTION_ACCOUNT_ID": self.function_version.id.account,
            # used by the init to spawn the x-ray daemon
            # LOCALEMU_USER conditionally added below
        }
        ...
        if logging_config.get("LogFormat") == LogFormat.JSON:
            ...
            # TODO: test this (currently not implemented in LocalEmu)
        ...
        # _HANDLER: zip functions carry it on ``config.handler``;
        # container-image functions resolve it from (in priority order):
        #   1. ImageConfig.Command (user-provided override at
        #      CreateFunction / UpdateFunctionConfiguration),
        #   2. the image's own ``CMD`` instruction.
        # The AWS Lambda runtime bootstrap (awslambdaric / RIE) reads
        # this env var to locate the user's handler; without it the
        # Python runtime exits on ``KeyError: '_HANDLER'`` and RAPID
        # times out during startup. We fall back to the image's CMD
        # via ``docker inspect`` — same contract real AWS follows.
        resolved_handler = self.function_version.config.handler
        if not resolved_handler:
            img_cfg = self.function_version.config.image_config
            if img_cfg and img_cfg.command:
                resolved_handler = img_cfg.command[0]
            else:
                img = self.function_version.config.image
                if img and img.image_uri:
                    try:
                        from localemu.utils.docker_utils import DOCKER_CLIENT
                        inspected = DOCKER_CLIENT.inspect_image(img.image_uri)
                        cmd = (inspected or {}).get("Config", {}).get("Cmd") or []
                        if cmd:
                            resolved_handler = cmd[0]
                    except Exception:
                        LOG.debug(
                            "Could not inspect image %s to resolve handler CMD",
                            img.image_uri, exc_info=True,
                        )
        if resolved_handler:
            env_vars["_HANDLER"] = resolved_handler
        ...
        if config.LAMBDA_INIT_USER is not None:
            env_vars["LOCALEMU_USER"] = config.LAMBDA_INIT_USER
        if config.LS_LOG in config.TRACE_LOG_LEVELS:
            env_vars["LOCALEMU_INIT_LOG_LEVEL"] = "info"
        if config.LAMBDA_INIT_POST_INVOKE_WAIT_MS:
            env_vars["LOCALEMU_POST_INVOKE_WAIT_MS"] = int(config.LAMBDA_INIT_POST_INVOKE_WAIT_MS)
        if config.LAMBDA_LIMITS_MAX_FUNCTION_PAYLOAD_SIZE_BYTES:
            env_vars["LOCALEMU_MAX_PAYLOAD_SIZE"] = int(
    ...
    def stop(self) -> None:
        ...
        with self.status_lock:
            ...
            # Cancel keepalive timer under lock to prevent race with release() assigning a new timer
    ...
    def release(self) -> None:
        ...
        with self.status_lock:
            ...
            self.status = RuntimeStatus.READY
            # Assign and start keepalive timer under lock to prevent race with stop()/reserve()
    ...
    def invoke(self, invocation: Invocation) -> InvocationResult:
        if self.status != RuntimeStatus.INVOKING:
            raise InvalidStatusException(
                f"Execution environment {self.id} must be in INVOKING status to invoke."
                f" Current status: {self.status}"
            )
        ...
        return self.runtime_executor.invoke(
            payload=invoke_payload, function_timeout=self.function_version.config.timeout
        )
