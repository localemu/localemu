...
from localemu.http import Response, route
from localemu.services.edge import ROUTER
from localemu.services.lambda_ import ldm
...
# Maximum Lambda function execution time as defined by AWS (seconds)
LAMBDA_MAX_TIMEOUT_SECONDS = 900

NAMESPACE = "/_localemu_lambda"
...
class ExecutorEndpoint(Endpoint):
    ...
    def invocation_response(self, request: Request, req_id: str) -> Response:
        ...
        if not self._try_set_invocation_result(result):
            LOG.warning("Received duplicate invocation response for request %s", req_id)
        return Response(status=HTTPStatus.ACCEPTED)

    def invocation_error(self, request: Request, req_id: str) -> Response:
        result = InvocationResult(req_id, request.data, is_error=True, logs=self.logs)
        if not self._try_set_invocation_result(result):
            LOG.warning("Received duplicate invocation error for request %s", req_id)
        return Response(status=HTTPStatus.ACCEPTED)

    def _try_set_invocation_result(self, result: InvocationResult) -> bool:
        """Attempt to set the invocation result. Returns False if already set."""
        if self.invocation_future is None or self.invocation_future.done():
            return False
        try:
    ...
    def invoke(
        self, payload: dict[str, str], function_timeout: int | None = None
    ) -> InvocationResult:
        ...
        elif function_timeout is not None:
            # Use the function's configured timeout plus a small buffer for container overhead
            timeout_seconds = function_timeout + invoke_timeout_buffer_seconds
        else:
            # Fallback: do not wait longer than the maximum Lambda timeout (900s) plus buffer
