...
# LocalEmu Lambda runtimes support policy
...
# 1. Update botocore and generate the Lambda API stubs using `python3 -m localemu.aws.scaffold upgrade`
...
# 8. Add the new runtime to the K8 image build: https://github.com/localemu/lambda-images
...
# LocalEmu can still provide best-effort support.

# TODO: Consider removing these as AWS is not using them anymore and they likely get outdated.
#  We currently use them in LocalEmu logs as bonus recommendation (DevX).
...
# A temporary list of missing runtimes not yet supported in LocalEmu. Used for modular updates.
MISSING_RUNTIMES = []

# An unordered list of all Lambda runtimes supported by LocalEmu.
