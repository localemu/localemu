...
from localemu.aws.api import CommonServiceException, RequestContext, ServiceException, handler
from localemu.aws.api.lambda_ import (
    AccountLimit,
    AccountUsage,
...
from localemu.aws.api.lambda_ import FunctionVersion as FunctionVersionApi
from localemu.aws.api.lambda_ import ServiceException as LambdaServiceException
...
from localemu.services.lambda_ import api_utils
...
from localemu.services.lambda_.api_utils import (
    ARCHITECTURES,
    STATEMENT_ID_REGEX,
    SUBNET_ID_REGEX,
    function_locators_from_arn,
)
from localemu.services.lambda_.event_source_mapping.esm_config_factory import (
    EsmConfigFactory,
)
from localemu.services.lambda_.event_source_mapping.esm_worker import (
    EsmState,
    EsmWorker,
)
from localemu.services.lambda_.event_source_mapping.esm_worker_factory import (
    EsmWorkerFactory,
)
from localemu.services.lambda_.event_source_mapping.pipe_utils import get_internal_client
from localemu.services.lambda_.invocation import AccessDeniedException
...
from localemu.services.lambda_.invocation.lambda_service import (
    LambdaService,
    create_image_code,
...
from localemu.services.lambda_.invocation.models import CapacityProvider as CapacityProviderModel
from localemu.services.lambda_.invocation.models import LambdaStore
from localemu.services.lambda_.invocation.runtime_executor import get_runtime_executor
from localemu.services.lambda_.lambda_utils import HINT_LOG
from localemu.services.lambda_.layerfetcher.layer_fetcher import LayerFetcher
from localemu.services.lambda_.provider_utils import (
    LambdaLayerVersionIdentifier,
    get_function_version,
    get_function_version_from_arn,
)
from localemu.services.lambda_.runtimes import (
    ALL_RUNTIMES,
    DEPRECATED_RUNTIMES,
...
from localemu.services.lambda_.urlrouter import FunctionUrlRouter
...
from localemu.utils.collections import PaginatedList, merge_recursive
from localemu.utils.event_matcher import validate_event_pattern
from localemu.utils.strings import get_random_hex, short_uid, to_bytes, to_str
...
class LambdaProvider(LambdaApi, ServiceLifecycleHook):
    ...
    def on_after_state_load(self):
        """Reconcile persisted Lambda state.

        This runs from an ``on_infra_start`` hook at priority 5 — BEFORE
        the HTTP gateway has finished routing to service providers. Any
        boto3 call to ``localhost:4566`` here races the gateway: in host
        mode the race is usually won, in Docker-in-Docker it's usually
        lost (the LocalEmu container's loopback adds just enough latency
        that the 5-second default timeout expires), and the version
        manager transitions every Lambda to ``Failed`` — which then
        makes every ``Invoke`` 409.

        Split into two phases:

        * **Phase 1 (synchronous, here).** Pure store mutations —
          refresh ``fn.instance_id``, flip each version to ``Pending``.
          No HTTP. Safe before the gateway is serving.
        * **Phase 2 (worker thread).** Everything that needs HTTP —
          ``create_function_version`` (downloads the code zip from
          LocalEmu's internal S3 bucket), provisioned-concurrency
          restore, and ESM worker bootstrap. The worker waits for the
          S3 service plugin to transition to ``RUNNING`` before making
          any call.

        Same pattern as ``CloudWatchProvider.on_after_state_load``.
        ``GetFunction`` returns ``Pending`` during the deferred window —
        identical to AWS cold-create semantics, so clients that
        ``wait function-active-v2`` continue to work.
        """
        ...
        # Phase 1 — synchronous store mutations, no HTTP.
        ...
        for account_id, account_bundle in lambda_stores.items():
            for region_name, state in account_bundle.items():
                for fn in state.functions.values():
                    ...
                    # Identifier unique to this function and LocalEmu instance.
                    # A LocalEmu restart or persistence load should create a new instance id.
                    ...
                    for fn_version in list(fn.versions.values()):
                        ...
                        if is_capacity_provider_latest:
                            continue
                        # Flip to Pending in-store so GetFunction reflects
                        # "restore in progress" until Phase 2 completes.
                    ...
                    pending_functions.append(fn)
                for esm in state.event_source_mappings.values():
                    pending_esms.append(esm)

        if not pending_functions and not pending_esms:
            return

        LOG.info(
            "Lambda persistence restore: scheduling deferred start "
            "for %d function(s), %d ESM(s)",
            len(pending_functions), len(pending_esms),
        )

        # Phase 2 — off-thread. Blocks until the S3 plugin is RUNNING so
        # boto3 calls over ``localhost:4566`` resolve cleanly.
        def _finish_restore(*_args, **_kwargs):
            if not poll_condition(
                lambda: SERVICE_PLUGINS.is_running("s3"),
                timeout=60, interval=0.25,
            ):
                LOG.warning(
                    "Lambda persistence restore: S3 service did not reach RUNNING "
                    "within 60s — restored functions will stay in Pending state"
                )
                return
            self._run_deferred_restore(pending_functions, pending_esms)

        start_worker_thread(_finish_restore, name="lambda-post-load-restore")

    def _run_deferred_restore(self, pending_functions, pending_esms) -> None:
        """Execute Phase 2 of ``on_after_state_load`` — the HTTP-making
        half. Runs on a worker thread after the S3 provider is
        confirmed RUNNING so boto3 download calls don't race the
        gateway. Each per-version / per-ESM failure is caught so one
        bad record doesn't abort the whole restore."""
        restored = failed = 0
        for fn in pending_functions:
            for fn_version in list(fn.versions.values()):
                if fn_version.config.state.state != State.Pending:
                    # Skip versions we didn't mark in Phase 1 (Deleting,
                    # capacity-provider-stopped, $LATEST ActiveNonInvocable).
                    continue
                try:
                    # 30s budget here — no longer racing the gateway, but
                    # waiting for Docker image pull if the runtime image
            ...
            try:
                ...
                if not poll_condition(
                    lambda arn=function_arn: (
                        get_function_version_from_arn(arn).config.state.state
                ...
                LOG.warning(
                    "Failed to restore ESM %s",
                    esm.get("UUID") or esm.get("FunctionArn"),
                    exc_info=LOG.isEnabledFor(logging.DEBUG),
                )

        LOG.info(
            "Lambda persistence restore complete: %d version(s) resumed, "
            "%d failed, %d ESM(s) processed",
            restored, failed, len(pending_esms),
        )
    ...
    _ENV_VAR_KEY_REGEX = re.compile(r"^[a-zA-Z_][a-zA-Z0-9_]*$")

    @staticmethod
    def _verify_env_variables(env_vars: dict[str, str]):
        for key in env_vars:
            if not LambdaProvider._ENV_VAR_KEY_REGEX.match(key):
                raise InvalidParameterValueException(
                    "Lambda was unable to configure your environment variables because the "
                    "environment variables you have provided contains invalid characters. "
            ...
            if layer_account_id == account_id:
                ...
                if layer is None or layer_version is None:
                    ...
                    if self.layer_fetcher is None:
                        # No layer fetcher configured. Match real AWS behaviour for a layer the
                        # caller has no permission to read: return AccessDenied. Same shape as the
                        # cross-region branch above.
                        user = "user/localemu-testing"
                        raise AccessDeniedException(
                            f"User: arn:{get_partition(region)}:iam::{account_id}:{user} is not authorized to perform: lambda:GetLayerVersion on resource: {layer_version_arn} because no resource-based policy allows the lambda:GetLayerVersion action"
                        )
    ...
    def create_function(
        ...
        with self.create_fn_lock:
            ...
            image = None
            image_config = None
            # Generate a runtime-specific version hash so different runtimes produce
            # distinct RuntimeVersionArn values, matching AWS behavior.
            runtime_version_hash = hashlib.sha256(
                (runtime or "unknown").encode("utf-8")
            ).hexdigest()
            runtime_version_config = RuntimeVersionConfig(
                RuntimeVersionArn=f"arn:{context.partition}:lambda:{context_region}::runtime:{runtime_version_hash}"
            )
            ...
            if "LoggingConfig" in request:
                ...
                if logging_config.get("LogFormat", None) == LogFormat.JSON:
                    ...
                    # Text format does not support log level keys; strip them unconditionally
                    logging_config.pop("ApplicationLogLevel", None)
                    logging_config.pop("SystemLogLevel", None)
    ...
    def _validate_runtime(self, package_type, runtime):
        ...
        if package_type == PackageType.Zip and runtime not in runtimes:
            ...
            if runtime in DEPRECATED_RUNTIMES:
                ...
                self._check_for_recommended_migration_target(runtime)
    ...
    def _check_for_recommended_migration_target(self, deprecated_runtime):
    ...
    def update_function_configuration(
        ...
        if "LoggingConfig" in request:
            ...
            # Text format does not support log level keys; strip them regardless of
            # previous format (covers JSON->Text transitions AND Text->Text with stale keys)
            if new_logging_config.get("LogFormat") == LogFormat.Text:
    ...
    def delete_function(
        ...
        **kwargs,
        ...
        if qualifier:
            ...
            for _qual, _url_cfg in list(function.function_url_configs.items()):
                try:
                    self.router.deregister_url_config(_url_cfg)
                except Exception:
                    LOG.debug(
                        "Failed to deregister Lambda URL %s on function delete",
                        getattr(_url_cfg, "url_id", None),
                    )
    ...
    def invoke(
        ...
        **kwargs,
        ...
        # Read the payload stream up-front so the CloudTrail recording hook,
        # which runs after the response, sees the actual bytes rather than a
        # consumed stream object. (The original SDK shape was IO[Blob] so the
        # provider could stream large bodies; we already buffer them here.)
        payload_bytes = payload.read() if payload else None
        if payload_bytes is not None and isinstance(context.service_request, dict):
            context.service_request["Payload"] = payload_bytes
        try:
    ...
    def list_versions_by_function(
        ...
        **kwargs,
        ...
        def _version_sort_key(v):
            return (0, 0) if v == "$LATEST" else (1, int(v))

        sorted_version_keys = sorted(function.versions.keys(), key=_version_sort_key)
        versions = [
            api_utils.map_to_list_response(
                api_utils.map_config_out(version=function.versions[v], return_qualified_arn=True)
            )
    ...
    def delete_alias(
        ...
        if version_alias and name in function.function_url_configs:
            ...
            self.router.deregister_url_config(url_config)
            LOG.debug(
        ...
        **kwargs,
        ...
        if routing_config is not None:
            ...
            if routing_config_dict := routing_config.get("AdditionalVersionWeights"):
                # Resolve target version for routing config validation
                target_version_str = function_version or alias.function_version
                target_fn_version = function.versions.get(target_version_str)
                if not target_fn_version:
                    raise ResourceNotFoundException(
                        f"Function version not found: {target_version_str}", Type="User"
                    )
                new_routing_config = self._create_routing_config_model(
                    routing_config_dict, target_fn_version
                )
    ...
    def validate_event_source_mapping(self, context, request):
        ...
        if service is None:
            ...
            if service == "sqs" and request["EventSourceArn"].endswith(".fifo"):
                service = "sqs-fifo"
        ...
        if qualifier:
            ...
            if api_utils.qualifier_is_alias(qualifier):
                ...
                if not fn_alias:
                    raise InvalidParameterValueException(
                        f"Function not found: {request_function_name}",
                        Type="User",
                    )
                ...
                raise CommonServiceException(
                    message=f"1 validation error detected: Value '{request_function_name}' at 'functionName' failed to satisfy constraint: Member must satisfy regular expression pattern: (arn:(aws[a-zA-Z-]*)?:lambda:)?([a-z]{{2}}(-gov)?-[a-z]+-\\d{{1}}:)?(\\d{{12}}:)?(function:)?([a-zA-Z0-9-_]+)(:(\\$LATEST|[a-zA-Z0-9-_]+))?",
    ...
    def _validate_invoke_mode(invoke_mode: str) -> None:
        ...
        if invoke_mode == InvokeMode.RESPONSE_STREAM:
            ...
            LOG.warning(
                "The invokeMode 'RESPONSE_STREAM' is not yet supported on LocalEmu. The property is only mocked, the execution will still be 'BUFFERED'"
            )
        ...
        **kwargs,
        ...
        if TAG_KEY_CUSTOM_URL in tags:
            ...
            if TAG_KEY_CUSTOM_URL_VALIDATOR.match(custom_id_tag_value):
                ...
                # strict parity with AWS over the localemu-only custom_id
                LOG.warning(
        ...
        # to the lambda itself (localemu-only).
        url_id: str
        ...
        host_definition = localemu_host(custom_port=config.GATEWAY_LISTEN[0].port)
        ...
        # Register in the URL router's O(1) index
        self.router.register_url_config(fn.function_url_configs[normalized_qualifier])
    ...
    def delete_function_url_config(
        ...
        **kwargs,
        ...
        # Remove from URL router's O(1) index before deleting from store
        self.router.deregister_url_config(url_config)
    ...
    # ===============  TAGS   ===============
    # =======================================
    # only Function, Event Source Mapping, and Code Signing Config (not currently supported by LocalEmu) ARNs
    ...
    def invoke_async(
        ...
        **kwargs,
        ...
        Delegates to the standard invoke with Event invocation type."""
        self.invoke(
            context=context,
            function_name=function_name,
        ...
        return InvokeAsyncResponse(Status=202)
