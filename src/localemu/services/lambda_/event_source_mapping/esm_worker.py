...
from localemu.services.lambda_.analytics import EsmExecutionStatus, esm_counter
...
from localemu.services.lambda_.provider_utils import get_function_version_from_arn
...
class EsmWorker:
    ...
    def poller_loop(self, *args, **kwargs):
        # Simulate the brief CREATING/ENABLING phase before transitioning to ENABLED
        with self._state_lock:
            if self.current_state in (EsmState.CREATING, EsmState.ENABLING):
                self.update_esm_state_in_store(self.current_state)
        # Brief delay to match AWS provisioning behavior; exit early on shutdown
    ...
    def update_esm_state_in_store(self, new_state: EsmState):
        ...
        with self._state_lock:
            esm = self._state.event_source_mappings.get(self.esm_config["UUID"])
            if esm is not None:
                esm.update(esm_update)
            else:
                LOG.debug(
                    "Skipping state update to %s for ESM %s: mapping already removed from store.",
                    new_state,
                    self.esm_config["UUID"],
                )
