...
from localemu.aws.api.pipes import PipeSourceSqsQueueParameters
from localemu.aws.api.sqs import MessageSystemAttributeName
...
from localemu.services.lambda_.event_source_mapping.senders.sender_utils import (
    batched,
)
...
from localemu.utils.strings import first_char_to_lower
...
class SqsPoller(Poller):
    ...
    def _register_client_hooks(self):
        ...
        def handle_message_count_override(params, context, **kwargs):
            ...
            context[HEADER_LOCALEMU_SQS_OVERRIDE_MESSAGE_COUNT] = str(requested_count)
        ...
        def handle_message_wait_time_seconds_override(params, context, **kwargs):
            ...
            context[HEADER_LOCALEMU_SQS_OVERRIDE_WAIT_TIME_SECONDS] = str(requested_wait)

        def handle_inject_headers(params, context, **kwargs):
            if override_message_count := context.pop(
                HEADER_LOCALEMU_SQS_OVERRIDE_MESSAGE_COUNT, None
            ):
                params["headers"][HEADER_LOCALEMU_SQS_OVERRIDE_MESSAGE_COUNT] = (
                    override_message_count
                )

            if override_wait_time := context.pop(
                HEADER_LOCALEMU_SQS_OVERRIDE_WAIT_TIME_SECONDS, None
            ):
                params["headers"][HEADER_LOCALEMU_SQS_OVERRIDE_WAIT_TIME_SECONDS] = (
                    override_wait_time
                )
    ...
    def poll_events(self) -> None:
        ...
        # * Higher batch collection durations and no. of records retrieved per request mean fewer calls to the LocalEmu gateway [3] when polling an event-source [4].
        # * LocalEmu shutdown works because the LocalEmu gateway shuts down and terminates the open connection.
        ...
        # - Blocks a LocalEmu gateway thread (default 1k) for every open connection, which could lead to resource contention if used at scale.
        #
        # Refs / Notes:
        ...
        # [2] PR (2025-02): https://github.com/localemu/localemu/pull/12002
        # [3] Note: Under high volumes of requests, the LocalEmu gateway becomes a major performance bottleneck.
        ...
        # imposed on payloads sent to a Lambda as well as LocalEmu Lambdas failing to handle large payloads efficiently.
...
def transform_into_events(messages: list[dict]) -> list[dict]:
    ...
    for message in messages:
        ...
        #  localemu.services.lambda_.event_source_listeners.sqs_event_source_listener.SQSEventSourceListener._send_event_to_lambda
...
# TTL cache for queue URL lookups. Entries expire after QUEUE_URL_CACHE_TTL_SECONDS
# to handle queue recreation (delete + create) scenarios where the URL may change.
QUEUE_URL_CACHE_TTL_SECONDS = 60
_queue_url_cache: dict[str, tuple[str, float]] = {}
_queue_url_cache_lock = threading.Lock()


def get_queue_url(queue_arn: str) -> str:
    now = time.monotonic()
    with _queue_url_cache_lock:
        cached = _queue_url_cache.get(queue_arn)
        if cached is not None:
            url, cached_at = cached
            if now - cached_at < QUEUE_URL_CACHE_TTL_SECONDS:
                return url
    ...
    with _queue_url_cache_lock:
        _queue_url_cache[queue_arn] = (queue_url, now)
