...
from localemu.services.lambda_.event_source_mapping.pollers.sqs_poller import get_queue_url
from localemu.services.lambda_.event_source_mapping.senders.sender_utils import (
    batched,
)
from localemu.utils.aws.arns import parse_arn, s3_bucket_name
from localemu.utils.backoff import ExponentialBackoff
from localemu.utils.batching import Batcher
...
class StreamPoller(Poller):
    ...
    def poll_events_from_shard(self, shard_id: str, shard_iterator: str):
        ...
        if not (next_shard_iterator := get_records_response.get("NextShardIterator")):
            ...
            # has expired on the DynamoDB Streams backend, hence we should re-initialise.
            self.shards = self.initialize_shards()
        else:
    ...
    def forward_events_to_target(self, shard_id, records):
        ...
        # localemu.services.lambda_.event_source_listeners.kinesis_event_source_listener.KinesisEventSourceListener._filter_records
