...
from localemu.aws.api.sns import MessageAttributeMap, TopicAttributesMap
from localemu.aws.connect import connect_to
from localemu.config import external_service_url
from localemu.services.sns import constants as sns_constants
from localemu.services.sns.certificate import SNS_SERVER_PRIVATE_KEY
from localemu.services.sns.executor import TopicPartitionedThreadPoolExecutor
from localemu.services.sns.filter import SubscriptionFilter
...
from localemu.services.sns.utils import get_topic_subscriptions, snake_to_pascal_case
...
from localemu.utils.aws.aws_responses import create_sqs_system_attributes
from localemu.utils.aws.client_types import ServicePrincipal
from localemu.utils.aws.dead_letter_queue import sns_error_to_dead_letter_queue
from localemu.utils.bootstrap import is_api_enabled
from localemu.utils.cloudwatch.cloudwatch_util import store_cloudwatch_logs
from localemu.utils.objects import not_none_or
from localemu.utils.strings import long_uid, md5, to_bytes, to_str
...
class TopicPublisher(abc.ABC):
    ...
    def publish(self, context: SnsPublishContext, subscriber: SnsSubscription):
        ...
        Discussion here: https://github.com/localemu/localemu/pull/7267#discussion_r1056873437
...
class EmailJsonTopicPublisher(TopicPublisher):
    ...
    def _publish(self, context: SnsPublishContext, subscriber: SnsSubscription):
        ...
        if endpoint := subscriber.get("Endpoint"):
            sender = config.SNS_SES_SENDER_ADDRESS or "no-reply@localemu.cloud"
...
class ApplicationTopicPublisher(TopicPublisher):
    ...
    This is not directly implemented yet in LocalEmu, we save the message to be retrieved later from an internal
    endpoint.
...
class SmsTopicPublisher(TopicPublisher):
    ...
    This is not directly implemented yet in LocalEmu, we only save the message.
...
def store_delivery_log(
    ...
    # seems like you need to configure on the Console, leave it on by default now in LocalEmu
