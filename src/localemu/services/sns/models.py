...
class BoundedList(list):
    """A list that evicts oldest items when exceeding maxlen."""

    def __init__(self, *args, maxlen: int = 10000, **kwargs):
...
class SnsStore(BaseStore):
    ...
    # list of sent SMS messages — bounded to prevent unbounded memory growth (1.1 fix)
    sms_messages: list[dict[str, Any]] = LocalAttribute(
        default=lambda: BoundedList(maxlen=10000)
    )
    ...
    # FIFO deduplication: maps topic_arn -> {dedup_id -> (message_id, sequence_number, timestamp)}
    fifo_dedup_cache: dict[str, dict[str, tuple[str, str, float]]] = LocalAttribute(default=dict)

    # Lock protecting all mutations of fifo_dedup_cache. Without this lock, two concurrent
    # publishes of the same deduplication id could both observe a cache miss and both deliver
    # the message, violating FIFO exactly-once semantics (SNS-1 / SNS-3).
    fifo_dedup_lock: threading.Lock = LocalAttribute(default=threading.Lock)
