...
from localemu.aws.api.sns import (
    ActionsList,
    AmazonResourceName,
...
from localemu.constants import AWS_REGION_US_EAST_1, DEFAULT_AWS_ACCOUNT_ID
...
from localemu.services.sns.analytics import internal_api_calls
from localemu.services.sns.certificate import SNS_SERVER_CERT
from localemu.services.sns.constants import (
    ATTR_TYPE_REGEX,
...
from localemu.services.sns.filter import FilterPolicyValidator
from localemu.services.sns.models import (
    SMS_ATTRIBUTE_NAMES,
...
from localemu.services.sns.publisher import (
    PublishDispatcher,
    SnsBatchPublishContext,
    SnsPublishContext,
)
from localemu.services.sns.utils import (
...
class SnsProvider(SnsApi, ServiceLifecycleHook):
    ...
    def delete_topic(self, context: RequestContext, topic_arn: topicARN, **kwargs) -> None:
        ...
        store = self.get_store(context.account_id, context.region)
        topic = store.topics.pop(topic_arn, None)
        # Clean up the topic's subscriptions from the store so that they do not linger as
        # orphaned entries in list_subscriptions / get_subscription_attributes (SNS-4).
        if topic is not None:
            for subscription_arn in list(topic.get("subscriptions", []) or []):
                store.subscriptions.pop(subscription_arn, None)
                store.subscription_filter_policy.pop(subscription_arn, None)
            # Drop any subscription tokens that still point at the removed subscriptions.
            stale_tokens = [
                token
                for token, sub_arn in store.subscription_tokens.items()
                if sub_arn not in store.subscriptions
            ]
            for token in stale_tokens:
                store.subscription_tokens.pop(token, None)
        # Also drop any FIFO dedup cache entries scoped to this topic.
        with store.fifo_dedup_lock:
            store.fifo_dedup_cache.pop(topic_arn, None)
    ...
    def unsubscribe(
        ...
        if subscription["Protocol"] in ["http", "https"]:
            ...
            publish_ctx = SnsPublishContext(
                ...
                store=store,
                request_headers=context.request.headers,
                # TODO: add the topic attributes once we ported them from moto to LocalEmu
    ...
    def get_subscription_attributes(
        ...
        # The subscription is stored in the store of the ACCOUNT/REGION that owns the
        # subscription (derived from the ARN), not necessarily in the caller's store. Using the
        # caller's store would make cross-account / cross-region subscription lookups fail
        # spuriously (SNS-5).
        try:
            parsed_sub_arn = parse_arn(subscription_arn)
        except InvalidArnException:
            raise NotFoundException("Subscription does not exist")
        store = self.get_store(
            account_id=parsed_sub_arn["account"], region=parsed_sub_arn["region"]
        )
    ...
    def list_subscriptions(
        ...
        # AWS ListSubscriptions returns subscriptions across all regions for the caller's account.
        # The previous implementation only returned the caller's (account, region) store, which
        # meant subscriptions created in a different region were invisible (SNS-6).
        subscriptions = []
    ...
    def publish(
        ...
        **kwargs,
        ...
        # FIFO deduplication: compute effective dedup ID and check 5-minute window. All
        # reads/writes of the shared dedup cache happen under ``store.fifo_dedup_lock`` so that
        # concurrent publishers cannot both observe a cache miss for the same dedup id and
        # deliver the message twice (SNS-1).
        if is_fifo and topic_or_target_arn:
            effective_dedup_id = message_deduplication_id
            if not effective_dedup_id:
                # Content-based deduplication: SHA-256 of message body
                import hashlib
                msg_str = message if isinstance(message, str) else json.dumps(message, sort_keys=True)
                effective_dedup_id = hashlib.sha256(msg_str.encode("utf-8")).hexdigest()

            # Check dedup cache
            with store.fifo_dedup_lock:
                dedup_cache = store.fifo_dedup_cache.setdefault(topic_or_target_arn, {})
                now = time.time()
                # Prune expired entries (older than 5 minutes)
                expired_keys = [k for k, v in dedup_cache.items() if now - v[2] > 300]
                for k in expired_keys:
                    del dedup_cache[k]

                cached = dedup_cache.get(effective_dedup_id)
                if cached:
                    # Duplicate within 5-minute window — return same message ID
                    return PublishResponse(MessageId=cached[0], SequenceNumber=cached[1])
        ...
        # Store in dedup cache after successful publish (under lock — SNS-1)
        if is_fifo and topic_or_target_arn:
            with store.fifo_dedup_lock:
                dedup_cache = store.fifo_dedup_cache.setdefault(topic_or_target_arn, {})
                dedup_cache[effective_dedup_id] = (
                    message_ctx.message_id, message_ctx.sequencer_number, time.time()
                )
    ...
    def publish_batch(
        ...
        **kwargs,
        ...
        # FIFO validations only depend on the topic/batch as a whole — validate ONCE before the
        # per-entry loop instead of re-checking every iteration (O(n^2) → O(n)) (SNS-2).
        ...
        total_batch_size = 0
        message_contexts = []
        # For FIFO topics we also take the dedup lock for the entire batch so that all
        # check-then-set operations against the dedup cache are atomic (SNS-3).
        dedup_lock_ctx = store.fifo_dedup_lock if is_fifo else contextlib.nullcontext()
        with dedup_lock_ctx:
            dedup_cache = None
            if is_fifo:
                dedup_cache = store.fifo_dedup_cache.setdefault(topic_arn, {})
                now_ts = time.time()
                # Prune expired entries once for the whole batch
                expired = [k for k, v in dedup_cache.items() if now_ts - v[2] > 300]
                for k in expired:
                    del dedup_cache[k]
            ...
            for entry_index, entry in enumerate(publish_batch_request_entries, start=1):
                ...
                # FIFO batch deduplication (cache access is guarded by ``dedup_lock_ctx``)
                dedup_id = None
                if is_fifo:
                    import hashlib
                    ...
                    if not dedup_id:
                        msg_str = (
                            ...
                            else json.dumps(entry["Message"], sort_keys=True)
                        )
                        dedup_id = hashlib.sha256(msg_str.encode("utf-8")).hexdigest()
                    cached = dedup_cache.get(dedup_id)
                    if cached:
                        response["Successful"].append(PublishBatchResultEntry(
                            Id=entry["Id"], MessageId=cached[0], SequenceNumber=cached[1],
                        ))
                        continue
                ...
                if is_fifo:
                    ...
                    dedup_cache[dedup_id] = (
                        msg_ctx.message_id, msg_ctx.sequencer_number, time.time(),
                    )
...
def _register_sns_api_resource(router: Router):
    """Register the retrospection endpoints as internal LocalEmu endpoints."""
...
class SNSServicePlatformEndpointMessagesApiResource(SNSInternalResource):
    ...
    This is registered as a LocalEmu internal HTTP resource.
