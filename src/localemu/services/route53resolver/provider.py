...
import localemu.services.route53resolver.utils
from localemu.aws.api import RequestContext
from localemu.aws.api.route53resolver import (
    Action,
...
from localemu.services.route53resolver.models import Route53ResolverStore, route53resolver_stores
from localemu.services.route53resolver.utils import (
...
class Route53ResolverProvider(Route53ResolverApi):
        ...
        **kwargs,
        ...
        if operation == FirewallDomainUpdateOperation.ADD:
            if not firewall_domains:
                ...
                store.firewall_domains[firewall_domain_list_id].extend(domains)
                ...
                for domain in domains:
                    if domain in firewall_domains:
                        ...
                        raise ValidationException(
                            f"[RSLVR-02502] The following domains don't exist in the DNS Firewall domain list '{firewall_domain_list_id}'. You can't delete a domain that isn't in a domain list. Example unknown domain: '{domain}'. Trace Id: '{localemu.services.route53resolver.utils.get_trace_id()}'"
                        )
    ...
    def create_firewall_rule(
        ...
        **kwargs,
        ...
        if firewall_rule_group_id not in store.firewall_rules:
            raise ResourceNotFoundException(
                f"Can't find the resource with ID '{firewall_rule_group_id}'. Trace Id: '{localemu.services.route53resolver.utils.get_trace_id()}'"
            )
    ...
    def list_firewall_rules(
        ...
        **kwargs,
        ...
        # Pagination: next_token is the string index into the filtered list
        start_idx = 0
        if next_token:
            try:
                ...
                start_idx = 0
        page_size = max_results if max_results else len(firewall_rules)
        page = firewall_rules[start_idx : start_idx + page_size]
        new_next_token = (
            str(start_idx + page_size)
            if (start_idx + page_size) < len(firewall_rules)
            else None
        )
        return ListFirewallRulesResponse(FirewallRules=page, NextToken=new_next_token)
        ...
        **kwargs,
        ...
        for firewall_rule_group_association in store.firewall_rule_group_associations.values():
            if (
                ...
                raise ValidationException(
                    f"[RSLVR-02302] This DNS Firewall rule group can't be associated to a VPC: '{vpc_id}'. It is already associated to VPC '{firewall_rule_group_id}'. Try again with another VPC or DNS Firewall rule group. Trace Id: '{localemu.services.route53resolver.utils.get_trace_id()}'"
                )
