...
from localemu.services.route53resolver.utils import get_firewall_config_id, validate_vpc
...
class Route53ResolverStore(BaseStore):
    ...
    def get_firewall_rule_group(self, id):
        ...
        if not firewall_rule_group:
            raise ResourceNotFoundException(
                f"Can't find the resource with ID '{id}'. Trace Id: '{localemu.services.route53resolver.utils.get_trace_id()}'"
            )
    ...
    def get_firewall_rule_group_association(self, id):
        ...
        if not firewall_rule_group_association:
            raise ResourceNotFoundException(
                f"[RSLVR-02025] Can't find the resource with ID '{id}'. Trace Id: '{localemu.services.route53resolver.utils.get_trace_id()}'"
            )
    ...
    def get_resolver_query_log_config(self, id):
        ...
        if not resolver_query_log_config:
            raise ResourceNotFoundException(
                f"[RSLVR-01601] The specified query logging configuration doesn't exist. Trace Id: '{localemu.services.route53resolver.utils.get_trace_id()}'"
            )
    ...
    def delete_resolver_query_log_config_associations(
        ...
        for association in self.resolver_query_log_config_associations.values():
            if not (
                ...
                raise ResourceNotFoundException(
                    f"[RSLVR-01602] The specified query logging configuration association doesn't exist. Trace Id: '{localemu.services.route53resolver.utils.get_trace_id()}'"
                )
