...
from localemu.aws.api.secretsmanager import (
...
from localemu.utils.time import today_no_time
...
class SecretsmanagerProvider(SecretsmanagerApi):
    ...
    def list_secret_version_ids(
        ...
        response: ListSecretVersionIdsResponse = ListSecretVersionIdsResponse(**json.loads(secrets))

        # SM-4: add pagination. The patched moto method returns all versions at once;
        # we slice here according to MaxResults / NextToken (integer offset encoded
        # as a string, consistent with the pattern used elsewhere in LocalEmu).
        ...
        if next_token:
            try:
                ...
                start = 0
        end = len(versions) if max_results is None else min(len(versions), start + max_results)
        response["Versions"] = versions[start:end]
        if end < len(versions):
    ...
    def put_secret_value(
        ...
        # SM-2: do NOT base64-encode here. The botocore serializer has already base64-
        # encoded the raw bytes the caller passed (SecretBinary is wire-serialized as
        # base64 before reaching the provider). Re-encoding would double-encode and
        # hand garbled data to moto, which stores the string verbatim.
    ...
    def update_secret(
        ...
        secret_string = request.get("SecretString")
        secret_binary = request.get("SecretBinary")
        # SM-3: botocore already base64-serialises SecretBinary on the wire. Do not
        # re-encode on the server side — that would produce garbled binary secrets.
        description = request.get("Description")
        kms_key_id = request.get("KmsKeyId")
        client_req_token = request.get("ClientRequestToken")
        self._raise_if_invalid_secret_id(secret_id)
        # SM-1: ClientRequestToken is required by AWS only when the secret value
        # is being modified. For description-only or KMS-only updates, it must
        # not be required.
