...
from localemu.testing.pytest.fixtures import StackDeployError
...
from localemu.aws.api.cloudformation import Capability
...
class InfraProvisioner:
    ...
    def get_asset_bucket(self):
        ...
        return f"localemu-testing-{account}-{region}"
    ...
    def provision(self, skip_deployment: bool | None = False):
        ...
        if all(
            ...
            # TODO: in localemu we might want to do a delete/create
            #  but generally this won't be a common use case when developing against LocalEmu
            is_update = True
    ...
    def teardown(self):
        ...
        if not is_aws_cloud():
            # TODO: also clean up s3 bucket on localemu?
    ...
    def _template_bucket_name(self):
        ...
        region = self.aws_client.sts.meta.region_name
        return f"localemu-testing-assets-{account_id}-{region}"
    ...
    def get_asset_bucket_cdk(stack: cdk.Stack):
        return cdk.Fn.join("-", ["localemu", "testing", stack.account, stack.region])
