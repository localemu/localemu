...
from localemu.aws.api.cloudformation import CreateChangeSetInput, Parameter
from localemu.aws.api.ec2 import CreateSecurityGroupRequest, CreateVpcEndpointRequest, VpcEndpoint
from localemu.aws.connect import ServiceLevelClientFactory
...
from localemu.testing.aws.cloudformation_utils import load_template_file, render_template
from localemu.testing.aws.util import get_lambda_logs, is_aws_cloud, wait_for_user
...
from localemu.utils.aws.client import SigningHttpClient
from localemu.utils.aws.resources import create_dynamodb_table
from localemu.utils.bootstrap import is_api_enabled
from localemu.utils.collections import ensure_list, select_from_typed_dict
from localemu.utils.functions import call_safe, run_safe
from localemu.utils.http import safe_requests as requests
from localemu.utils.id_generator import ResourceIdentifier, localemu_id_manager
from localemu.utils.json import CustomEncoder, json_safe
from localemu.utils.net import wait_for_port_open
from localemu.utils.strings import short_uid, to_str
from localemu.utils.sync import ShortCircuitWaitException, poll_condition, retry, wait_until
...
def kms_create_grant(kms_create_key, aws_client):
    ...
    def _create_grant(**kwargs):
        # Just a random ARN, since KMS in LocalEmu currently doesn't validate GranteePrincipal,
...
def create_lambda_function_aws(aws_client):
    ...
    def _create_lambda_function(**kwargs):
        ...
        # localemu should normally not require the retries and will just continue here
...
#
# We would like to have localemu tests to pass not just against localemu, but also against AWS to make sure
...
def acm_request_certificate(aws_client_factory):
    ...
    def factory(**kwargs) -> str:
        if "DomainName" not in kwargs:
            kwargs["DomainName"] = f"test-domain-{short_uid()}.localhost"
...
def setup_sender_email_address(ses_verify_identity):
    ...
    def inner(sender_email_address: str | None = None) -> str:
        if is_aws_cloud():
            ...
            # overwrite the given parameters with localemu specific ones
...
def pytest_collection_modifyitems(config: Config, items: list[Item]):
    only_localemu = pytest.mark.skipif(
        is_aws_cloud(),
        reason="test only applicable if run against localemu",
    )
    for item in items:
        for mark in item.iter_markers():
            if mark.name.endswith("only_localemu"):
                item.add_marker(only_localemu)
...
def assert_host_customisation(monkeypatch):
    localemu_host = "foo.bar"
    monkeypatch.setattr(
        config, "LOCALEMU_HOST", config.HostAndPort(host=localemu_host, port=8888)
    )
    ...
    def asserter(
        ...
        *,
        ...
        if custom_host is not None:
            ...
            assert localemu_host not in url
        else:
            assert localemu_host in url, f"Could not find `{localemu_host}` in `{url}`"
...
def set_resource_custom_id():
    ...
    def _set_custom_id(resource_identifier: ResourceIdentifier, custom_id):
        localemu_id_manager.set_custom_id(
    ...
    for resource_identifier in set_ids:
        localemu_id_manager.unset_custom_id(resource_identifier)
