...
from localemu.testing.snapshots.transformer import (
    JsonpathTransformer,
    RegexTransformer,
    TransformContext,
)
...
from localemu.services.stepfunctions.asl.eval.event.logging import is_logging_enabled_for
from localemu.services.stepfunctions.asl.utils.encoding import to_json_str
from localemu.services.stepfunctions.asl.utils.json_path import NoSuchJsonPathError, extract_json
...
_SAMPLING_INTERVAL_SECONDS_LOCALEMU: Final[float] = 0.2
...
def _get_sampling_interval_seconds() -> int | float:
    return (
        ...
        if is_aws_cloud()
        else _SAMPLING_INTERVAL_SECONDS_LOCALEMU
    )
