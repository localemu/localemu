...
def check_provider_signature(sub_class: type, base_class: type, method_name: str) -> None:
    ...
    try:
        ...
        # ``inspect.getfullargspec`` returns annotations as raw strings when
        # the defining module uses ``from __future__ import annotations``.
        # Resolve them via :func:`typing.get_type_hints` so the comparison
        # below operates on real type objects, not strings.
        sub_annotations = _resolve_annotations(sub_function, sub_spec.annotations)
        base_annotations = _resolve_annotations(base_function, base_spec.annotations)

        error_msg = f"{sub_class.__name__}#{method_name} breaks with {base_class.__name__}#{method_name}."
        ...
        for kwarg in sub_annotations:
            if kwarg == "return":
                assert sub_annotations[kwarg] == base_annotations[kwarg]
            else:
                ...
                if kwarg not in base_annotations:
                ...
                sub_type = _remove_optional(sub_annotations[kwarg])
                base_type = _remove_optional(base_annotations[kwarg])
...
def _resolve_annotations(fn: FunctionType, fallback: dict) -> dict:
    """Return type-resolved annotations for ``fn``.

    Calls :func:`typing.get_type_hints`, which evaluates string annotations
    produced by ``from __future__ import annotations`` (PEP 563) into real
    type objects. Falls back to the raw ``fallback`` dict if resolution
    fails for any reason (forward reference to an unknown name, etc.).
    """
    try:
