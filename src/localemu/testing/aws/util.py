...
from localemu.aws.forwarder import create_http_request
from localemu.aws.protocol.parser import create_parser
from localemu.aws.spec import LOCALEMU_BUILTIN_DATA_PATH, load_service
...
from localemu.utils.aws.request_context import get_account_id_from_request
...
# Supports test executions against both LocalEmu and production AWS
...
def base_aws_session() -> boto3.Session:
    ...
    session._loader.search_paths.insert(0, LOCALEMU_BUILTIN_DATA_PATH)
    return session
...
def secondary_aws_session() -> boto3.Session:
    ...
    if not is_aws_cloud():
        ...
        # LocalEmu
        session._loader.search_paths.append(LOCALEMU_BUILTIN_DATA_PATH)
    return session
