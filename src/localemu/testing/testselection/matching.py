...
from localemu.aws.scaffold import is_keyword
...
DEFAULT_SEARCH_PATTERNS = (
    r"localemu/services/([^/]+)/.+",
    r"localemu/aws/api/([^/]+)/__init__\.py",
...
def generic_service_test_matching_rule(
    ...
    if api_dependencies is None:
        from localemu.utils.bootstrap import API_DEPENDENCIES, API_DEPENDENCIES_OPTIONAL
...
MATCHING_RULES: list[MatchingRule] = [
    ...
    Matchers.glob("localemu/config.py").full_suite(),
    Matchers.glob("localemu/constants.py").full_suite(),
    Matchers.glob("localemu/plugins.py").full_suite(),
    Matchers.glob("localemu/utils/**").full_suite(),
    # testing
    Matchers.glob("localemu/testing/**").full_suite(),
