...
# We assume that test selection scripts are only run on repositories under the localemu organization
REPO_OWNER = "localemu"
...
def get_pr_details_from_url(repo_name: str, pr_url: str, token: str) -> (str, str):
    ...
    Example pr_url: https://github.com/localemu/localemu/pull/1
    """
    parts = pr_url.split("/")
    pr_number = parts[-1]
