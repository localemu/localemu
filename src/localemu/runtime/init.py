"""Module for initialization hooks https://localemu.cloud/docs/env-vars"""
...
class ScriptRunner(Plugin):
    ...
    namespace = "localemu.init.runner"
    suffixes = []
...
class PythonScriptRunner(ScriptRunner):
    ...
    def run(self, path: str) -> None:
        LOG.info("Executing init script via exec(): %s", path)
...
@hooks.on_infra_start(priority=5)
def _init_persistence():
    """Load persisted state early — before services start processing requests."""
    try:
        from localemu.state.persistence import register_persistence

        register_persistence()
    except Exception:
        LOG.warning("Persistence initialization failed", exc_info=True)
...
def main():
    ...
    Run the init scripts for a particular stage. For example, to run all boot scripts run::
        ...
        python -m localemu.runtime.init BOOT
