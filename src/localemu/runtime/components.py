"""
This package contains code to define and manage the core components that make up a ``LocalemuRuntime``.
These include:
  - A ``Gateway``
...
class Components(Plugin):
    """
    A Plugin that allows a specific localemu runtime implementation (aws, snowflake, ...) to expose its
    own component factory.
    """

    namespace = "localemu.runtime.components"
