...
from localemu.runtime import events, hooks
from localemu.utils import files, functions, net, sync, threads
...
class LocalemuRuntime:
    """
    The localemu runtime. It has the following responsibilities:

      - Manage localemu filesystem directories
      - Execute runtime lifecycle hook plugins from ``localemu.runtime.hooks``.
      - Manage the localemu SSL certificate
    ...
    def __init__(self, components: Components):
        ...
        # the one from the current runtime. This will allow us to truly instantiate multiple localemu
        ...
        # TODO: move away from `localemu.runtime.events` and instantiate new `threading.Event()` here
        #  instead
    ...
    def run(self):
        ...
        # localemu runtime). this is necessary for disabling certain hooks that may run in the context of
        # the CLI host mode. TODO: should not be needed over time.
        os.environ[constants.LOCALEMU_INFRA_PROCESS] = "1"
...
def create_from_environment() -> LocalemuRuntime:
    """
    Creates a new runtime instance from the current environment. It uses a plugin manager to resolve the
    necessary components from the ``localemu.runtime.components`` plugin namespace to start the runtime.

    :return: a new LocalemuRuntime instance
    """
    ...
    if config.RUNTIME_COMPONENTS:
        try:
            ...
            return LocalemuRuntime(component)
    ...
    return LocalemuRuntime(components[0])
